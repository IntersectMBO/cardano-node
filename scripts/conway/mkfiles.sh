...
cp scripts/conway/template-files/alonzo.json "${ROOT}/genesis.alonzo.spec.json"
cp scripts/conway/template-files/conway.json "${ROOT}/genesis.conway.spec.json"

cp scripts/conway/template-files/config.json "${ROOT}/"
sed -i "${ROOT}/config.json" \
     -e 's/"ByronGenesisFile": "byron-genesis.json"/"ByronGenesisFile": "genesis\/byron\/genesis.json"/g' \
     -e 's/"ShelleyGenesisFile": "shelley-genesis.json"/"ShelleyGenesisFile": "genesis\/shelley\/genesis.json"/g' \
     -e 's/"AlonzoGenesisFile": "alonzo-genesis.json"/"AlonzoGenesisFile": "genesis\/shelley\/genesis.alonzo.json"/g' \
     -e 's/"ConwayGenesisFile": "conway-genesis.json"/"ConwayGenesisFile": "genesis\/shelley\/genesis.conway.json"/g' \
     ${ROOT}/config.json

$CARDANO_CLI conway genesis create-testnet-data \
  --genesis-keys 3 \
  --pools 3 \
  --stake-delegators 3 \
  --drep-keys 3 \
  --total-supply 9000000000000 \
  --delegated-supply 900000000000 \
...
# jq -M '. + {slotLength:0.1, securityParam:10, activeSlotsCoeff:0.1, securityParam:10, epochLength:600, maxLovelaceSupply:10000000000000, updateQuorum:2}' "${ROOT}/genesis/shelley/copy-genesis.json" > "${ROOT}/genesis/shelley/copy2-genesis.json"
# jq --raw-output '.protocolParams.protocolVersion.major = 8 | .protocolParams.minFeeA = 44 | .protocolParams.minFeeB = 155381 | .protocolParams.minUTxOValue = 1000000 | .protocolParams.decentralisationParam = 0.7 | .protocolParams.keyDeposit = 2000000 | .protocolParams.rho = 0.1 | .protocolParams.tau = 0.1' "${ROOT}/genesis/shelley/copy2-genesis.json" > "${ROOT}/genesis/shelley/genesis.json"
