...
sleep 2
tmux select-pane -t 0
tmux send-keys "${CMD} $(mklogcfg 0) $(mkdlgkey 0) $(mkdlgcert 0) ${NETARGS[*]} -n 0 --host-addr ${HOST} --port 3000 ${SPECIAL}" C-m
tmux select-pane -t 1
tmux send-keys "${CMD} $(mklogcfg 1) $(mkdlgkey 1) $(mkdlgcert 1) ${NETARGS[*]} -n 1 --host-addr ${HOST} --port 3001 ${SPECIAL}" C-m
tmux select-pane -t 2
tmux send-keys "${CMD} $(mklogcfg 2) $(mkdlgkey 2) $(mkdlgcert 2) ${NETARGS[*]} -n 2 --host-addr ${HOST} --port 3002 ${SPECIAL}" C-m
