...
# This script will initiate the transition to protocol version 7 (Babbage).
...
# 4 (which happens one or two epoch boundaries after invoking update-5.sh).
...
# Create the update proposal to change the protocol version to 7

cardano-cli governance create-update-proposal \
            --out-file update-proposal-babbage \
            --epoch ${EPOCH} \
...
cardano-cli transaction build-raw \
            ...
            --update-proposal-file update-proposal-babbage \
            --out-file tx4.txbody
...
sed -i configuration.yaml \
    -e 's/LastKnownBlockVersion-Major: 5/LastKnownBlockVersion-Major: 7/' \
