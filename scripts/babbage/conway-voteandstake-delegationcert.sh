...
for i in {1..3}; do
  $CARDANO_CLI conway stake-address stake-and-vote-delegation-certificate \
    ...
    --out-file "${TRANSACTIONS_DIR}/stakeandvote-deleg${i}.cert"
done
...
$CARDANO_CLI conway transaction build \
  ...
  --certificate-file "${TRANSACTIONS_DIR}/stakeandvote-deleg1.cert" \
  --certificate-file "${TRANSACTIONS_DIR}/stakeandvote-deleg2.cert" \
  --certificate-file "${TRANSACTIONS_DIR}/stakeandvote-deleg3.cert" \
  --witness-override 4 \
  --out-file "${TRANSACTIONS_DIR}/stakeandvote-deleg-tx.raw"

$CARDANO_CLI conway transaction sign \
  --testnet-magic $NETWORK_MAGIC \
  --tx-body-file "${TRANSACTIONS_DIR}/stakeandvote-deleg-tx.raw" \
  ...
  --out-file "${TRANSACTIONS_DIR}/stakeandvote-deleg-tx.signed"

$CARDANO_CLI conway transaction submit \
  --testnet-magic $NETWORK_MAGIC \
  --tx-file "${TRANSACTIONS_DIR}/stakeandvote-deleg-tx.signed"
