...
In this example we will use the [always succeeds](../../../plutus-example/plutus-example/src/Cardano/PlutusExample/AlwaysSucceeds.hs) Plutus spending script. In order to execute a Plutus spending script, we require the following:
...
- The plutus script serialized in the text envelope format. `cardano-cli` expects Plutus scripts to be serialised in the text envelope format.

#### Create the always succeeds Plutus Spending Script
...
In order to require a Plutus script to validate the spending of a tx ouput, we must put the tx output at the script address of said Plutus script. However before we do that, we must create a datum hash.
