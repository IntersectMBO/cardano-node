...
The deregistration certificate contains the _epoch_ in which we want to retire the pool. This epoch must be _after_ the current epoch and _not later than_ `poolRetireMaxEpoch` epochs in the future, where `poolRetireMaxEpoch` is a protocol parameter.
...
So one epoch lasts for 432000 slots. We get the current slot by querying the tip:
    ...
    {
        "hash": "cdd3d9075d22a52f0a8e5d92a6c76e6982a2a1036fd4ce8732f70d89bb778a67",
        "block": 5361172,
        "slot": 22141376
    }
...
We can look up `poolRetireMaxEpoch` by querying the current protocol parameters:
    ...
    cat protocol.json | grep poolRetireMaxEpoch
    > "poolRetireMaxEpoch": 18,

This means the earliest epoch for retirement is 52 (one in the future), and the latest is 69 (current epoch plus `poolRetireMaxEpoch`).
