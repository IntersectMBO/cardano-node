let
  ...
  makeScripts = cluster: let
    getScript = name: {
      x86_64-linux = (getArchDefault "x86_64-linux").scripts.${name}.${cluster};
      x86_64-darwin = (getArchDefault "x86_64-darwin").scripts.${name}.${cluster};
    };
  in {
    node = getScript "node";
  };
...
commonLib.nix-tools.release-nix {
  ...
  extraBuilds = (builtins.listToAttrs (map makeRelease [
    "mainnet"
    "staging"
