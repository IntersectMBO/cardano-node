...
pNodeEnvironment = fmap (maybe NoUserProvidedEnv UserProvidedEnv) <$> optional $ OA.strOption $
     OA.long "node-env"
  <> OA.metavar "FILEPATH"
...
pEnvOutputDir = OA.strOption
  (   OA.long "output-dir"
  <>  OA.help "Directory where to store the sandbox environment, sockets, and so on."
  <>  OA.metavar "DIRECTORY"
  )
