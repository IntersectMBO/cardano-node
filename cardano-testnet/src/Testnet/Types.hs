...
spoNodes = fmap testnetNodeRuntime . filter isTestnetNodeRoleSpo . testnetNodes

relayNodes :: TestnetRuntime -> [NodeRuntime]
relayNodes = fmap testnetNodeRuntime . filter (not . isTestnetNodeRoleSpo) . testnetNodes
...
poolNodeStdout = nodeStdout . testnetNodeRuntime
...
data TestnetNodeKeys = TestnetNodeKeys
  { poolNodeKeysCold :: KeyPair SpoColdKey
  , poolNodeKeysVrf :: KeyPair VrfKey
  , poolNodeKeysStaking :: KeyPair StakingKey
  } deriving (Eq, Show)

type instance Element TestnetNodeKeys = FilePath
instance MonoFunctor TestnetNodeKeys where
  omap f (TestnetNodeKeys cold vrf staking) = TestnetNodeKeys (omap f cold) (omap f vrf) (omap f staking)
