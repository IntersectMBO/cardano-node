...
babbageTestnet testnetOptions H.Conf {H.configurationTemplate, H.tempAbsPath} = do
  ...
  wallets <- forM [1..3] $ \idx -> do
    paymentSKey <- H.note $ tempAbsPath' </> "utxo-keys/utxo" <> show @Int idx <> ".skey"
    paymentVKey <- H.note $ tempAbsPath' </> "utxo-keys/utxo" <> show @Int idx <> ".vkey"

    paymentAddress <- H.noteShowM $ fmap Text.pack $ execCli
      [ "address", "build"
