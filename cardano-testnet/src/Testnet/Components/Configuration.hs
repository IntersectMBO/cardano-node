...
createSPOGenesisAndFiles
  ...
  defaultShelleyGenesis <- getDefaultShelleyGenesis cardanoNodeEra cardanoMaxSupply genesisOptions
  defaultAlonzoGenesis <- getDefaultAlonzoGenesis sbe
  ...
  UserProvidedGeneses
    ...
    , upgConwayGenesis = mConwayGenesis
      ...
      defaultShelleyGenesis defaultAlonzoGenesis Defaults.defaultConwayGenesis
      onChainParams
  ...
  generateGenesisIfAbsent ShelleyEra mShelleyGenesis defaultShelleyGenesis
  generateGenesisIfAbsent AlonzoEra mAlonzoGenesis defaultAlonzoGenesis
  generateGenesisIfAbsent ConwayEra mConwayGenesis Defaults.defaultConwayGenesis
...
resolveOnChainParams shelleyGenesis' alonzoGenesis' conwayGenesis' = \case

  NoCustomParams -> pure def

  CustomParamsGeneses gens -> pure gens

  CustomParamsFile file -> do
    eParams <- H.readJsonFile file
    params <- H.leftFail eParams
    pure $ blockfrostToGenesis params

  CustomParamsFlavour flavour -> do
    let req = flavourRequest flavour
    mainnetParams <- HTTP.getResponseBody <$> HTTP.httpJSON req
    pure $ blockfrostToGenesis mainnetParams
