...
createSPOGenesisAndFiles
  ...
  let defaultShelleyGenesis = defaultShelleyGenesis'
  ...
  UserProvidedGeneses
    ...
    , upgConwayGenesis = mConwayGenesis
      ...
      (defaultAlonzoGenesis, Defaults.defaultConwayGenesis, defaultShelleyGenesis)
      onChainParams
...
resolveOnChainParams geneses = \case
  ...
  OnChainParamsMainnet -> do
    mainnetParams <- HTTP.getResponseBody <$> HTTP.httpJSON mainnetParamsRequest
