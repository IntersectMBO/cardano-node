...
import           Testnet.Start.Types (GenesisOrigin (..), NumDReps (..), NumPools (..),
...
createSPOGenesisAndFiles
 ...
 -- Epoch length should be "10 * k / f" where "k = securityParam, f = activeSlotsCoeff"
  ...
  overwriteCreateTestnetDataGenesis inputGenesisShelleyFp ShelleyEra shelleyOrigin
  overwriteCreateTestnetDataGenesis inputGenesisAlonzoFp AlonzoEra alonzoOrigin
  overwriteCreateTestnetDataGenesis inputGenesisConwayFp ConwayEra conwayOrigin
  ...
  where
    ...
    overwriteCreateTestnetDataGenesis ::
      ...
      FilePath -> eon era -> GenesisOrigin -> m ()
    overwriteCreateTestnetDataGenesis genesisFp e = \case
      DefaultedOrigin -> pure ()
      UserProvidedOrigin -> H.copyFile genesisFp (tempAbsPath </> eraToString e <> "-genesis.json")
