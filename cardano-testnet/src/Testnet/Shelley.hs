...
shelleyTestnet testnetOptions H.Conf {..} = do
  ...
  -- configuration files.
  let sourceAlonzoGenesisSpecFile = base </> "cardano-cli/test/cardano-cli-test/files/golden/alonzo/genesis.alonzo.spec.json"
  alonzoSpecFile <- H.noteTempFile tempAbsPath "genesis.alonzo.spec.json"
  H.copyFile sourceAlonzoGenesisSpecFile alonzoSpecFile

  let sourceConwayGenesisSpecFile = base </> "cardano-cli/test/cardano-cli-test/files/golden/conway/genesis.conway.spec.json"
