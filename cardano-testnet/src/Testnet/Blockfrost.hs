...
instance FromJSON BlockfrostParams where
  ...
  parseJSON = withObject "BlockfrostParams" $ \o -> do
    ...
    bfgConwayCostModel <- case Map.lookup PlutusV3 models of
      Nothing -> parseFail "Found no cost model for PlutusV3"
      Just model -> pure model

    pure $ BlockfrostParams {..}

    where
      -- Sometimes BlockFrost encodes integers as strings. This parser tries to convert
      -- from String to Number if the default parsing fails
