...
hprop_ledger_events_treasury_withdrawal = H.integrationRetryWorkspace 0 {- FIXME set to 2 -} "treasury-withdrawal" $ \tempAbsBasePath' -> do
  ...
  H.nothingFailM $
    H.nothingFailM (mEpoch <$> queryTip execConfig) >>=
      getAnyWithdrawals (File configurationFile) (File socketPath) . appEndo (stimes @_ @Int 5 succ')
...
getAnyWithdrawals nodeConfigFile socketPath maxEpoch = withFrozenCallStack $ do
  fmap snd . H.leftFailM . runExceptT $ foldEpochState nodeConfigFile socketPath FullValidation maxEpoch Nothing
