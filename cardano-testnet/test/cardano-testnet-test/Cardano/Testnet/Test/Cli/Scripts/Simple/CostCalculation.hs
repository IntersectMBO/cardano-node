...
module Cardano.Testnet.Test.Cli.Scripts.Simple.CostCalculation
  ( hprop_ref_simple_script_cost_calculation
  ) where
...
-- @DISABLE_RETRIES=1 cabal test cardano-testnet-test --test-options '-p "/Simple Script.Cost Calculation Simple Reference Script/"'@
hprop_ref_simple_script_cost_calculation :: Property
hprop_ref_simple_script_cost_calculation = integrationRetryWorkspace 2 "ref-simple-script" $ \tempAbsBasePath' -> H.runWithDefaultWatchdog_ $ do
  H.note_ SYS.os
