  ...
  forallQueryCommands $ \case
    ...
    TestQueryTipCmd ->
      ...
      do
        ...
        let changes = Map.fromList (map (, "<redacted>") ["hash", "block", "epoch", "slot", "slotInEpoch", "slotsToEpochEnd"])
    ...
    TestQueryDRepStateCmd ->
      ...
      do
        ...
        let drepsRedacted = redactJsonFields (Map.fromList [("keyHash", "<redacted>")]) dreps
        H.diffVsGoldenFile
        ...
        redactJsonFieldsInFile (Map.fromList [("keyHash", "<redacted>")]) drepStateOutFile drepStateRedactedOutFile
        H.diffFileVsGoldenFile
