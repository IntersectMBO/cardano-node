...
hprop_collateral_with_tokens = integrationWorkspace "collateral-with-tokens" $ \tempAbsBasePath' -> H.runWithDefaultWatchdog_ $ do
  ...
  let mintTokensTxBody = work </> "mint-tokens-tx-body"
      mintTokensTx = work </> "mint-tokens-tx"
      collateralAddr = T.unpack $ paymentKeyInfoAddr wallet1
      mintValue = mconcat ["100 ", mintingPolicyId, ".", assetName]
      collateralValue = mconcat [collateralAddr, "+", show @Int 3_000_000, "+", mintValue]
      fundScriptValue = mconcat [plutusSpendingScriptAddr, "+", show @Int 5_000_000]
  ...
  txinCollateralWithTokensM <- 
    findLargestMultiAssetUtxoWithAddress epochStateView sbe $ T.pack collateralAddr
  ...
  -- This will fail with the "CollateralContainsNonADA" error
  void $ execCli' execConfig
    [ "latest", "transaction", "submit"
