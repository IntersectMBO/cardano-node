...
module Cardano.Testnet.Test.Cli.Conway.Plutus
  ( hprop_plutus_v3
  , hprop_create_transaction_with_two_script_certs_v2
  ) where
...
hprop_create_transaction_with_two_script_certs_v2 :: Property
hprop_create_transaction_with_two_script_certs_v2 = integrationWorkspace "create-transaction-two-script-certs" $ \tempAbsBasePath' -> H.runWithDefaultWatchdog_ $ do
  conf@Conf { tempAbsPath } <- mkConf tempAbsBasePath'
  let tempAbsPath' = unTmpAbsPath tempAbsPath
  work <- H.createDirectoryIfMissing $ tempAbsPath' </> "work"
