...
evalTxFile sbe' rpcServer txFile = withFrozenCallStack $ do
  textEnvelope <- H.leftFailM . H.evalIO $ readTextEnvelopeFromFile txFile
  ShelleyTx _ ledgerTx <- H.leftFail $ shelleyBasedEraConstraints sbe' $ deserialiseFromTextEnvelope textEnvelope
