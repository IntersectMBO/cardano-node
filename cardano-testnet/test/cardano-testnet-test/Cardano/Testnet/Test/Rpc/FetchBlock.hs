...
hprop_rpc_fetch_block = integrationRetryWorkspace 2 "rpc-fetch-block" $ \tempAbsBasePath' -> H.runWithDefaultWatchdog_ $ do
  ...
  do
    ...
    block <- expectSingleBlock response
  ...
  (txBlockSlot, txBlockHash, txBlockTxCount) <- do
    ...
    fs <- H.evalIO $ mkNodeConfigFs configurationFile
    mTxBlock <-
      H.timeout 60_000_000 . runExceptT $
        foldBlocks fs configurationFile (nodeSocketPath node0) QuickValidation Nothing $
  ...
  do
    ...
    txBlock <- expectSingleBlock txBlockResponse
  ...
  do
    ...
    mintBlock <- expectSingleBlock mintBlockResponse
...
-- | Expect exactly one block in a FetchBlock response.
expectSingleBlock
  :: HasCallStack
  => H.MonadTest m
  => Rpc.Proto U5c.FetchBlockResponse
  -> m (Rpc.Proto U5c.AnyChainBlock)
expectSingleBlock response =
  case response ^. U5c.block of
    [block] -> pure block
    blocks ->
      H.failMessage callStack $
        "Expected exactly one block in the FetchBlock response, got: " <> show (length blocks)
