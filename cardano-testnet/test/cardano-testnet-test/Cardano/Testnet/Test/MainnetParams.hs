...
-- | Execute me with:
-- @DISABLE_RETRIES=1 cabal test cardano-testnet-test --test-options '-p "/Can get on-chain parameters from mainnet/"'@
...
hprop_mainnet_params = integrationRetryWorkspace 2 "mainnet-params" $ \tmpDir -> H.runWithDefaultWatchdog_ $ do
  ...
  let testnetOptions = def { cardanoOutputDir = UserProvidedEnv tmpDir }
      ...
      createEnvOptions = def {ceoOnChainParams = OnChainParamsMainnet}
  ...
  H.evalIO $ IO.terminateProcess nodeProcessHandle
