...
golden_shelleyGovernanceCreatePoll =
  propertyOnce . H.moduleWorkspace "tmp" $ \tempDir -> do
    ...
    noteInputFile "cardano-cli-golden/files/golden/shelley/governance/create/basic.json"
      >>= H.readFile
      >>= (H.===) stdout
...
golden_shelleyGovernanceCreateLongPoll =
  propertyOnce . H.moduleWorkspace "tmp" $ \tempDir -> do
    ...
    void $ H.readFile pollFile
    noteInputFile "cardano-cli-golden/files/golden/shelley/governance/create/long-text.json"
      >>= H.readFile
      >>= (H.===) stdout
    H.assertFileOccurences 1 "GovernancePoll" pollFile
    H.assertEndsWithSingleNewline pollFile
