...
golden_shelleyVRFKeys = propertyOnce . H.moduleWorkspace "tmp" $ \tempDir -> do
  -- Reference keys
  referenceVerKey <- noteInputFile "cardano-cli-golden/files/golden/shelley/keys/vrf_keys/verification_key"
  referenceSignKey <- noteInputFile "cardano-cli-golden/files/golden/shelley/keys/vrf_keys/signing_key"
