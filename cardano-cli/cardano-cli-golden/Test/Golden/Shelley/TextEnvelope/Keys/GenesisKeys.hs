...
golden_shelleyGenesisKeys = propertyOnce . H.moduleWorkspace "tmp" $ \tempDir -> do
  -- Reference keys
  referenceVerKey <- noteInputFile "cardano-cli-golden/files/golden/shelley/keys/genesis_keys/verification_key"
  referenceSignKey <- noteInputFile "cardano-cli-golden/files/golden/shelley/keys/genesis_keys/signing_key"
