...
golden_shelleyPaymentKeys = propertyOnce . H.moduleWorkspace "tmp" $ \tempDir -> do
  -- Reference keys
  referenceVerKey <- noteInputFile "cardano-cli-golden/files/golden/shelley/keys/payment_keys/verification_key"
  referenceSignKey <- noteInputFile "cardano-cli-golden/files/golden/shelley/keys/payment_keys/signing_key"
