...
golden_shelleyTx = propertyOnce . H.moduleWorkspace "tmp" $ \tempDir -> do
  -- Reference keys
  let referenceTx = "cardano-cli-golden/files/golden/alonzo/tx"

  -- Key filepaths
  paymentSignKey <- noteInputFile "cardano-cli-golden/files/golden/shelley/transaction-sign/utxo.skey"
