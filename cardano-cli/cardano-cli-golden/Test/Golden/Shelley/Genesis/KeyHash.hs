...
golden_shelleyGenesisKeyHash = propertyOnce . H.moduleWorkspace "tmp" $ \tempDir -> do
  referenceVerificationKey <- noteInputFile "cardano-cli-golden/files/golden/shelley/keys/genesis_keys/verification_key"
  goldenGenesisVerificationKeyHashFile <- noteInputFile "cardano-cli-golden/files/golden/shelley/keys/genesis_keys/verification_key.key-hash"
