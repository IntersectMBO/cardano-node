...
golden_shelleyNodeIssueOpCert = propertyOnce . H.moduleWorkspace "tmp" $ \tempDir -> do
  hotKesVerificationKeyFile <- noteInputFile "cardano-cli-golden/files/golden/shelley/keys/kes_keys/verification_key"
  coldSigningKeyFile <- noteInputFile "cardano-cli-golden/files/golden/shelley/keys/genesis_delegate_keys/signing_key"
  originalOperationalCertificateIssueCounterFile <- noteInputFile "cardano-cli-golden/files/golden/shelley/keys/genesis_delegate_keys/operational_certificate_counter"
