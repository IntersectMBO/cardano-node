...
prop_deserialise_legacy_signing_Key = propertyOnce $ do
  legSkeyBs <- H.evalIO $ LB.readFile "cardano-cli-golden/files/golden/byron/keys/legacy.skey"
...
prop_deserialise_nonLegacy_signing_Key = propertyOnce $ do
  skeyBs <- H.evalIO $ LB.readFile "cardano-cli-golden/files/golden/byron/keys/byron.skey"
...
prop_print_legacy_signing_key_address = propertyOnce $ do
  let legKeyFp = "cardano-cli-golden/files/golden/byron/keys/legacy.skey"
...
prop_print_nonLegacy_signing_key_address = propertyOnce $ do
  let nonLegKeyFp = "cardano-cli-golden/files/golden/byron/keys/byron.skey"
...
prop_deserialise_NonLegacy_Signing_Key_API = propertyOnce $ do
  eFailOrWit <- H.evalIO . runExceptT $ readByronSigningKey NonLegacyByronKeyFormat "cardano-cli-golden/files/golden/byron/keys/byron.skey"
  case eFailOrWit of
...
prop_deserialiseLegacy_Signing_Key_API = propertyOnce $ do
  eFailOrWit <- H.evalIO . runExceptT $ readByronSigningKey LegacyByronKeyFormat "cardano-cli-golden/files/golden/byron/keys/legacy.skey"
  case eFailOrWit of
    Left keyFailure -> failWith Nothing $ show keyFailure
    Right _ -> success
