...
golden_byron_yes_vote = propertyOnce $ H.moduleWorkspace "tmp" $ \tempDir -> do
  goldenYesVote <- noteInputFile "cardano-cli-golden/files/golden/byron/votes/vote-yes"
  proposal <- noteInputFile "cardano-cli-golden/files/golden/byron/update-proposal"
  signingKey <- noteInputFile "cardano-cli-golden/files/golden/byron/keys/byron.skey"
...
golden_byron_no_vote = propertyOnce $ H.moduleWorkspace "tmp" $ \tempDir -> do
  goldenNoVote <- noteInputFile "cardano-cli-golden/files/golden/byron/votes/vote-no"
  proposal <- noteInputFile "cardano-cli-golden/files/golden/byron/update-proposal"
  signingKey <- noteInputFile "cardano-cli-golden/files/golden/byron/keys/byron.skey"
