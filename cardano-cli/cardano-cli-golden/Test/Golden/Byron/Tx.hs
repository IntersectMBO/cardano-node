...
golden_byronTx_legacy = propertyOnce $ H.moduleWorkspace "tmp" $ \tempDir -> do
  signingKey <- noteInputFile "cardano-cli-golden/files/golden/byron/keys/legacy.skey"
  goldenTx <- noteInputFile "cardano-cli-golden/files/golden/byron/tx/legacy.tx"
...
golden_byronTx = propertyOnce $ H.moduleWorkspace "tmp" $ \tempDir -> do
  ...
  goldenTx <- noteInputFile "cardano-cli-golden/files/golden/byron/tx/normal.tx"
  createdTx <- noteTempFile tempDir "tx"
  void $ execCardanoCLI
    [ "byron","transaction","issue-utxo-expenditure"
    , "--mainnet"
