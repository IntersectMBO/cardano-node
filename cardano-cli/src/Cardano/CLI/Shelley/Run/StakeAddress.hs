...
renderShelleyStakeAddressCmdError err = case err of
  ShelleyStakeAddressKeyPairError bech32PrivKey bech32PubKey ->
    "Error while deriving the shelley verification key from bech32 private Key: "
      <> bech32PrivKey
      <> " Corresponding bech32 public key: "
      <> bech32PubKey
...
runStakeAddressCmd (StakeAddressKeyHash vk mOutputFp) =
  runStakeAddressKeyHash vk mOutputFp
runStakeAddressCmd (StakeAddressBuild vk nw mOutputFp) =
  runStakeAddressBuild vk nw mOutputFp
...
runStakeAddressKeyHash (VerificationKeyFile vkeyPath) mOutputFp = do
  vkey <-
    firstExceptT ShelleyStakeAddressReadFileError
    . newExceptT
...
runStakeKeyDelegationCert
  ...
  -> OutputFile
  -> ExceptT ShelleyStakeAddressCmdError IO ()
runStakeKeyDelegationCert (VerificationKeyFile stkKey) poolVKeyHashOrFile (OutputFile outFp)
  = do
    stakeVkey <-
      firstExceptT ShelleyStakeAddressReadFileError
      . newExceptT
    ...
    poolStakeVKeyHash <- case poolVKeyHashOrFile of
      StakePoolVerificationKeyHash hash -> pure hash
      StakePoolVerificationKeyFile (VerificationKeyFile fp) -> bimapExceptT
    ...
    let stakeCred = StakeCredentialByKey (verificationKeyHash stakeVkey)
        delegCert =
          makeStakeAddressDelegationCertificate stakeCred poolStakeVKeyHash
    firstExceptT ShelleyStakeAddressWriteFileError
      . newExceptT
...
runStakeKeyDeRegistrationCert (VerificationKeyFile vkFp) (OutputFile oFp) = do
 ...
 where
  ...
  deregCertDesc =
    TextViewDescription "Stake Address Deregistration Certificate"
