...
renderShelleyKeyCmdError err = case err of
...
runKeyCmd cmd = case cmd of
  KeyGetVerificationKey skf vkf -> runGetVerificationKey skf vkf

  KeyNonExtendedKey evkf vkf -> runNonExtendedKey evkf vkf
...
runGetVerificationKey
  :: SigningKeyFile -> VerificationKeyFile -> ExceptT ShelleyKeyCmdError IO ()
runGetVerificationKey skf (VerificationKeyFile vkf) = do
  ssk <- firstExceptT ShelleyKeyCmdReadFileError $ readSigningKeyFile skf
  withSomeSigningKey ssk $ \sk ->
    let vk = getVerificationKey sk
    in  firstExceptT ShelleyKeyCmdWriteFileError
          . newExceptT
...
withSomeSigningKey
  ...
  -> a
withSomeSigningKey ssk f = case ssk of
...
readSigningKeyFile
  :: SigningKeyFile -> ExceptT (FileError TextEnvelopeError) IO SomeSigningKey
...
runConvertByronKey (ByronPaymentKey NonLegacyByronKeyFormat) (AVerificationKeyFile vkeyPathOld)
...
runConvertByronKey (ByronGenesisKey NonLegacyByronKeyFormat) (AVerificationKeyFile vkeyPathOld)
  = convertByronVerificationKey convert vkeyPathOld
 where
...
runConvertByronKey (ByronDelegateKey NonLegacyByronKeyFormat) (AVerificationKeyFile vkeyPathOld)
  = convertByronVerificationKey convert vkeyPathOld
 where
...
runConvertByronKey (ByronPaymentKey LegacyByronKeyFormat) AVerificationKeyFile{}
  = const legacyVerificationKeysNotSupported

runConvertByronKey (ByronGenesisKey LegacyByronKeyFormat) AVerificationKeyFile{}
  = const legacyVerificationKeysNotSupported

runConvertByronKey (ByronDelegateKey LegacyByronKeyFormat) AVerificationKeyFile{}
  = const legacyVerificationKeysNotSupported

legacyVerificationKeysNotSupported :: ExceptT e IO a
legacyVerificationKeysNotSupported = liftIO $ do
  putStrLn
...
convertByronSigningKey
  ...
  -> OutputFile          -- ^ Output file: new format
  -> ExceptT ShelleyKeyCmdError IO ()
convertByronSigningKey byronFormat convert skeyPathOld (OutputFile skeyPathNew)
  = do
...
convertByronVerificationKey
  ...
  -> OutputFile          -- ^ Output file: new format
  -> ExceptT ShelleyKeyCmdError IO ()
convertByronVerificationKey convert (VerificationKeyFile vkeyPathOld) (OutputFile vkeyPathNew)
  = do
...
runConvertByronGenesisVerificationKey
  ...
  -> OutputFile             -- ^ Output file: new format
  -> ExceptT ShelleyKeyCmdError IO ()
runConvertByronGenesisVerificationKey (VerificationKeyBase64 b64ByronVKey) (OutputFile vkeyPathNew)
  = do

    vk <-
      firstExceptT (ShelleyKeyCmdByronKeyParseError . show)
      . hoistEither
...
runConvertITNStakeKey (AVerificationKeyFile (VerificationKeyFile vk)) (OutputFile outFile)
  = do
    bech32publicKey <-
      firstExceptT ShelleyKeyCmdItnKeyConvError . newExceptT $ readFileITNKey vk
    vkey <-
      hoistEither
...
runConvertITNStakeKey (ASigningKeyFile (SigningKeyFile sk)) (OutputFile outFile)
  = do
    bech32privateKey <-
      firstExceptT ShelleyKeyCmdItnKeyConvError . newExceptT $ readFileITNKey sk
    skey <-
      hoistEither . first ShelleyKeyCmdItnKeyConvError $ convertITNSigningKey
        bech32privateKey
    firstExceptT ShelleyKeyCmdWriteFileError
      . newExceptT
...
renderConversionError err = case err of
  ...
  Bech32ReadError fp readErr ->
    "Error reading bech32 key at: " <> textShow fp <> " Error: " <> Text.pack
      (displayException readErr)
  ITNError hRpart dp ->
    "Error extracting a ByteString from DataPart: "
      <> Bech32.dataPartToText dp
      <> " With human readable part: "
      <> Bech32.humanReadablePartToText hRpart
...
convertITNVerificationKey
  :: Text -> Either ConversionError (VerificationKey StakeKey)
...
decodeBech32Key key = case Bech32.decodeLenient key of
