...
import           Cardano.Api.Typed              ( Error(..)
                                                , FileError
                                                , TextEnvelopeError
...
renderShelleyTextViewFileError err = case err of
...
runTextViewCmd cmd = case cmd of
  TextViewInfo fpath mOutfile -> runTextViewInfo fpath mOutfile

runTextViewInfo
  :: FilePath -> Maybe OutputFile -> ExceptT ShelleyTextViewFileError IO ()
runTextViewInfo fpath mOutFile = do
  tv <- firstExceptT ShelleyTextViewReadFileError
    $ newExceptT (readTextEnvelopeFromFile fpath)
  let lbCBOR = LBS.fromStrict (tvRawCBOR tv)
  case mOutFile of
    Just (OutputFile oFpath) -> liftIO $ LBS.writeFile oFpath lbCBOR
    Nothing ->
      firstExceptT ShelleyTextViewCBORPrettyPrintError $ pPrintCBOR lbCBOR
