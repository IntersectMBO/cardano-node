...
renderShelleyGenesisCmdError err = case err of
  ...
  ShelleyGenesisCmdFilesDupIndex files ->
    "The genesis keys files are expected to have a unique numeric index but these do not:\n"
      <> Text.unlines (map Text.pack files)
  ShelleyGenesisCmdReadFileError fileErr -> Text.pack (displayError fileErr)
  ShelleyGenesisCmdWriteFileError fileErr -> Text.pack (displayError fileErr)
...
runGenesisCmd (GenesisKeyGenDelegate vk sk ctr) =
...
runGenesisCmd (GenesisCreate gd gn un ms am nw) =
  runGenesisCreate gd gn un ms am nw
...
runGenesisKeyGenDelegate
  ...
  -> OpCertCounterFile
  -> ExceptT ShelleyGenesisCmdError IO ()
runGenesisKeyGenDelegate (VerificationKeyFile vkeyPath) (SigningKeyFile skeyPath) (OpCertCounterFile ocertCtrPath)
  = do
 ...
 where
  ...
  certCtrDesc =
    TextViewDescription $ "Next certificate issue number: " <> BS.pack
      (show initialCounter)
...
runGenesisKeyGenDelegateVRF
  ...
  -> SigningKeyFile
  -> ExceptT ShelleyGenesisCmdError IO ()
runGenesisKeyGenDelegateVRF (VerificationKeyFile vkeyPath) (SigningKeyFile skeyPath)
  = do
...
runGenesisKeyGenUTxO
  ...
  -> SigningKeyFile
  -> ExceptT ShelleyGenesisCmdError IO ()
runGenesisKeyGenUTxO (VerificationKeyFile vkeyPath) (SigningKeyFile skeyPath) =
  do
...
runGenesisKeyHash (VerificationKeyFile vkeyPath) = do
  vkey <-
    ...
    $ readFileTextEnvelopeAnyOf
        [ FromSomeType (AsVerificationKey AsGenesisKey) AGenesisKey
        , FromSomeType (AsVerificationKey AsGenesisDelegateKey)
                       AGenesisDelegateKey
        , FromSomeType (AsVerificationKey AsGenesisUTxOKey) AGenesisUTxOKey
        ]
        vkeyPath
 ...
 where
  ...
  renderVerificationKeyHash
    :: Key keyrole => VerificationKey keyrole -> ByteString
  renderVerificationKeyHash = serialiseToRawBytesHex . verificationKeyHash
...
runGenesisVerKey (VerificationKeyFile vkeyPath) (SigningKeyFile skeyPath) = do
  skey <-
    ...
    . newExceptT
    $ readFileTextEnvelopeAnyOf
        [ FromSomeType (AsSigningKey AsGenesisKey) AGenesisKey
        , FromSomeType (AsSigningKey AsGenesisDelegateKey) AGenesisDelegateKey
        , FromSomeType (AsSigningKey AsGenesisUTxOKey) AGenesisUTxOKey
        ]
        skeyPath
...
runGenesisAddr (VerificationKeyFile vkeyPath) network mOutFile = do
  ...
  let vkh = verificationKeyHash (castVerificationKey vkey)
      addr =
        makeShelleyAddress network (PaymentCredentialByKey vkh) NoStakeAddress
...
runGenesisCreate
  ...
  -> NetworkId
  -> ExceptT ShelleyGenesisCmdError IO ()
runGenesisCreate (GenesisDir rootdir) genNumGenesisKeys genNumUTxOKeys mStart mAmount network
  = do
    liftIO $ do
    ...
    forM_ [1 .. genNumUTxOKeys] $ \index -> createUtxoKeys utxodir index
...
getCurrentTimePlus30 = plus30sec <$> liftIO getCurrentTime
 where
...
readShelleyGenesis fpath = do
  readAndDecode `catchError` \err -> case err of
    ShelleyGenesisCmdReadGenesisIOError _ ioe | isDoesNotExistError ioe ->
      writeDefault
    _ -> left err
 where
  readAndDecode = do
    lbs <- handleIOExceptT (ShelleyGenesisCmdReadGenesisIOError fpath)
      $ LBS.readFile fpath
    firstExceptT
        (ShelleyGenesisCmdReadGenesisAesonDecodeError fpath . Text.pack)
      . hoistEither
...
updateTemplate (SystemStart start) mAmount delKeys utxoAddrs template =
 ...
 where
  shelleyDelKeys = Map.fromList
    ...
    | (GenesisKeyHash gh, (GenesisDelegateKeyHash gdh, VrfKeyHash h)) <-
      Map.toList delKeys
    ]
...
writeShelleyGenesis fpath sg =
  handleIOExceptT (ShelleyGenesisCmdWriteGenesisIOError fpath)
    $ LBS.writeFile fpath (encodePretty sg)

toShelleyAddr :: Address Shelley -> Ledger.Addr TPraosStandardCrypto
toShelleyAddr (ByronAddress addr) =
...
readGenDelegsMap gendir deldir = do
  ...
  let combinedMap
      ...
      combinedMap =
        Map.intersectionWith (,) gkm (Map.intersectionWith (,) dkm vkm)
  ...
  unless (Map.null gkmExtra && Map.null dkmExtra && Map.null vkmExtra)
    $ throwError
    $ ShelleyGenesisCmdMismatchedGenesisKeyFiles (Map.keys gkm)
                                                 (Map.keys dkm)
                                                 (Map.keys vkm)

  let delegsMap :: Map (Hash GenesisKey) (Hash GenesisDelegateKey, Hash VrfKey)
      delegsMap = Map.fromList
        [ (gh, (dh, vh))
...
readGenesisKeys
  :: FilePath
  -> ExceptT ShelleyGenesisCmdError IO (Map Int (VerificationKey GenesisKey))
readGenesisKeys gendir = do
  files <- liftIO (listDirectory gendir)
  fileIxs <- extractFileNameIndexes
    [ gendir </> file | file <- files, takeExtension file == ".vkey" ]
  firstExceptT ShelleyGenesisCmdReadFileError $ Map.fromList <$> sequence
    [ (,) ix <$> readKeyFile file | (file, ix) <- fileIxs ]
 where
  readKeyFile =
    newExceptT . readFileTextEnvelope (AsVerificationKey AsGenesisKey)
...
readDelegateKeys deldir = do
  ...
  fileIxs <- extractFileNameIndexes
    [ deldir </> file | file <- files, takeExtensions file == ".vkey" ]
  firstExceptT ShelleyGenesisCmdReadFileError $ Map.fromList <$> sequence
    [ (,) ix <$> readKeyFile file | (file, ix) <- fileIxs ]
 where
  readKeyFile =
    newExceptT . readFileTextEnvelope (AsVerificationKey AsGenesisDelegateKey)

readDelegateVrfKeys
  :: FilePath
  -> ExceptT ShelleyGenesisCmdError IO (Map Int (VerificationKey VrfKey))
readDelegateVrfKeys deldir = do
  files <- liftIO (listDirectory deldir)
  fileIxs <- extractFileNameIndexes
    [ deldir </> file | file <- files, takeExtensions file == ".vrf.vkey" ]
  firstExceptT ShelleyGenesisCmdReadFileError $ Map.fromList <$> sequence
    [ (,) ix <$> readKeyFile file | (file, ix) <- fileIxs ]
 where
  readKeyFile = newExceptT . readFileTextEnvelope (AsVerificationKey AsVrfKey)
...
extractFileNameIndex fp = case filter isDigit fp of
  [] -> Nothing
  xs -> readMaybe xs

extractFileNameIndexes
  :: [FilePath] -> ExceptT ShelleyGenesisCmdError IO [(FilePath, Int)]
...
readInitialFundAddresses utxodir nw = do
  ...
  vkeys <- firstExceptT ShelleyGenesisCmdReadFileError $ sequence
    [ newExceptT $ readFileTextEnvelope (AsVerificationKey AsGenesisUTxOKey)
                                        (utxodir </> file)
    | file <- files
  ...
  return
    ...
    , let vkh = verificationKeyHash (castVerificationKey vkey)
          addr =
            makeShelleyAddress nw (PaymentCredentialByKey vkh) NoStakeAddress
    ]
