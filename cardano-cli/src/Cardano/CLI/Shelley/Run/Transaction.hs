    ...
    mUpdateProp <-
      case mUpdatePropFile of
        ...
        Just (UpdateProposalFile file) ->
          fmap Just <$> firstExceptT ShelleyTxReadUpdateError $
            newExceptT $
...
runTxSign (TxBodyFile txbodyFile) skFiles mnw (TxFile txFile) = do
  txbody <-
    firstExceptT ShelleyTxReadUnsignedTxError . newExceptT $
...
runTxSubmit protocol network txFile = do
  ...
  withlocalNodeConnectInfo protocol network sockPath $ \connectInfo ->
    case (localNodeConsensusMode connectInfo, tx) of
      ...
      (ByronMode {}, Right {}) ->
        left $
          ShelleyTxSubmitErrorEraMismatch
            EraMismatch
              { ledgerEraName = "Byron",
...
readSigningKeyFile (SigningKeyFile skfile) =
  ...
  where
    fileTypes =
      [ Api.FromSomeType
          (Api.AsSigningKey Api.AsByronKey)
          AByronSigningKey,
        Api.FromSomeType
          (Api.AsSigningKey Api.AsPaymentKey)
          APaymentSigningKey,
        Api.FromSomeType
          (Api.AsSigningKey Api.AsPaymentExtendedKey)
          APaymentExtendedSigningKey,
        Api.FromSomeType
          (Api.AsSigningKey Api.AsStakeKey)
          AStakeSigningKey,
        Api.FromSomeType
          (Api.AsSigningKey Api.AsStakeExtendedKey)
          AStakeExtendedSigningKey,
        Api.FromSomeType
          (Api.AsSigningKey Api.AsStakePoolKey)
          AStakePoolSigningKey,
        Api.FromSomeType
          (Api.AsSigningKey Api.AsGenesisKey)
          AGenesisSigningKey,
        Api.FromSomeType
          (Api.AsSigningKey Api.AsGenesisExtendedKey)
          AGenesisExtendedSigningKey,
        Api.FromSomeType
          (Api.AsSigningKey Api.AsGenesisDelegateKey)
          AGenesisDelegateSigningKey,
        Api.FromSomeType
          (Api.AsSigningKey Api.AsGenesisDelegateExtendedKey)
          AGenesisDelegateExtendedSigningKey,
        Api.FromSomeType
...
readFileTxMetaData (MetaDataFileJSON fp) = do
  bs <-
    handleIOExceptT (ShelleyTxMetaDataFileError fp) $
      LBS.readFile fp
  v <-
...
jsonToMetadataValue (Aeson.Object kvs) =
  Api.TxMetaMap
    <$> mapM
      ( \(k, v) ->
          (,) <$> jsonToMetadataValue (Aeson.String k)
