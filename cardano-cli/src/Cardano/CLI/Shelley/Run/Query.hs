...
import           Cardano.CLI.Helpers            ( HelpersError
                                                , pPrintCBOR
                                                , renderHelpersError
                                                )
...
import           Shelley.Spec.Ledger.Delegation.Certificates
                                                ( PoolDistr(..) )
import qualified Shelley.Spec.Ledger.Delegation.Certificates
                                               as Ledger
                                                ( PoolDistr(..) )
...
writeProtocolParameters
  :: Maybe OutputFile -> PParams -> ExceptT ShelleyQueryCmdError IO ()
writeProtocolParameters mOutFile pparams = case mOutFile of
...
runQueryUTxO protocol qfilter network mOutFile = do
  ...
  filteredUtxo <-
    firstExceptT ShelleyQueryNodeLocalStateQueryError $ withlocalNodeConnectInfo
      ...
      network
      sockPath
      (queryUTxOFromLocalState qfilter)
...
renderLocalStateQueryError lsqErr = case lsqErr of
...
writeLedgerState
  :: Maybe OutputFile
  -> EpochState TPraosStandardCrypto
  -> ExceptT ShelleyQueryCmdError IO ()
writeLedgerState mOutFile lstate = case mOutFile of
...
writeFilteredUTxOs
  :: Maybe OutputFile
  -> Ledger.UTxO TPraosStandardCrypto
  -> ExceptT ShelleyQueryCmdError IO ()
writeFilteredUTxOs mOutFile utxo = case mOutFile of
  Nothing -> liftIO $ printFilteredUTxOs utxo
  Just (OutputFile fpath) ->
    handleIOExceptT (ShelleyQueryWriteFilteredUTxOsError fpath)
...
printFilteredUTxOs (Ledger.UTxO utxo) = do
 ...
 where
  ...
  printUtxo
    :: (Ledger.TxIn TPraosStandardCrypto, Ledger.TxOut TPraosStandardCrypto)
    -> IO ()
...
printStakeDistribution (PoolDistr stakeDist) = do
  ...
  putStrLn $ replicate (Text.length title + 2) '-'
  sequence_
    [ putStrLn $ showStakeDistr (StakePoolKeyHash poolId)
                                stakeFraction
                                (VrfKeyHash vrfKeyId)
 ...
 where
  ...
  showStakeDistr :: PoolId -> Rational -> Hash VrfKey -> String
  showStakeDistr poolId stakeFraction _vrfKeyId = concat
...
queryUTxOFromLocalState qFilter connectInfo@LocalNodeConnectInfo { localNodeConsensusMode }
  = case localNodeConsensusMode of
    ...
    ShelleyMode{} -> do
      ...
      DegenQueryResult result <-
        firstExceptT AcquireFailureError . newExceptT $ queryNodeLocalState
          connectInfo
 ...
 where
  ...
  toShelleyAddrs
    :: Set (Address Shelley) -> Set (Ledger.Addr TPraosStandardCrypto)
...
instance ToJSON DelegationsAndRewards where
  toJSON (DelegationsAndRewards nw delegsAndRwds) =
    Aeson.Object $ Map.foldlWithKey' delegAndRwdToJson HMS.empty delegsAndRwds
   where
    delegAndRwdToJson
      ...
      -> (Maybe (Hash StakePoolKey), Coin)
      -> HashMap Text Aeson.Value
    delegAndRwdToJson acc k (d, r) = HMS.insert
      (toKey k)
...
--
queryPParamsFromLocalState
  :: LocalNodeConnectInfo mode block -> ExceptT LocalStateQueryError IO PParams
queryPParamsFromLocalState LocalNodeConnectInfo { localNodeConsensusMode = ByronMode{} }
  = throwError ByronProtocolNotSupportedError

queryPParamsFromLocalState connectInfo@LocalNodeConnectInfo { localNodeConsensusMode = ShelleyMode }
  = do
    tip <- liftIO $ getLocalTip connectInfo
    DegenQueryResult result <-
      firstExceptT AcquireFailureError . newExceptT $ queryNodeLocalState
...
queryPParamsFromLocalState connectInfo@LocalNodeConnectInfo { localNodeConsensusMode = CardanoMode{} }
  = do
    tip <- liftIO $ getLocalTip connectInfo
    result <-
      firstExceptT AcquireFailureError . newExceptT $ queryNodeLocalState
...
queryStakeDistributionFromLocalState LocalNodeConnectInfo { localNodeConsensusMode = ByronMode{} }
  = throwError ByronProtocolNotSupportedError

queryStakeDistributionFromLocalState connectInfo@LocalNodeConnectInfo { localNodeConsensusMode = ShelleyMode{} }
  = do
    tip <- liftIO $ getLocalTip connectInfo
    DegenQueryResult result <-
      firstExceptT AcquireFailureError . newExceptT $ queryNodeLocalState
...
queryStakeDistributionFromLocalState connectInfo@LocalNodeConnectInfo { localNodeConsensusMode = CardanoMode{} }
  = do
    tip <- liftIO $ getLocalTip connectInfo
    result <-
      firstExceptT AcquireFailureError . newExceptT $ queryNodeLocalState
...
queryLocalLedgerState connectInfo@LocalNodeConnectInfo { localNodeConsensusMode }
    ...
    ShelleyMode{} -> do
      ...
      DegenQueryResult result <-
        firstExceptT AcquireFailureError . newExceptT $ queryNodeLocalState
          connectInfo
          ( getTipPoint tip
          , DegenQuery $ GetCBOR GetCurrentEpochState  -- Get CBOR-in-CBOR version
          )
    ...
    CardanoMode{} -> do
      ...
      result <-
        firstExceptT AcquireFailureError . newExceptT $ queryNodeLocalState
          connectInfo
          ( getTipPoint tip
          , QueryIfCurrentShelley (GetCBOR GetCurrentEpochState)
  ...
  where
    ...
    -- CBOR decode.
        decodeLedgerState (Serialised lbs) = first (const lbs) (decodeFull lbs)
...
queryDelegationsAndRewardsFromLocalState stakeaddrs connectInfo@LocalNodeConnectInfo { localNodeNetworkId, localNodeConsensusMode }
  = case localNodeConsensusMode of
    ByronMode{} -> throwError ByronProtocolNotSupportedError
    ...
    ShelleyMode{} -> do
      ...
      DegenQueryResult result <-
        firstExceptT AcquireFailureError . newExceptT $ queryNodeLocalState
          connectInfo
          ( getTipPoint tip
          , DegenQuery $ GetFilteredDelegationsAndRewardAccounts
    ...
    CardanoMode{} -> do
      ...
      result <-
        firstExceptT AcquireFailureError . newExceptT $ queryNodeLocalState
          connectInfo
          ( getTipPoint tip
          , QueryIfCurrentShelley $ GetFilteredDelegationsAndRewardAccounts
            (toShelleyStakeCredentials stakeaddrs)
          )
      case result of
        QueryResultEraMismatch err -> throwError (EraMismatchError err)
 ...
 where
  ...
  toDelegsAndRwds delegs rwdAcnts =
    DelegationsAndRewards localNodeNetworkId $ Map.mapWithKey
      (\k v -> (StakePoolKeyHash <$> Map.lookup k delegs, v))
      rwdAcnts

  toShelleyStakeCredentials
    :: Set StakeAddress -> Set (Ledger.StakeCredential TPraosStandardCrypto)
