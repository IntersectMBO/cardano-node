...
data ShelleyQueryCmdError
  ...
  | ShelleyQueryCmdLocalStateFailure !LocalStateQueryFailure
...
renderShelleyQueryCmdError err =
  case err of
    ShelleyQueryCmdLocalStateFailure localStateFailure -> Text.pack $ displayError localStateFailure
...
runQueryProtocolParameters (AnyConsensusModeParams cModeParams) network mOutFile = do
  ...
  result <- firstExceptT ShelleyQueryCmdLocalStateFailure . newExceptT
    ...
    CardanoMode -> do
      ...
      eLocalState
        <- liftIO $ executeLocalStateQueryExpr localNodeConnInfo Nothing
                  $ do era <- queryExpr (QueryCurrentEra CardanoModeIsMultiEra)
