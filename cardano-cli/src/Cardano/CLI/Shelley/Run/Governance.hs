...
renderShelleyGovernanceError err = case err of
...
runGovernanceCmd cmd =
  liftIO $ putStrLn $ "TODO: runGovernanceCmd: " ++ show cmd
...
runGovernanceMIRCertificate mirPot vKeys rwdAmts (OutputFile oFp) = do
  ...
  firstExceptT GovernanceWriteFileError . newExceptT $ writeFileTextEnvelope
    oFp
    (Just mirCertDesc)
    mirCert
 where
  ...
  checkEqualKeyRewards keys rwds = do
    ...
    if numVKeys == numRwdAmts
      then return ()
        ...
        $ GovernanceMIRCertificateKeyRewardMistmach oFp numVKeys numRwdAmts

  readStakeKeyToCred
    :: VerificationKeyFile -> ExceptT ShelleyGovernanceError IO StakeCredential
  readStakeKeyToCred (VerificationKeyFile stVKey) = do
    stakeVkey <-
      firstExceptT GovernanceReadFileError . newExceptT $ readFileTextEnvelope
...
runGovernanceUpdateProposal
  ...
  -> ExceptT ShelleyGovernanceError IO ()
runGovernanceUpdateProposal (OutputFile upFile) eNo genVerKeyFiles upPprams =
  do
