...
renderShelleyNodeCmdError err = case err of
...
runNodeKeyGenCold (VerificationKeyFile vkeyPath) (SigningKeyFile skeyPath) (OpCertCounterFile ocertCtrPath)
  = do
    skey <- liftIO $ generateSigningKey AsStakePoolKey
    let vkey = getVerificationKey skey
    firstExceptT ShelleyNodeWriteFileError . newExceptT $ writeFileTextEnvelope
      skeyPath
      (Just skeyDesc)
      skey
    firstExceptT ShelleyNodeWriteFileError . newExceptT $ writeFileTextEnvelope
...
runNodeKeyGenKES
  :: VerificationKeyFile -> SigningKeyFile -> ExceptT ShelleyNodeCmdError IO ()
...
runNodeKeyHashVRF (VerificationKeyFile vkeyPath) mOutputFp = do
  vkey <-
    firstExceptT ShelleyNodeReadFileError . newExceptT $ readFileTextEnvelope
...
runNodeNewCounter
  ...
  -> OpCertCounterFile
  -> ExceptT ShelleyNodeCmdError IO ()
runNodeNewCounter (VerificationKeyFile vkeyPath) counter (OpCertCounterFile ocertCtrPath)
  = do

    vkey <-
      firstExceptT ShelleyNodeReadFileError
      . newExceptT
...
runNodeIssueOpCert
  ...
  -> OutputFile
  -> ExceptT ShelleyNodeCmdError IO ()
runNodeIssueOpCert (VerificationKeyFile vkeyKesPath) (SigningKeyFile skeyStakePoolPath) (OpCertCounterFile ocertCtrPath) kesPeriod (OutputFile certFile)
  = do
    ...
    (ocert, nextOcertCtr) <-
      ...
      . hoistEither
      $ issueOperationalCertificate verKeyKes
                                    signKey
                                    kesPeriod
                                    ocertIssueCounter
 ...
 where
  ...
  ocertCtrDesc n =
    TextViewDescription $ "Next certificate issue number: " <> BS.pack (show n)
