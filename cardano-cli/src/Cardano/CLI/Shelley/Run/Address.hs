...
renderShelleyAddressCmdError err = case err of
...
runAddressCmd cmd = case cmd of
  ...
  AddressInfo txt mOFp ->
    firstExceptT ShelleyAddressCmdAddressInfoError $ runAddressInfo txt mOFp
...
runAddressBuild payVkeyFp mstkVkeyFp nw mOutFp = do
  ...
  addr <- case payVKey of
    AByronVerificationKey vk -> return (makeByronAddress nw vk)

    APaymentVerificationKey vk -> buildShelleyAddress vk
...
readAddressVerificationKeyFile (VerificationKeyFile vkfile) =
  newExceptT $ readFileTextEnvelopeAnyOf fileTypes vkfile
 where
  fileTypes =
