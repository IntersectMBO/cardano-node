...
parseShelleyCommands = Opt.subparser $ mconcat
  [ Opt.command
    "address"
    (Opt.info (AddressCmd <$> pAddressCmd)
    $ Opt.progDesc "Shelley payment address commands"
    )
  , Opt.command
    "stake-address"
    (Opt.info (StakeAddressCmd <$> pStakeAddress)
    $ Opt.progDesc "Shelley stake address commands"
    )
  , Opt.command
    "key"
  ...
  , Opt.command
    "transaction"
    (Opt.info (TransactionCmd <$> pTransaction)
    $ Opt.progDesc "Shelley transaction commands"
    )
  , Opt.command
    "node"
    (Opt.info (NodeCmd <$> pNodeCmd)
    $ Opt.progDesc "Shelley node operaton commands"
    )
  , Opt.command
    "stake-pool"
    (Opt.info (PoolCmd <$> pPoolCmd)
    $ Opt.progDesc "Shelley stake pool commands"
    )
  , Opt.command
    "query"
    (Opt.info (QueryCmd <$> pQueryCmd) . Opt.progDesc $ mconcat
  ...
  , Opt.command
    "system"
    (Opt.info (SystemCmd <$> pSystemCmd)
    $ Opt.progDesc "Shelley system commands"
    )
  , Opt.command
    "genesis"
    (Opt.info (GenesisCmd <$> pGenesisCmd)
    $ Opt.progDesc "Shelley genesis block commands"
    )
  , Opt.command
    "governance"
    (Opt.info (GovernanceCmd <$> pGovernanceCmd)
    $ Opt.progDesc "Shelley governance commands"
    )
  , Opt.command
    "text-view"
...
pTextViewCmd = Opt.subparser $ mconcat
  [ Opt.command
      "decode-cbor"
...
pCBORInFile =
  Opt.strOption
    ...
    <|> Opt.strOption (Opt.long "file" <> Opt.internal)

pAddressCmd :: Parser AddressCmd
pAddressCmd = Opt.subparser $ mconcat
  [ Opt.command
    "key-gen"
  ...
  , Opt.command
    "build-multisig"
    (Opt.info pAddressBuildMultiSig
    $ Opt.progDesc "Build a Shelley payment multi-sig address."
    )
  , Opt.command
    "info"
 ...
 where
  ...
  pAddressKeyHash =
    AddressKeyHash <$> pPaymentVerificationKeyFile <*> pMaybeOutputFile
...
pPaymentVerificationKeyFile =
  VerificationKeyFile
    <$> (Opt.strOption
        ...
        <|> Opt.strOption (Opt.long "verification-key-file" <> Opt.internal)
        )


pStakeAddress :: Parser StakeAddressCmd
pStakeAddress = Opt.subparser $ mconcat
  [ Opt.command
    "key-gen"
  ...
  , Opt.command
    "delegate"
    (Opt.info pStakeAddressDelegate
    $ Opt.progDesc "Delegate from a stake address to a stake pool"
    )
  , Opt.command
    "de-register"
    (Opt.info pStakeAddressDeRegister
    $ Opt.progDesc "De-register a stake address"
    )
  , Opt.command
  ...
  , Opt.command
    ...
    (Opt.info pStakeAddressDelegationCert
    $ Opt.progDesc "Create a stake address delegation certificate"
    )
  ]
 where
  ...
  pStakeAddressKeyHash =
    StakeAddressKeyHash <$> pStakeVerificationKeyFile <*> pMaybeOutputFile
  ...
  pStakeAddressRegister =
    StakeKeyRegister <$> pPrivKeyFile <*> parseNodeAddress
  ...
  pStakeAddressDeRegister =
    StakeKeyDeRegister <$> pPrivKeyFile <*> parseNodeAddress

  pStakeAddressRegistrationCert :: Parser StakeAddressCmd
  pStakeAddressRegistrationCert =
    StakeKeyRegistrationCert <$> pStakeVerificationKeyFile <*> pOutputFile

  pStakeAddressDeregistrationCert :: Parser StakeAddressCmd
  pStakeAddressDeregistrationCert =
    StakeKeyDeRegistrationCert <$> pStakeVerificationKeyFile <*> pOutputFile
...
pDelegationFee = Lovelace <$> Opt.option
  Opt.auto
  (Opt.long "delegation-fee" <> Opt.metavar "LOVELACE" <> Opt.help
    "The delegation fee in Lovelace."
  )

pKeyCmd :: Parser KeyCmd
pKeyCmd = Opt.subparser $ mconcat
 ...
 where
  ...
  pKeyConvertByronKey =
    KeyConvertByronKey <$> pByronKeyType <*> pByronKeyFile <*> pOutputFile
  ...
  pByronKeyType =
    Opt.flag'
      ...
      <|> Opt.flag'
            ...
            (Opt.long "legacy-byron-genesis-delegate-key-type" <> Opt.help
  ...
  pByronSigningKeyFile = SigningKeyFile <$> Opt.strOption
  ...
  pByronVerificationKeyFile = VerificationKeyFile <$> Opt.strOption
  ...
  pKeyConvertByronGenesisVKey =
    KeyConvertByronGenesisVKey <$> pByronGenesisVKeyBase64 <*> pOutputFile

  pByronGenesisVKeyBase64 :: Parser VerificationKeyBase64
  pByronGenesisVKeyBase64 = VerificationKeyBase64 <$> Opt.strOption
  ...
  pKeyConvertITNKey = KeyConvertITNStakeKey <$> pITNKeyFIle <*> pOutputFile
  ...
  pITNSigningKeyFile = ASigningKeyFile . SigningKeyFile <$> Opt.strOption
  ...
  pITNVerificationKeyFile =
    AVerificationKeyFile . VerificationKeyFile <$> Opt.strOption
...
pTransaction = Opt.subparser $ mconcat
  [ Opt.command
    "build-raw"
    (Opt.info pTransactionBuild
    $ Opt.progDesc "Build a transaction (low-level, inconvenient)"
    )
  , Opt.command
    "sign"
  ...
  , Opt.command
    "sign-witness"
    (Opt.info pTransactionSignWit
    $ Opt.progDesc "Sign and witness a transaction"
    )
  , Opt.command
    "check"
  ...
  , Opt.command
    "submit"
    (Opt.info pTransactionSubmit . Opt.progDesc $ mconcat
  ...
  , Opt.command
    ...
    $ Opt.progDesc "Calulate the min fee for a transaction"
    )
  , Opt.command
    "txid"
 ...
 where
  ...
  pTransactionSignWit =
    TxSignWitness <$> pTxBodyFile Input <*> some pWitnessFile <*> pOutputFile

  pTransactionCheck :: Parser TransactionCmd
  pTransactionCheck = pure TxCheck

  pTransactionSubmit :: Parser TransactionCmd
  pTransactionSubmit = TxSubmit <$> pProtocol <*> pNetworkId <*> pTxSubmitFile
...
pNodeCmd = Opt.subparser $ mconcat
  [ Opt.command
    "key-gen"
  ...
  , Opt.command
    "issue-op-cert"
    (Opt.info pIssueOpCert $ Opt.progDesc "Issue a node operational certificate"
    )
  ]
 where
  ...
  pCounterValue = Opt.option
    Opt.auto
    (Opt.long "counter-value" <> Opt.metavar "INT" <> Opt.help
...
pPoolCmd = Opt.subparser $ mconcat
  ...
  , Opt.command
    "registration-certificate"
    (Opt.info pStakePoolRegistrationCert
 ...
 where
  ...
  pPoolMetaDataHashSubCmd =
    PoolMetaDataHash <$> pPoolMetaDataFile <*> pMaybeOutputFile


pQueryCmd :: Parser QueryCmd
pQueryCmd = Opt.subparser $ mconcat
  [ Opt.command
    "pool-id"
  ...
  , Opt.command
    ...
    $ Opt.progDesc "Dump the current state of the node"
    )
  , Opt.command
    "status"
 ...
 where
  ...
  pQueryProtocolParameters =
    QueryProtocolParameters <$> pProtocol <*> pNetworkId <*> pMaybeOutputFile
  ...
  pQueryUTxO =
    QueryUTxO <$> pProtocol <*> pQueryFilter <*> pNetworkId <*> pMaybeOutputFile

  pQueryStakeDistribution :: Parser QueryCmd
  pQueryStakeDistribution =
    QueryStakeDistribution <$> pProtocol <*> pNetworkId <*> pMaybeOutputFile
  ...
  pQueryLedgerState =
    QueryLedgerState <$> pProtocol <*> pNetworkId <*> pMaybeOutputFile
...
pBlockCmd = Opt.subparser $ mconcat
  [ Opt.command "info"
...
pGovernanceCmd = Opt.subparser $ mconcat
  [ Opt.command
  ...
  , Opt.command
    ...
    (Opt.info pUpdateProposal $ Opt.progDesc "Create an update proposal")
  , Opt.command "protocol-update"
                (Opt.info pProtocolUpdate $ Opt.progDesc "Protocol update")
  , Opt.command "cold-keys" (Opt.info pColdKeys $ Opt.progDesc "Cold keys")
  ]
 where
  ...
  pMIRPot =
    Opt.flag' Shelley.ReservesMIR
              (Opt.long "reserves" <> Opt.help "Use the reserves pot.")
      <|> Opt.flag' Shelley.ReservesMIR
                    (Opt.long "treasury" <> Opt.help "Use the treasury pot.")
  ...
  pGenesisKeyFile = pColdSigningKeyFile
    <|> Opt.strOption (Opt.long "genesis-key" <> Opt.internal)

pRewardAmt :: Parser Lovelace
pRewardAmt = Opt.option
  (readerFromAttoParser parseLovelace)
  (Opt.long "reward" <> Opt.metavar "LOVELACE" <> Opt.help
    "The reward for the relevant reward account."
  )

pSystemCmd :: Parser SystemCmd
pSystemCmd = Opt.subparser $ mconcat
  [ Opt.command "start" (Opt.info pSystemStart $ Opt.progDesc "Start system")
  , Opt.command "stop" (Opt.info pSystemStop $ Opt.progDesc "Stop system")
  ]
 where
...
pGenesisCmd = Opt.subparser $ mconcat
  [ Opt.command
    "key-gen-genesis"
    (Opt.info pGenesisKeyGen $ Opt.progDesc "Create a Shelley genesis key pair")
  , Opt.command
    "key-gen-delegate"
  ...
  , Opt.command
    "hash"
    (Opt.info pGenesisHash $ Opt.progDesc "Compute the hash of a genesis file")
  ]
 where
  ...
  pGenesisKeyHash = GenesisCmdKeyHash <$> pVerificationKeyFile Input
  ...
  pGenesisHash = GenesisHashFile <$> pGenesisFile

  pGenesisDir :: Parser GenesisDir
  pGenesisDir = GenesisDir <$> Opt.strOption
  ...
  pMaybeSystemStart =
    Opt.optional $ SystemStart . convertTime <$> Opt.strOption
      (Opt.long "start-time"
  ...
  pGenesisNumGenesisKeys = Opt.option
    Opt.auto
  ...
  pGenesisNumUTxOKeys = Opt.option
    Opt.auto
  ...
  convertTime = parseTimeOrError False
                                 defaultTimeLocale
                                 (iso8601DateFormat $ Just "%H:%M:%SZ")

  pInitialSupply :: Parser (Maybe Lovelace)
  pInitialSupply = Opt.optional $ Lovelace <$> Opt.option
    Opt.auto
    (Opt.long "supply"
...
pAddressKeyType =
  Opt.flag'
    ...
    <|> Opt.flag' AddressKeyByron
                  (Opt.long "byron-key" <> Opt.help "Use a Byron-era key.")
    <|> pure AddressKeyShelley


pProtocolParamsFile :: Parser ProtocolParamsFile
pProtocolParamsFile = ProtocolParamsFile <$> Opt.strOption
...
pPoolMetaDataFile = PoolMetaDataFile <$> Opt.strOption
...
pMetaDataFile =
  MetaDataFileJSON
    <$> (Opt.strOption
        ...
        <|> Opt.strOption (Opt.long "metadata-file" -- backward compat name
                                                    <> Opt.internal)
        )
    <|> MetaDataFileCBOR
    <$> Opt.strOption
...
pWithdrawal = Opt.option
  (readerFromAttoParser parseWithdrawal)
  (Opt.long "withdrawal"
...
pUpdateProposalFile =
  UpdateProposalFile
    <$> (Opt.strOption
        ...
        <|> Opt.strOption (Opt.long "update-proposal" <> Opt.internal)
        )
...
pColdSigningKeyFile =
  SigningKeyFile
    <$> (Opt.strOption
        ...
        <|> Opt.strOption (Opt.long "signing-key-file" <> Opt.internal)
        )

pSomeSigningKeyFiles :: Parser [SigningKeyFile]
pSomeSigningKeyFiles = some $ SigningKeyFile <$> Opt.strOption
...
pSigningKeyFile fdir = SigningKeyFile <$> Opt.strOption
  (Opt.long "signing-key-file"
  <> Opt.metavar "FILE"
...
pBlockId = BlockId <$> Opt.strOption
  (Opt.long "block-id" <> Opt.metavar "STRING" <> Opt.help
    "The block identifier."
  )

pKesPeriod :: Parser KESPeriod
pKesPeriod = KESPeriod <$> Opt.option
  Opt.auto
  (Opt.long "kes-period" <> Opt.metavar "NATURAL" <> Opt.help
    "The start of the KES key validity period."
  )

pEpochNo :: Parser EpochNo
pEpochNo = EpochNo <$> Opt.option
  Opt.auto
  (Opt.long "epoch" <> Opt.metavar "NATURAL" <> Opt.help "The epoch number.")


pEpochNoUpdateProp :: Parser EpochNo
pEpochNoUpdateProp = EpochNo <$> Opt.option
  Opt.auto
  (Opt.long "epoch" <> Opt.metavar "NATURAL" <> Opt.help
...
pMaybeOutputFile = optional $ OutputFile <$> Opt.strOption
  (Opt.long "out-file"
  <> Opt.metavar "FILE"
...
pOutputFile = OutputFile <$> Opt.strOption
  (Opt.long "out-file"
  <> Opt.metavar "FILE"
...
pPoolId = PoolId <$> Opt.strOption
  (Opt.long "pool-id" <> Opt.metavar "STRING" <> Opt.help "The pool identifier."
  )
...
pPrivKeyFile =
  PrivKeyFile
    <$> (Opt.strOption
        ...
        <|> Opt.strOption (Opt.long "private-key" <> Opt.internal)
        )
...
pVerificationKeyFile fdir = VerificationKeyFile <$> Opt.strOption
...
pExtendedVerificationKeyFile fdir = VerificationKeyFile <$> Opt.strOption
  (Opt.long "extended-verification-key-file"
  <> Opt.metavar "FILE"
  <> Opt.help
       (show fdir ++ " filepath of the ed25519-bip32 verification key.")
  <> Opt.completer (Opt.bashCompleter "file")
  )

pGenesisVerificationKeyFile :: Parser VerificationKeyFile
pGenesisVerificationKeyFile = VerificationKeyFile <$> Opt.strOption
...
pKESVerificationKeyFile =
  VerificationKeyFile
    <$> (Opt.strOption
        ...
        <|> Opt.strOption
              (Opt.long "hot-kes-verification-key-file" <> Opt.internal)
        )
...
pNetworkId = pMainnet <|> fmap Testnet pTestnetMagic
 where
  ...
  pMainnet = Opt.flag'
    Mainnet
    (Opt.long "mainnet" <> Opt.help "Use the mainnet magic id.")

pTestnetMagic :: Parser NetworkMagic
pTestnetMagic = NetworkMagic <$> Opt.option
  Opt.auto
  (Opt.long "testnet-magic" <> Opt.metavar "NATURAL" <> Opt.help
    "Specify a testnet magic id."
  )

pTxSubmitFile :: Parser FilePath
pTxSubmitFile = Opt.strOption
  (Opt.long "tx-file"
  <> Opt.metavar "FILE"
...
pTxIn :: Parser TxIn
pTxIn = Opt.option
  (readerFromAttoParser parseTxIn)
  (Opt.long "tx-in"
  <> Opt.metavar "TX-IN"
  <> Opt.help
...
renderTxIn (TxIn txid (TxIx txix)) = mconcat
  [Text.decodeUtf8 (serialiseToRawBytesHex txid), "#", Text.pack (show txix)]
...
pTxOut = Opt.option
  (readerFromAttoParser parseTxOut)
  (Opt.long "tx-out"
  <> Opt.metavar "TX-OUT"
  <> Opt.help
 ...
 where
  ...
  parseTxOut = TxOut <$> parseAddress <* Atto.char '+' <*> parseLovelace

pTxTTL :: Parser SlotNo
pTxTTL = SlotNo <$> Opt.option
  Opt.auto
  (Opt.long "ttl" <> Opt.metavar "SLOT" <> Opt.help "Time to live (in slots).")

pTxFee :: Parser Lovelace
pTxFee = Lovelace . (fromIntegral :: Natural -> Integer) <$> Opt.option
  Opt.auto
  (Opt.long "fee" <> Opt.metavar "LOVELACE" <> Opt.help
    "The fee amount in Lovelace."
  )

pWitnessFile :: Parser WitnessFile
pWitnessFile = WitnessFile <$> Opt.strOption
...
pTxBodyFile fdir =
  TxBodyFile
    <$> (Opt.strOption
        ...
        <|> Opt.strOption (Opt.long "tx-body-file" <> Opt.internal)
        )
 where
  optName = case fdir of
    Input -> "tx-body-file"
    Output -> "out-file"
...
pTxFile fdir =
  TxFile
    <$> (Opt.strOption
        ...
        <|> Opt.strOption (Opt.long "tx-file" <> Opt.internal)
        )
 where
  optName = case fdir of
    Input -> "tx-file"
...
pTxInCount = TxInCount <$> Opt.option
  Opt.auto
  (Opt.long "tx-in-count" <> Opt.metavar "NATURAL" <> Opt.help
    "The number of transaction inputs."
  )

pTxOutCount :: Parser TxOutCount
pTxOutCount = TxOutCount <$> Opt.option
  Opt.auto
  (Opt.long "tx-out-count" <> Opt.metavar "NATURAL" <> Opt.help
    "The number of transaction outputs."
  )

pTxShelleyWitnessCount :: Parser TxShelleyWitnessCount
pTxShelleyWitnessCount = TxShelleyWitnessCount <$> Opt.option
  Opt.auto
  (Opt.long "witness-count" <> Opt.metavar "NATURAL" <> Opt.help
    "The number of Shelley key witnesses."
  )

pTxByronWitnessCount :: Parser TxByronWitnessCount
pTxByronWitnessCount = TxByronWitnessCount <$> Opt.option
  Opt.auto
...
pQueryFilter = pAddresses <|> pure NoFilter
 where
  ...
  pAddresses = FilterByAddress . Set.fromList <$> some pFilterByAddress

pFilterByAddress :: Parser (Address Shelley)
pFilterByAddress = Opt.option
  (readerFromAttoParser parseAddress)
  (Opt.long "address" <> Opt.metavar "ADDRESS" <> Opt.help
    "Filter by Cardano address(es) (Bech32-encoded)."
  )

pFilterByStakeAddress :: Parser StakeAddress
pFilterByStakeAddress = Opt.option
  (readerFromAttoParser parseStakeAddress)
  (Opt.long "address" <> Opt.metavar "ADDRESS" <> Opt.help
    "Filter by Cardano stake address (Bech32-encoded)."
  )

pAddress :: Parser Text
pAddress = Text.pack <$> Opt.strOption
  (Opt.long "address" <> Opt.metavar "ADDRESS" <> Opt.help "A Cardano address")
...
pStakeVerificationKeyFile =
  VerificationKeyFile
    <$> (Opt.strOption
        ...
        <|> Opt.strOption
              (Opt.long "staking-verification-key-file" <> Opt.internal)
        )
...
pPoolStakeVerificationKeyFile =
  VerificationKeyFile
    <$> (Opt.strOption
        ...
        <|> Opt.strOption
              (Opt.long "stake-pool-verification-key-file" <> Opt.internal)
        )

pStakePoolVerificationKeyHash :: Parser (Hash StakePoolKey)
pStakePoolVerificationKeyHash = Opt.option
  (Opt.maybeReader spvkHash)
  (Opt.long "cold-verification-key-hash" <> Opt.metavar "HASH" <> Opt.help
    "Stake pool verification key hash (hex-encoded)."
  )
 where
...
pStakePoolVerificationKeyHashOrFile =
  StakePoolVerificationKeyFile
    <$> pPoolStakeVerificationKeyFile
    <|> StakePoolVerificationKeyHash
    <$> pStakePoolVerificationKeyHash

pVRFVerificationKeyFile :: Parser VerificationKeyFile
pVRFVerificationKeyFile = VerificationKeyFile <$> Opt.strOption
...
pRewardAcctVerificationKeyFile =
  VerificationKeyFile
    <$> (Opt.strOption
            ...
            <> Opt.metavar "FILE"
            <> Opt.help
                 ("Filepath of the reward account staking verification key.")
            <> Opt.completer (Opt.bashCompleter "file")
            )
        <|> Opt.strOption
              (Opt.long "reward-account-verification-key-file" <> Opt.internal
              )
        )
...
pPoolOwner =
  VerificationKeyFile
    <$> (Opt.strOption
            ...
            <> Opt.metavar "FILE"
            <> Opt.help
                 ("Filepath of the pool owner staking verification key.")
            <> Opt.completer (Opt.bashCompleter "file")
            )
        <|> Opt.strOption
              (Opt.long "pool-owner-staking-verification-key" <> Opt.internal)
        )


pPoolPledge :: Parser Lovelace
pPoolPledge = Opt.option
  (readerFromAttoParser parseLovelace)
  (Opt.long "pool-pledge" <> Opt.metavar "LOVELACE" <> Opt.help
    "The stake pool's pledge."
  )


pPoolCost :: Parser Lovelace
pPoolCost = Opt.option
  (readerFromAttoParser parseLovelace)
  (Opt.long "pool-cost" <> Opt.metavar "LOVELACE" <> Opt.help
    "The stake pool's cost."
  )

pPoolMargin :: Parser Rational
pPoolMargin = Opt.option
  readRationalUnitInterval
  (Opt.long "pool-margin" <> Opt.metavar "DOUBLE" <> Opt.help
...
pMultiHostName = StakePoolRelayDnsSrvRecord <$> pDNSName
 where
  pDNSName :: Parser ByteString
  pDNSName = Opt.option
    (Opt.eitherReader eDNSName)
...
pSingleHostName = StakePoolRelayDnsARecord <$> pDNSName <*> optional pPort
 where
  pDNSName :: Parser ByteString
  pDNSName = Opt.option
    (Opt.eitherReader eDNSName)
...
pSingleHostAddress = liftA3 (\ip4 ip6 port -> singleHostAddress ip4 ip6 port)
                            (optional pIpV4)
                            (optional pIpV6)
                            pPort
 where
  singleHostAddress
    :: Maybe IP.IPv4 -> Maybe IP.IPv6 -> PortNumber -> StakePoolRelay
  singleHostAddress ipv4 ipv6 port = case (ipv4, ipv6) of
    (Nothing, Nothing) ->
      panic $ "Please enter either an IPv4 or IPv6 address for the pool relay"
    (Just i4, Nothing) -> StakePoolRelayIp (Just i4) Nothing (Just port)
    (Nothing, Just i6) -> StakePoolRelayIp Nothing (Just i6) (Just port)
    (Just i4, Just i6) -> StakePoolRelayIp (Just i4) (Just i6) (Just port)



pIpV4 :: Parser IP.IPv4
pIpV4 = Opt.option
  (Opt.maybeReader readMaybe :: Opt.ReadM IP.IPv4)
  (Opt.long "pool-relay-ipv4" <> Opt.metavar "STRING" <> Opt.help
    "The stake pool relay's IPv4 address"
  )

pIpV6 :: Parser IP.IPv6
pIpV6 = Opt.option
  (Opt.maybeReader readMaybe :: Opt.ReadM IP.IPv6)
  (Opt.long "pool-relay-ipv6" <> Opt.metavar "STRING" <> Opt.help
    "The stake pool relay's IPv6 address"
  )

pPort :: Parser PortNumber
pPort = Opt.option
  (fromInteger <$> Opt.eitherReader readEither)
  (Opt.long "pool-relay-port" <> Opt.metavar "INT" <> Opt.help
...
pStakePoolMetadataUrl = Opt.option
  (readURIOfMaxLength 64)
  (Opt.long "metadata-url" <> Opt.metavar "URL" <> Opt.help
    "Pool metadata URL (maximum length of 64 characters)."
  )

pStakePoolMetadataHash :: Parser (Hash StakePoolMetadata)
pStakePoolMetadataHash = Opt.option
  (Opt.maybeReader metadataHash)
  (Opt.long "metadata-hash" <> Opt.metavar "HASH" <> Opt.help
    "Pool metadata hash."
  )
 where
  getHashFromHexString
    :: String -> Maybe (Crypto.Hash Crypto.Blake2b_256 ByteString)
...
pMinFeeLinearFactor = Opt.option
  Opt.auto
  (Opt.long "min-fee-linear" <> Opt.metavar "NATURAL" <> Opt.help
    "The linear factor for the minimum fee calculation."
  )

pMinFeeConstantFactor :: Parser Natural
pMinFeeConstantFactor = Opt.option
  Opt.auto
  (Opt.long "min-fee-constant" <> Opt.metavar "LOVELACE" <> Opt.help
    "The constant factor for the minimum fee calculation."
  )

pMinUTxOValue :: Parser Lovelace
pMinUTxOValue = Opt.option
  (readerFromAttoParser parseLovelace)
  (Opt.long "min-utxo-value" <> Opt.metavar "NATURAL" <> Opt.help
    "The minimum allowed UTxO value."
  )

pMinPoolCost :: Parser Lovelace
pMinPoolCost = Opt.option
  (readerFromAttoParser parseLovelace)
  (Opt.long "min-pool-cost" <> Opt.metavar "NATURAL" <> Opt.help
    "The minimum allowed cost parameter for stake pools."
  )

pMaxBodySize :: Parser Natural
pMaxBodySize = Opt.option
  Opt.auto
  (Opt.long "max-block-body-size" <> Opt.metavar "NATURAL" <> Opt.help
    "Maximal block body size."
  )

pMaxTransactionSize :: Parser Natural
pMaxTransactionSize = Opt.option
  Opt.auto
  (Opt.long "max-tx-size" <> Opt.metavar "NATURAL" <> Opt.help
    "Maximum transaction size."
  )

pMaxBlockHeaderSize :: Parser Natural
pMaxBlockHeaderSize = Opt.option
  Opt.auto
  (Opt.long "max-block-header-size" <> Opt.metavar "NATURAL" <> Opt.help
    "Maximum block header size."
  )

pKeyRegistDeposit :: Parser Lovelace
pKeyRegistDeposit = Opt.option
  (readerFromAttoParser parseLovelace)
  (Opt.long "key-reg-deposit-amt" <> Opt.metavar "NATURAL" <> Opt.help
    "Key registration deposit amount."
  )

pPoolDeposit :: Parser Lovelace
pPoolDeposit = Opt.option
  (readerFromAttoParser parseLovelace)
  (Opt.long "pool-reg-deposit" <> Opt.metavar "NATURAL" <> Opt.help
    "The amount of a pool registration deposit."
  )

pEpochBoundRetirement :: Parser EpochNo
pEpochBoundRetirement = EpochNo <$> Opt.option
  Opt.auto
  (Opt.long "pool-retirement-epoch-boundary" <> Opt.metavar "INT" <> Opt.help
    "Epoch bound on pool retirement."
  )

pNumberOfPools :: Parser Natural
pNumberOfPools = Opt.option
  Opt.auto
  (Opt.long "number-of-pools" <> Opt.metavar "NATURAL" <> Opt.help
    "Desired number of pools."
  )

pPoolInfluence :: Parser Rational
pPoolInfluence = Opt.option
  readRational
  (Opt.long "pool-influence" <> Opt.metavar "DOUBLE" <> Opt.help
    "Pool influence."
  )

pTreasuryExpansion :: Parser Rational
pTreasuryExpansion = Opt.option
  readRationalUnitInterval
  (Opt.long "treasury-expansion" <> Opt.metavar "DOUBLE" <> Opt.help
    "Treasury expansion."
  )

pMonetaryExpansion :: Parser Rational
pMonetaryExpansion = Opt.option
  readRationalUnitInterval
  (Opt.long "monetary-expansion" <> Opt.metavar "DOUBLE" <> Opt.help
    "Monetary expansion."
  )

pDecentralParam :: Parser Rational
pDecentralParam = Opt.option
  readRationalUnitInterval
  (Opt.long "decentralization-parameter" <> Opt.metavar "DOUBLE" <> Opt.help
...
pExtraEntropy =
  Opt.option
      (Just <$> readerFromAttoParser parseEntropyBytes)
      (Opt.long "extra-entropy" <> Opt.metavar "HEX" <> Opt.help
 ...
 where
  ...
  parseEntropyBytes = (fst . Base16.decode) <$> Atto.takeWhile1 Char.isHexDigit
...
pProtocol =
  (Opt.flag'
    ...
    *> pShelley
    ...
    <|> (Opt.flag'
            ()
            (Opt.long "cardano-mode" <> Opt.help
 ...
 where
  ...
  pEpochSlots =
    (EpochSlots <$> Opt.option
        Opt.auto
        (Opt.long "epoch-slots" <> Opt.metavar "NATURAL" <> Opt.help
  ...
  pSecurityParam =
    (SecurityParam <$> Opt.option
        Opt.auto
        (Opt.long "security-param" <> Opt.metavar "NATURAL" <> Opt.help
...
pProtocolVersion = (,) <$> pProtocolMajorVersion <*> pProtocolMinorVersion
 where
  pProtocolMajorVersion = Opt.option
    Opt.auto
    (Opt.long "protocol-major-version" <> Opt.metavar "NATURAL" <> Opt.help
      "Major protocol version. An increase indicates a hard fork."
    )
  pProtocolMinorVersion = Opt.option
    Opt.auto
...
lexPlausibleAddressString = Text.decodeLatin1
  <$> Atto.takeWhile1 isPlausibleAddressChar
 where
