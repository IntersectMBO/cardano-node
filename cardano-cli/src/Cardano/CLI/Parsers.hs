...
parseClientCommand = asum [parseByron, parseShelley, parseDisplayVersion]

parseByron :: Parser ClientCommand
parseByron = fmap ByronCommand $ subparser $ mconcat
...
parseShelley = subparser $ mconcat
...
parseDisplayVersion =
  subparser
    ...
    <|> flag'
          DisplayVersion
          (long "version" <> help "Show the cardano-cli version" <> hidden)
