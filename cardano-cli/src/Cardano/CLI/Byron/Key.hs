...
renderByronKeyFailure err = case err of
  ...
  ReadVerificationKeyFailure vKeyFp readErr ->
    "Error reading verification key at: "
      <> textShow vKeyFp
      <> " Error: "
      <> textShow readErr
  SigningKeyDeserialisationFailed sKeyFp deSerError ->
    "Error derserializing signing key at: "
      <> textShow sKeyFp
      <> " Error: "
      <> textShow deSerError
  VerificationKeyDeserialisationFailed vKeyFp deSerError ->
    "Error derserializing verification key at: "
      <> textShow vKeyFp
      <> " Error: "
      <> textShow deSerError
...
serialiseSigningKey
  :: CardanoEra -> Crypto.SigningKey -> Either ByronKeyFailure LB.ByteString
serialiseSigningKey ByronEraLegacy (Crypto.SigningKey k) =
  pure $ toLazyByteString (Crypto.toCBORXPrv k)
serialiseSigningKey ByronEra (Crypto.SigningKey k) =
  pure $ toLazyByteString (Crypto.toCBORXPrv k)
...
readEraSigningKey
  :: CardanoEra -> SigningKeyFile -> ExceptT ByronKeyFailure IO SigningKey
readEraSigningKey era (SigningKeyFile fp) = do
  sK <- handleIOExceptT (ReadSigningKeyFailure fp . T.pack . displayException)
    $ LB.readFile fp
...
readPaymentVerificationKey
  :: VerificationKeyFile -> ExceptT ByronKeyFailure IO Crypto.VerificationKey
readPaymentVerificationKey (VerificationKeyFile fp) = do
  vkB <- handleIOExceptT
    (ReadVerificationKeyFailure fp . T.pack . displayException)
    (SB.readFile fp)
  -- Verification Key
  let
    eVk =
      hoistEither . Crypto.parseFullVerificationKey . fromString $ UTF8.toString
        vkB
...
serialisePoorKey
  :: CardanoEra -> Genesis.PoorSecret -> Either ByronKeyFailure LB.ByteString
...
readPassword prompt = Crypto.PassPhrase . BA.convert <$> loop
  ...
  loop = do
    ...
    if v1 == v2
      then pure v1
        ...
        hPutStrLn stdout ("Sorry, entered passwords don't match." :: String)
          >> loop
