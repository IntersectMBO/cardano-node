...
renderByronDelegationError err = case err of
  CertificateValidationErrors certFp errs ->
    "Certificate validation error(s) at: "
      <> textShow certFp
      <> " Errors: "
      <> textShow errs
  DlgCertificateDeserialisationFailed certFp deSererr ->
    "Certificate deserialisation error at: "
      <> textShow certFp
      <> " Error: "
      <> textShow deSererr
...
issueByronGenesisDelegation magic epoch issuerSK delegateVK =
  Dlg.signCertificate magic delegateVK epoch $ Crypto.noPassSafeSigner issuerSK
...
checkByronGenesisDelegation (CertificateFile certF) magic issuer delegate = do
  ...
  case ecert of
    ...
    Right (cert :: Dlg.Certificate) -> do
      ...
      unless (null issues) $ left $ CertificateValidationErrors certF issues
...
serialiseDelegateKey
  :: CardanoEra -> SigningKey -> Either ByronDelegationError LB.ByteString
serialiseDelegateKey ByronEraLegacy sk =
  pure
    . toLazyByteString
    . Legacy.encodeLegacyDelegateKey
    $ Legacy.LegacyDelegateKey sk
serialiseDelegateKey ByronEra sk =
  first ByronDelegationKeyError $ serialiseSigningKey ByronEra sk
