...
renderByronClientCmdError err = case err of
...
runByronClientCommand c = case c of
  NodeCmd bc -> runNodeCmd bc
  Genesis outDir params era -> runGenesisCommand outDir params era
  GetLocalNodeTip network ->
    firstExceptT ByronCmdQueryError $ runGetLocalNodeTip network
  ...
  MigrateDelegateKeyFrom oldEra oldKey newEra nskf ->
    runMigrateDelegateKeyFrom oldEra oldKey newEra nskf
  PrintGenesisHash genFp -> runPrintGenesisHash genFp
  PrintSigningKeyAddress era networkid skF ->
    runPrintSigningKeyAddress era networkid skF
  ...
  CheckDelegation nw cert issuerVF delegateVF ->
    runCheckDelegation nw cert issuerVF delegateVF
...
runNodeCmd (CreateVote nw sKey upPropFp voteBool outputFp) =
  firstExceptT ByronCmdVoteError
...
runPrettySigningKeyPublic
  :: CardanoEra -> SigningKeyFile -> ExceptT ByronClientCmdError IO ()
...
runMigrateDelegateKeyFrom oldEra oldKey newEra (NewSigningKeyFile newKey) = do
  ...
  sDk <- hoistEither . first ByronCmdDelegationError $ serialiseDelegateKey
    newEra
    sk
...
runPrintGenesisHash genFp = do
  genesis <- firstExceptT ByronCmdGenesisError $ readGenesis genFp dummyNetwork
...
runKeygen era (NewSigningKeyFile skF) passReq = do
  ...
  serDk <- hoistEither . first ByronCmdDelegationError $ serialiseDelegateKey
    era
    sK
...
runIssueDelegationCertificate nw era epoch issuerSK delegateVK cert = do
  ...
  let byGenDelCert :: Delegation.Certificate
      byGenDelCert =
        issueByronGenesisDelegation (toByronProtocolMagicId nw) epoch sk vk
...
runCheckDelegation nw cert issuerVF delegateVF = do
  issuerVK <- firstExceptT ByronCmdKeyFailure
    $ readPaymentVerificationKey issuerVF
  delegateVK <- firstExceptT ByronCmdKeyFailure
    $ readPaymentVerificationKey delegateVF
  firstExceptT ByronCmdDelegationError $ checkByronGenesisDelegation
    cert
...
runSpendGenesisUTxO
  ...
  -> NonEmpty TxOut
  -> ExceptT ByronClientCmdError IO ()
runSpendGenesisUTxO genesisFile nw era (NewTxFile ctTx) ctKey genRichAddr outs
  = do
