...
import           Cardano.Chain.UTxO             ( TxId
                                                , TxIn(..)
                                                , TxOut(..)
                                                )
...
import           Cardano.Config.Parsers         ( parseIntegral
                                                , parseFraction
                                                , parseLovelace
                                                , readDouble
                                                , parseFilePath
...
parseNode = mconcat
  [ Opt.command
      ...
      $ Opt.progDesc "Byron node operation commands"
      )
  ]

parseCBORObject :: Parser CBORObject
parseCBORObject = asum
  [ CBORBlockByron <$> option
    auto
    (long "byron-block"
    <> (help
  ...
  , flag' CBORTxByron $ long "byron-tx" <> help
    "The CBOR file is a byron era tx"
  , flag' CBORUpdateProposalByron $ long "byron-update-proposal" <> help
    "The CBOR file is a byron era update proposal"
  , flag' CBORVoteByron $ long "byron-vote" <> help
    "The CBOR file is a byron era vote"
  ]

parseDelegationRelatedValues :: Mod CommandFields ByronCommand
parseDelegationRelatedValues = mconcat
  [ command'
  ...
  $ CheckDelegation
  <$> pNetworkId
  <*> parseCertificateFile "certificate"
                           "The certificate embodying delegation to verify."
  <*> parseVerificationKeyFile "issuer-key"
...
parseGenesisParameters =
  GenesisParameters
    ...
    <*> parseFilePath "protocol-parameters-file"
...
parseKeyRelatedValues = mconcat
  ...
  <$> parseCardanoEra
  <*> pNetworkId
  <*> parseSigningKeyFile "secret"
                          "Signing key, whose address is to be printed."
  , command' "migrate-delegate-key-from"
...
parseLocalNodeQueryValues = mconcat
...
parseMiscellaneous = mconcat
  [ command' "validate-cbor" "Validate a CBOR blockchain object."
  $ ValidateCBOR
  <$> parseCBORObject
  <*> parseFilePath "filepath" "Filepath of CBOR file."
  , command' "pretty-print-cbor" "Pretty print a CBOR file."
  $ PrettyPrintCBOR
  <$> parseFilePath "filepath" "Filepath of CBOR file."
  ]
...
parseTxRelatedValues = mconcat
  ...
  <$> pNetworkId
  <*> parseTxFile "tx"
  , command' "issue-genesis-utxo-expenditure"
...
parseVerificationKeyFile opt desc =
  VerificationKeyFile <$> parseFilePath opt desc

pNodeCmd :: Parser NodeCmd
pNodeCmd = Opt.subparser $ mconcat
  [ Opt.command
    "create-update-proposal"
    (Opt.info parseByronUpdateProposal
  ...
  , Opt.command
    ...
    (Opt.info parseByronUpdateProposalSubmission
...
parseByronUpdateProposalSubmission =
  SubmitUpdateProposal <$> pNetworkId <*> parseFilePath
    "filepath"
...
parseByronVote =
  CreateVote
    <$> pNetworkId
    <*> (SigningKeyFile
        <$> parseFilePath "signing-key" "Filepath of signing key."
        )
...
parseScriptVersion = optional $ ScriptVersion <$> option
  auto
  (long "script-version" <> metavar "WORD16" <> help "Proposed script version.")

parseSlotDuration :: Parser (Maybe ParametersToUpdate)
parseSlotDuration = optional $ SlotDuration <$> option
  auto
  (long "slot-duration" <> metavar "NATURAL" <> help "Proposed slot duration.")

parseSystemTag :: Parser SystemTag
parseSystemTag = option
  (eitherReader checkSysTag)
  (long "system-tag" <> metavar "STRING" <> help
...
parseInstallerHash = InstallerHash . hashRaw . C8.pack <$> strOption
  (long "installer-hash" <> metavar "HASH" <> help "Software hash.")

parseMaxBlockSize :: Parser (Maybe ParametersToUpdate)
parseMaxBlockSize = optional $ MaxBlockSize <$> option
  auto
  (long "max-block-size" <> metavar "NATURAL" <> help "Proposed max block size."
  )

parseMaxHeaderSize :: Parser (Maybe ParametersToUpdate)
parseMaxHeaderSize = optional $ MaxHeaderSize <$> option
  auto
  (long "max-header-size" <> metavar "NATURAL" <> help
    "Proposed max block header size."
  )

parseMaxTxSize :: Parser (Maybe ParametersToUpdate)
parseMaxTxSize = optional $ MaxTxSize <$> option
  auto
  (long "max-tx-size" <> metavar "NATURAL" <> help
    "Proposed max transaction size."
  )

parseMaxProposalSize :: Parser (Maybe ParametersToUpdate)
parseMaxProposalSize = optional $ MaxProposalSize <$> option
  auto
  (long "max-proposal-size" <> metavar "NATURAL" <> help
    "Proposed max update proposal size."
  )

parseMpcThd :: Parser (Maybe ParametersToUpdate)
parseMpcThd = optional $ MpcThd . rationalToLovelacePortion <$> parseFraction
  "max-mpc-thd"
...
parseProtocolVersion =
  ProtocolVersion
    <$> (parseWord "protocol-version-major" "Protocol verson major." "WORD16" :: Parser
            Word16
        )
    <*> (parseWord "protocol-version-minor" "Protocol verson minor." "WORD16" :: Parser
            Word16
        )
    <*> (parseWord "protocol-version-alt" "Protocol verson alt." "WORD8" :: Parser
            Word8
        )

parseHeavyDelThd :: Parser (Maybe ParametersToUpdate)
parseHeavyDelThd =
  optional $ HeavyDelThd . rationalToLovelacePortion <$> parseFraction
    "heavy-del-thd"
    "Proposed heavy delegation threshold."

parseUpdateVoteThd :: Parser (Maybe ParametersToUpdate)
parseUpdateVoteThd =
  optional $ UpdateVoteThd . rationalToLovelacePortion <$> parseFraction
    "update-vote-thd"
    "Propose update vote threshold."

parseUpdateProposalThd :: Parser (Maybe ParametersToUpdate)
parseUpdateProposalThd =
  optional $ UpdateProposalThd . rationalToLovelacePortion <$> parseFraction
    "update-proposal-thd"
    "Propose update proposal threshold."

parseUpdateProposalTTL :: Parser (Maybe ParametersToUpdate)
parseUpdateProposalTTL = optional $ UpdateProposalTTL . SlotNumber <$> option
  auto
  (long "time-to-live" <> metavar "WORD64" <> help
...
parseSoftforkRuleParam =
  optional
    $ SoftforkRuleParam
    <$> (SoftforkRule
        <$> (rationalToLovelacePortion <$> parseFraction
              "softfork-init-thd"
...
parseApplicationName = option
  (eitherReader checkAppNameLength)
  (long "application-name" <> metavar "STRING" <> help
...
parseNumSoftwareVersion = parseWord
...
parseVoteBool =
  flag'
      True
      (long "vote-yes" <> help "Vote yes with respect to an update proposal.")
    <|> flag'
          False
          (long "vote-no" <> help "Vote no with respect to an update proposal.")

parseUnlockStakeEpoch :: Parser (Maybe ParametersToUpdate)
parseUnlockStakeEpoch = optional $ UnlockStakeEpoch . EpochNumber <$> option
  auto
  (long "unlock-stake-epoch" <> metavar "WORD64" <> help
...
parseWord optname desc metvar =
  option (fromInteger <$> auto) $ long optname <> metavar metvar <> help desc
...
parseCardanoEra = asum
  [ flag' ByronEraLegacy $ long "byron-legacy-formats" <> help
    "Byron/cardano-sl formats and compatibility"
  , flag' ByronEra $ long "byron-formats" <> help
...
pNetworkId = pMainnet' <|> fmap Typed.Testnet pTestnetMagic
 where
...
pTestnetMagic = Typed.NetworkMagic <$> Opt.option
  Opt.auto
...
cliParseLovelace =
  either (panic . ("Bad Lovelace value: " <>) . show) identity . mkLovelace
