...
import           Cardano.Chain.UTxO             ( mkTxAux
                                                , annotateTxAux
                                                , Tx(..)
                                                , TxId
                                                , TxIn
...
import qualified Ouroboros.Consensus.Byron.Ledger
                                               as Byron
...
renderByronTxError err = case err of
  TxDeserialisationFailed txFp decErr ->
    "Transaction deserialisation failed at "
      <> textShow txFp
      <> " Error: "
      <> textShow decErr
...
prettyAddress addr =
  sformat (Common.addressF % "\n" % Common.addressDetailedF) addr addr
...
genesisUTxOTxIn gc vk genAddr =
 ...
 where
  ...
  fromCompactTxInTxOutList
    :: [(UTxO.CompactTxIn, UTxO.CompactTxOut)] -> [(UTxO.TxIn, UTxO.TxOut)]
  fromCompactTxInTxOutList =
  ...
  handleMissingAddr =
    fromMaybe
      ...
      <> Cardano.Prelude.concat
           (T.unpack . prettyAddress <$> Map.keys initialUtxo)
...
txSpendGenesisUTxOByronPBFT gc nw sk genAddr outs = annotateTxAux
  $ mkTxAux tx (pure wit)
 where
...
txSpendUTxOByronPBFT
  ...
  -> NonEmpty TxOut
  -> UTxO.ATxAux ByteString
txSpendUTxOByronPBFT nw sk ins outs = annotateTxAux
  $ mkTxAux tx (Vector.singleton wit)
 where
...
fromCborTxAux
  :: LB.ByteString -> Either Binary.DecoderError (UTxO.ATxAux B.ByteString)
fromCborTxAux lbs = fmap (annotationBytes lbs) $ Binary.decodeFullDecoder
  "Cardano.Chain.UTxO.TxAux.fromCborTxAux"
  Binary.fromCBOR
  lbs
 where
  annotationBytes
    :: Functor f => LB.ByteString -> f Binary.ByteSpan -> f B.ByteString
