...
import           Cardano.Chain.Update           ( AProposal(..)
...
import           Ouroboros.Consensus.Byron.Ledger.Block
                                                ( ByronBlock )
import qualified Ouroboros.Consensus.Byron.Ledger.Mempool
                                               as Mempool
...
import           Cardano.CLI.Byron.Tx           ( ByronTxError
                                                , nodeSubmitTx
                                                )
...
renderByronUpdateProposalError err = case err of
  ByronReadUpdateProposalFileFailure fp rErr ->
    "Error reading update proposal at "
      <> textShow fp
      <> " Error: "
      <> textShow rErr
...
runProposalCreation nw sKey@(SigningKeyFile sKeyfp) pVer sVer sysTag insHash outputFp params
  = do
    sK <- firstExceptT (ReadSigningKeyFailure sKeyfp)
      $ readEraSigningKey ByronEra sKey
...
createProtocolParametersUpdate init paramsToUpdate = go init paramsToUpdate
 where
  go i [] = i
  go i (paramToUpdate : rest) = case paramToUpdate of
...
convertProposalToGenTx prop =
  Mempool.ByronUpdateProposal (recoverUpId prop) prop
...
createUpdateProposal nw sKey pVer sVer sysTag inshash paramsToUpdate =
 ...
 where
  ...
  protocolParamsUpdate =
    createProtocolParametersUpdate emptyProtocolParametersUpdate paramsToUpdate
...
deserialiseByronUpdateProposal
  :: LByteString -> Either ByronUpdateProposalError (AProposal ByteString)
deserialiseByronUpdateProposal bs = case Binary.decodeFull bs of
...
readByronUpdateProposal
  :: FilePath -> ExceptT ByronUpdateProposalError IO LByteString
readByronUpdateProposal fp = handleIOExceptT
  (ByronReadUpdateProposalFileFailure fp . toS . displayException)
  (LB.readFile fp)


submitByronUpdateProposal
  :: NetworkId -> FilePath -> ExceptT ByronUpdateProposalError IO ()
