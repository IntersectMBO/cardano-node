...
import Cardano.Api (CardanoEra, IsShelleyBasedEra, ShelleyBasedEra, TxInMode(TxInMode))
...
handleSendMoney era sbe FaucetState{network,utxoTMVar,skey,queue} addr = do
  eResult <- liftIO $ runExceptT $ do
    ...
    liftIO $ atomically $ writeTQueue queue (TxInMode signedTx eraInMode)
    putStrLn $ friendlyTxBS era signedTx
