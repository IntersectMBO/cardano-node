...
golden_shelleyGenesisKeys = propertyOnce . H.moduleWorkspace "tmp" $ \tempDir -> do
  -- Reference keys
  referenceVerKey <- noteInputFile "golden/files/golden/shelley/keys/genesis_keys/verification_key"
  referenceSignKey <- noteInputFile "golden/files/golden/shelley/keys/genesis_keys/signing_key"
