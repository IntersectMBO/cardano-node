...
golden_shelleyGenesisKeyHash = OP.propertyOnce $ do
  OP.workspace "tmp/genesis-key-hash" $ \tempDir -> do
    referenceVerificationKey <- OP.noteInputFile
      "test/Test/golden/shelley/keys/genesis_keys/verification_key"
    goldenGenesisVerificationKeyHashFile <- OP.noteInputFile
      "test/Test/golden/shelley/keys/genesis_keys/verification_key.key-hash"
    ...
    liftIO
      $ IO.writeFile genesisVerificationKeyHashFile genesisVerificationKeyHash

    goldenGenesisVerificationKeyHash <- OP.noteEvalM . liftIO $ IO.readFile
