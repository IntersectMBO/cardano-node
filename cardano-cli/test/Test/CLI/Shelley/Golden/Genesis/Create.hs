...
parseMaxLovelaceSupply =
  J.withObject "Object" $ \o -> o J..: "maxLovelaceSupply"
...
parseHashMap (J.Object hm) =
  pure $ HM.fromList $ fmap (first T.unpack) (HM.toList (hm))
...
golden_shelleyGenesisCreate = OP.propertyOnce $ do
  OP.workspace "tmp/genesis-create" $ \tempDir -> do
    sourceGenesisSpecFile <- OP.noteInputFile
      "test/Test/golden/shelley/genesis/genesis.spec.json"
    ...
    (supply, fmtSupply) <- fmap (withSnd show) $ forAll $ G.int
      (R.linear 10000000 4000000000)
    (delegateCount, fmtDelegateCount) <- fmap (withSnd show) $ forAll $ G.int
      (R.linear 4 19)
    (utxoCount, fmtUtxoCount) <- fmap (withSnd show) $ forAll $ G.int
      (R.linear 4 19)
    ...
    actualSupply <- H.evalEither
      $ J.parseEither parseMaxLovelaceSupply actualJson
    actualStartTime <- H.evalEither $ J.parseEither parseSystemStart actualJson
    actualDelegateCount <- H.evalEither
      $ J.parseEither parseDelegateCount actualJson
    actualTotalSupply <- H.evalEither
      $ J.parseEither parseTotalSupply actualJson
    actualHashKeys <- H.evalEither $ J.parseEither parseHashKeys actualJson
    actualDelegateKeys <- H.evalEither
      $ J.parseEither parseDelegateKeys actualJson
    ...
    for_ [1 .. delegateCount] $ \i -> do
      -- Check Genesis keys
      OP.assertFileOccurences 1 "GenesisSigningKey_ed25519"
        $ tempDir
        <> "/genesis-keys/genesis"
        <> show i
        <> ".skey"
      OP.assertFileOccurences 1 "GenesisVerificationKey_ed25519"
        $ tempDir
        <> "/genesis-keys/genesis"
        <> show i
        <> ".vkey"

      OP.assertEndsWithSingleNewline
        $ tempDir
        <> "/genesis-keys/genesis"
        <> show i
        <> ".skey"
      ...
      -- Check delegate keys
      OP.assertFileOccurences 1 "GenesisDelegateSigningKey_ed25519"
        $ tempDir
        <> "/delegate-keys/delegate"
        <> show i
        <> ".skey"
      OP.assertFileOccurences 1 "GenesisDelegateVerificationKey_ed25519"
        $ tempDir
        <> "/delegate-keys/delegate"
        <> show i
        <> ".vkey"
      ...
      -- Check utxo keys
      OP.assertFileOccurences 1 "GenesisUTxOSigningKey_ed25519"
        $ tempDir
        <> "/utxo-keys/utxo"
        <> show i
        <> ".skey"
      OP.assertFileOccurences 1 "GenesisUTxOVerificationKey_ed25519"
        $ tempDir
        <> "/utxo-keys/utxo"
        <> show i
        <> ".vkey"
