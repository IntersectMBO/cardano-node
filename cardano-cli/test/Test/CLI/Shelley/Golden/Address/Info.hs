...
module Test.CLI.Shelley.Golden.Address.Info
...
golden_shelleyAddressInfo = OP.propertyOnce $ do
  ...
  when False $ do
    ...
    infoText1 <- OP.noteEvalM $ OP.execCardanoCLI
      ["shelley", "address", "info", "--address", byronBase58]
    ...
    infoText2 <- OP.noteEvalM $ OP.execCardanoCLI
      ["shelley", "address", "info", "--address", byronHex]
    ...
    let
      shelleyHex
        = "82065820d8b4a892f2f6f1820d350c207d17d4cd7e7a1f7e0a83059e2d698a65ab8f96ed"

    infoText3 <- OP.noteEvalM $ OP.execCardanoCLI
      ["shelley", "address", "info", "--address", shelleyHex]
