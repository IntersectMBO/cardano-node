...
golden_shelleyAddressBuild =
  OP.propertyOnce $ OP.workspace "tmp/address-build" $ \tempDir -> do
    addressVKeyFile <- OP.noteInputFile
      "test/Test/golden/shelley/keys/payment_keys/verification_key"
    addressSKeyFile <- OP.noteInputFile
      "test/Test/golden/shelley/keys/stake_keys/verification_key"
    goldenStakingAddressHexFile <- OP.noteInputFile
      "test/Test/golden/shelley/addresses/staking-address.hex"
    goldenEnterpriseAddressHexFile <- OP.noteInputFile
      "test/Test/golden/shelley/addresses/enterprise-address.hex"
    ...
    goldenStakingAddressHex <- OP.noteEvalM . liftIO $ IO.readFile
    ...
    goldenEnterpriseAddressHex <- OP.noteEvalM . liftIO $ IO.readFile
