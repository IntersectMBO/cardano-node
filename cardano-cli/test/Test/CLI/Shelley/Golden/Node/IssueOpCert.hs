...
golden_shelleyNodeIssueOpCert = OP.propertyOnce $ do
  OP.workspace "tmp/node-issue-op-cert" $ \tempDir -> do
    hotKesVerificationKeyFile <- OP.noteInputFile
      "test/Test/golden/shelley/keys/kes_keys/verification_key"
    coldSigningKeyFile <- OP.noteInputFile
      "test/Test/golden/shelley/keys/genesis_delegate_keys/signing_key"
    originalOperationalCertificateIssueCounterFile <-
      OP.noteInputFile
        "test/Test/golden/shelley/keys/genesis_delegate_keys/operational_certificate_counter"
    operationalCertificateIssueCounterFile <- OP.noteTempFile
      tempDir
      "delegate-op-cert.counter"
    operationalCertFile <- OP.noteTempFile tempDir "operational.cert"

    void . liftIO $ IO.copyFile originalOperationalCertificateIssueCounterFile
    ...
    OP.assertFileOccurences 1 "Next certificate issue number: 1"
      $ operationalCertificateIssueCounterFile
