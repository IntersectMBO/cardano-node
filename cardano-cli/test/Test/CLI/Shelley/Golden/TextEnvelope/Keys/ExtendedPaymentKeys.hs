...
golden_shelleyExtendedPaymentKeys = propertyOnce $ do

    -- Reference keys
  let referenceVerKey =
        "test/Test/golden/shelley/keys/extended_payment_keys/verification_key"
      referenceSignKey =
        "test/Test/golden/shelley/keys/extended_payment_keys/signing_key"
  ...
  let signingKeyType = textEnvelopeType (AsSigningKey AsPaymentExtendedKey)
      verificationKeyType =
        textEnvelopeType (AsVerificationKey AsPaymentExtendedKey)
