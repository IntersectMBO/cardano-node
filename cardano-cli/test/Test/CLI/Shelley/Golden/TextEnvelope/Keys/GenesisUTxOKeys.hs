...
golden_shelleyGenesisUTxOKeys = propertyOnce $ do

    -- Reference keys
  let referenceVerKey =
        "test/Test/golden/shelley/keys/genesis_utxo_keys/verification_key"
      referenceSignKey =
        "test/Test/golden/shelley/keys/genesis_utxo_keys/signing_key"
  ...
  let signingKeyType = textEnvelopeType (AsSigningKey AsGenesisUTxOKey)
      verificationKeyType =
        textEnvelopeType (AsVerificationKey AsGenesisUTxOKey)
