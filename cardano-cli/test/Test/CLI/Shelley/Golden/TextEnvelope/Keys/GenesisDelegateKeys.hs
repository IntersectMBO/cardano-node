...
golden_shelleyGenesisDelegateKeys = propertyOnce $ do

    -- Reference keys
  let
    referenceVerKey =
      "test/Test/golden/shelley/keys/genesis_delegate_keys/verification_key"
    referenceSignKey =
  ...
  let signingKeyType = textEnvelopeType (AsSigningKey AsGenesisDelegateKey)
      verificationKeyType =
        textEnvelopeType (AsVerificationKey AsGenesisDelegateKey)
      operationalCertCounterType =
        textEnvelopeType AsOperationalCertificateIssueCounter
