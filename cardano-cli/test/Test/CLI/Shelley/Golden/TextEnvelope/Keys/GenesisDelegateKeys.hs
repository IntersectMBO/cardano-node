...
golden_shelleyGenesisDelegateKeys = propertyOnce . moduleWorkspace "tmp" $ \tempDir -> do
  -- Reference keys
  referenceVerKey <- noteInputFile "test/Test/golden/shelley/keys/genesis_delegate_keys/verification_key"
  referenceSignKey <- noteInputFile "test/Test/golden/shelley/keys/genesis_delegate_keys/signing_key"
  referenceOpCertCounter <- noteInputFile "test/Test/golden/shelley/keys/genesis_delegate_keys/operational_certificate_counter"
