...
branch — `.#cardano-node-debug`, built from the haskell.nix `ghcDebug` project
variant (`nix/haskell.nix`) — is compiled with the flags it is supposed to
...
The `evidence/` files are per-branch build products; (re)generate them on this
branch with:

    nix derivation show \
      '.#legacyPackages.x86_64-linux.cardanoNodeProject.ghcDebug.hsPkgs.cardano-node.components.exes.cardano-node' \
      > ipe-verification/evidence/cardano-node.exe.ghcdebug.drv.json
    nix derivation show \
      '.#legacyPackages.x86_64-linux.cardanoNodeProject.ghcDebug.hsPkgs.ouroboros-consensus.components.library' \
      > ipe-verification/evidence/ouroboros-consensus.library.ghcdebug.drv.json
