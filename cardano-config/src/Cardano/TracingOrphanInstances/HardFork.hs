...
instance
  ( All (ToObject `Compose` WrapLedgerWarning) xs,
    All SingleEraBlock xs
  ) =>
  ToObject (HardForkLedgerWarning xs)
  where
...
instance
  ( All (ToObject `Compose` WrapLedgerUpdate) xs,
    All SingleEraBlock xs
  ) =>
  ToObject (HardForkLedgerUpdate xs)
  where
...
instance All (ToObject `Compose` WrapCannotLead) xs => ToObject (OneEraCannotLead xs) where
  toObject verb =
    hcollapse
      . hcmap
        (Proxy @(ToObject `Compose` WrapCannotLead))
        (K . toObject verb)
      . getOneEraCannotLead
...
instance
  All (ToObject `Compose` WrapChainIndepState) xs =>
  ToObject (PerEraChainIndepState xs)
  where
  toObject verb =
    mconcat -- Mash all the eras together, hope they do not clash
      . hcollapse
      . hcmap
        (Proxy @(ToObject `Compose` WrapChainIndepState))
        (K . toObject verb)
      . getPerEraChainIndepState

instance
  ToObject (ChainIndepState (BlockProtocol blk)) =>
  ToObject (WrapChainIndepState blk)
  where
