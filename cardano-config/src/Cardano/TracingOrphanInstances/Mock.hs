...
module Cardano.TracingOrphanInstances.Mock
  ()
where
...
import qualified Ouroboros.Consensus.Mock.Ledger
                                               as Mock
import qualified Ouroboros.Consensus.Mock.Protocol.Praos
                                               as Praos
...
instance ToObject (Praos.HotKey Praos.PraosMockCrypto) where
  toObject _verb (Praos.HotKey signKey) = mkObject
    [ "kind" .= String "HotKey"
    , "vkey"
      .= (hashVerKeyKES (deriveVerKeyKES signKey) :: Hash
             (Praos.PraosHash Praos.PraosMockCrypto)
...
instance StandardHash blk
 => ToObject (Mock.MockError blk) where
  toObject _verb (Mock.MockUtxoError e) =
    mkObject ["kind" .= String "MockUtxoError", "error" .= String (show e)]
  toObject _verb (Mock.MockInvalidHash expect act) = mkObject
    [ "kind" .= String "MockInvalidHash"
    , "expected" .= String (show expect)
    , "actual" .= String (show act)
    ]
  toObject _verb (Mock.MockExpired expiredSlot validatedSlot) = mkObject
    ["kind" .= String "MockExpired", "error" .= String (pack msg)]
   where
    msg =
      "transaction expired in slot "
        <> condense expiredSlot
        <> ", current slot "
        <> condense validatedSlot


instance ToObject (Praos.PraosValidationError c) where
  toObject _verb (Praos.PraosInvalidSlot expect act) = mkObject
    [ "kind" .= String "PraosInvalidSlot"
    , "expected" .= String (show expect)
    , "actual" .= String (show act)
    ]
  toObject _verb (Praos.PraosUnknownCoreId cid) = mkObject
    ["kind" .= String "PraosUnknownCoreId", "error" .= String (show cid)]
  toObject _verb (Praos.PraosInvalidSig str _ _ _) =
    mkObject ["kind" .= String "PraosInvalidSig", "error" .= String (pack str)]
  toObject _verb (Praos.PraosInvalidCert _vkvrf y nat _vrf) = mkObject
    [ "kind" .= String "PraosInvalidCert"
    , "y" .= String (show y)
    , "nat" .= String (show nat)
    ]
  toObject _verb (Praos.PraosInsufficientStake t y) = mkObject
