...
module Cardano.TracingOrphanInstances.Shelley
  ()
where
...
import qualified Ouroboros.Consensus.Ledger.SupportsMempool
                                               as SupportsMempool
...
import           Shelley.Spec.Ledger.TxData     ( TxId(..)
                                                , TxIn(..)
                                                , TxOut(..)
                                                , MIRPot(..)
                                                )
...
instance ToObject (HotKey TPraosStandardCrypto) where
  toObject _verb HotKey { hkStart, hkEnd, hkEvolution, hkKey } = mkObject
    [ "kind" .= String "HotKey"
    , "start" .= hkStart
    , "end" .= hkEnd
    , "evolution" .= hkEvolution
    , "vkey"
      .= (hashVerKeyKES (deriveVerKeyKES hkKey) :: Hash
...
instance ToObject (TPraosCannotLead c) where
  toObject _verb (TPraosCannotLeadUnusableKESKey TPraosUnusableKey { tpraosUnusableKeyStart, tpraosUnusableKeyEnd, tpraosUnusableKeyCurrent, tpraosUnusableWallClock })
    = mkObject
...
instance Crypto c =>  ToObject (ShelleyLedgerError c) where
  toObject verb (BBodyError (BlockTransitionError fs)) = mkObject
    ["kind" .= String "BBodyError", "failures" .= map (toObject verb) fs]
  toObject verb (TickError (TickTransitionError fs)) = mkObject
    ["kind" .= String "TickError", "failures" .= map (toObject verb) fs]

instance Crypto c => ToObject (ChainTransitionError c) where
  toObject verb (ChainTransitionError fs) = mkObject
    [ "kind" .= String "ChainTransitionError"
    , "failures" .= map (toObject verb) fs
    ]

instance Crypto c => ToObject (PredicateFailure (CHAIN c)) where
  toObject _verb (HeaderSizeTooLargeCHAIN hdrSz maxHdrSz) = mkObject
    [ "kind" .= String "HeaderSizeTooLarge"
    , "headerSize" .= hdrSz
    , "maxHeaderSize" .= maxHdrSz
    ]
  toObject _verb (BlockSizeTooLargeCHAIN blkSz maxBlkSz) = mkObject
    [ "kind" .= String "BlockSizeTooLarge"
    , "blockSize" .= blkSz
    , "maxBlockSize" .= maxBlkSz
    ]
  toObject _verb (ObsoleteNodeCHAIN currentPtcl supportedPtcl) = mkObject
...
instance Crypto c => ToObject (PrtlSeqFailure c) where
  ...
  toObject _verb (WrongBlockNoPrtclSeq lab currentBlockNo) = mkObject
...
instance Crypto c =>  ToObject (PredicateFailure (BBODY c)) where
  ...
  toObject _verb (InvalidBodyHashBBODY actualHash claimedHash) = mkObject
...
instance Crypto c => ToObject (PredicateFailure (UTXOW c)) where
  toObject _verb (InvalidWitnessesUTXOW wits) = mkObject
    [ "kind" .= String "InvalidWitnessesUTXOW"
    , "invalidWitnesses" .= map textShow wits
    ]
  toObject _verb (MissingVKeyWitnessesUTXOW (WitHashes wits)) = mkObject
    ["kind" .= String "MissingVKeyWitnessesUTXOW", "missingWitnesses" .= wits]
  toObject _verb (MissingScriptWitnessesUTXOW missingScripts) = mkObject
    [ "kind" .= String "MissingScriptWitnessesUTXOW"
    , "missingScripts" .= missingScripts
    ]
  toObject _verb (ScriptWitnessNotValidatingUTXOW failedScripts) = mkObject
    [ "kind" .= String "ScriptWitnessNotValidatingUTXOW"
    , "failedScripts" .= failedScripts
    ]
  toObject verb (UtxoFailure f) = toObject verb f
  toObject _verb (MIRInsufficientGenesisSigsUTXOW genesisSigs) = mkObject
    [ "kind" .= String "MIRInsufficientGenesisSigsUTXOW"
    , "genesisSigs" .= genesisSigs
    ]
  toObject _verb (MissingTxBodyMetaDataHash metaDataHash) = mkObject
    [ "kind" .= String "MissingTxBodyMetaDataHash"
    , "metaDataHash" .= metaDataHash
    ]
  toObject _verb (MissingTxMetaData txBodyMetaDataHash) = mkObject
...
instance Crypto c => ToObject (PredicateFailure (UTXO c)) where
  toObject _verb (BadInputsUTxO badInputs) = mkObject
    [ "kind" .= String "BadInputsUTxO"
    , "badInputs" .= badInputs
    , "error" .= renderBadInputsUTxOErr badInputs
    ]
  toObject _verb (ExpiredUTxO ttl slot) =
    mkObject ["kind" .= String "ExpiredUTxO", "ttl" .= ttl, "slot" .= slot]
  toObject _verb (MaxTxSizeUTxO txsize maxtxsize) = mkObject
    ["kind" .= String "MaxTxSizeUTxO", "size" .= txsize, "maxSize" .= maxtxsize]
  -- TODO: Add the minimum allowed UTxO value to OutputTooSmallUTxO
  toObject _verb (OutputTooSmallUTxO badOutputs) = mkObject
  ...
  toObject _verb (OutputBootAddrAttrsTooBig badOutputs) = mkObject
  ...
  toObject _verb (FeeTooSmallUTxO minfee txfee) = mkObject
    ["kind" .= String "FeeTooSmallUTxO", "minimum" .= minfee, "fee" .= txfee]
  toObject _verb (ValueNotConservedUTxO consumed produced) = mkObject
  ...
  toObject _verb (WrongNetwork network addrs) = mkObject
    ["kind" .= String "WrongNetwork", "network" .= network, "addrs" .= addrs]
  toObject _verb (WrongNetworkWithdrawal network addrs) = mkObject
...
instance ToObject (PredicateFailure (PPUP c)) where
  toObject _verb (NonGenesisUpdatePPUP proposalKeys genesisKeys) = mkObject
  ...
  toObject _verb (PVCannotFollowPPUP badPv) = mkObject
    ["kind" .= String "PVCannotFollowPPUP", "badProtocolVersion" .= badPv]


instance Crypto c => ToObject (PredicateFailure (DELEGS c)) where
  toObject _verb (DelegateeNotRegisteredDELEG targetPool) = mkObject
    ["kind" .= String "DelegateeNotRegisteredDELEG", "targetPool" .= targetPool]
...
instance ToObject (PredicateFailure (DELEG c)) where
  toObject _verb (StakeKeyAlreadyRegisteredDELEG alreadyRegistered) = mkObject
  ...
  toObject _verb (StakeKeyInRewardsDELEG alreadyRegistered) = mkObject
    [ "kind" .= String "StakeKeyInRewardsDELEG"
    , "credential" .= String (textShow alreadyRegistered)
    , "error" .= String "Staking credential registered in rewards map"
    ]
  toObject _verb (StakeKeyNotRegisteredDELEG notRegistered) = mkObject
  ...
  toObject _verb (StakeKeyNonZeroAccountBalanceDELEG remBalance) = mkObject
    [ "kind" .= String "StakeKeyNonZeroAccountBalanceDELEG"
    , "remainingBalance" .= remBalance
    ]
  toObject _verb (StakeDelegationImpossibleDELEG unregistered) = mkObject
  ...
  toObject _verb (GenesisKeyNotInpMappingDELEG (KeyHash genesisKeyHash)) =
    mkObject
      [ "kind" .= String "GenesisKeyNotInpMappingDELEG"
  ...
  toObject _verb (DuplicateGenesisVRFDELEG vrfKeyHash) = mkObject
    ["kind" .= String "DuplicateGenesisVRFDELEG", "keyHash" .= vrfKeyHash]
...
instance ToObject (PredicateFailure (POOL c)) where
  ...
  toObject _verb (StakePoolCostTooLowPOOL certCost protCost) = mkObject
...
-- Apparently this should never happen accoring to the shelley exec spec
  toObject _verb (WrongCertificateTypePOOL index) = case index of
    0 -> mkObject
...
instance ToObject (PredicateFailure (NEWEPOCH c)) where
  ...
  toObject _verb (CorruptRewardUpdate update) = mkObject
    ["kind" .= String "CorruptRewardUpdate", "update" .= String (show update)]
...
instance Crypto c => ToObject (PredicateFailure (OVERLAY c)) where
  toObject _verb (UnknownGenesisKeyOVERLAY (KeyHash genKeyHash)) = mkObject
  ...
  toObject _verb (VRFKeyUnknown (KeyHash kHash)) = mkObject
  ...
  toObject _verb (WrongGenesisColdKeyOVERLAY actual expected) = mkObject
    [ "kind" .= String "WrongGenesisColdKeyOVERLAY"
    , "actual" .= actual
    , "expected" .= expected
    ]
  toObject _verb (WrongGenesisVRFKeyOVERLAY issuer actual expected) = mkObject
...
txInToText (TxIn (TxId txidHash) ix) =
  hashToText txidHash <> Text.pack "#" <> Text.pack (show ix)

instance Crypto c => ToJSON (TxOut c) where
  toJSON (TxOut addr amount) =
    Aeson.object ["address" .= addr, "amount" .= amount]
