...
-- to diagnose.
newtype NodeHostAddress = NodeHostAddress {unNodeHostAddress :: Maybe IP}
  deriving newtype (Show)
  deriving (Eq, Ord)
...
instance HasKESMetricsData ByronBlock

instance HasKESMetricsData (SimpleBlock a b)

instance
  (HasKESMetricsData x, NoHardForks x) =>
  HasKESMetricsData (HardForkBlock '[x])
  where
  getKESMetricsData forgeState =
