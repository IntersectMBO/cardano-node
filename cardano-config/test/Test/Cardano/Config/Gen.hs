...
genSigningKey = do
  seed <- genSeed
    $ fromIntegral (seedSizeDSIGN (Proxy :: Proxy (DSIGN TPraosStandardCrypto)))
  return $ genKeyDSIGN seed

genNodeAddress :: Gen NodeAddress
genNodeAddress = NodeAddress <$> genNodeHostAddress <*> fmap
  fromIntegral
  (Gen.word16 $ Range.linear 100 20000)

genNodeHostAddress :: Gen NodeHostAddress
genNodeHostAddress = NodeHostAddress <$> Gen.choice
