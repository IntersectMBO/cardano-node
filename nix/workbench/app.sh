...
app() {
  ...
  case "$op" in
    ...
    compose )
      ...
      yq --argjson profileScenario "\"$profile_scenario\"" --argjson nodeImageName "\"$nodeImageName\"" --argjson nodeImageTag "\"$nodeImageTag\"" --yaml-output -f "$global_basedir"/backend/docker-compose.jq $nodespecs;;
