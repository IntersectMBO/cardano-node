...
let
  ...
  # Max number of '--tx-out' when splitting genesis funds.
  outs_per_transaction = 150;
  ...
  do
    ...
    then
      ...
      then
        ...
        tx_in="$(get_address_utxo_expected "''${nodeName}" "''${funds_addr}")"
        ${coreutils}/bin/echo "funds_from_to: Wait for funds:  $(${coreutils}/bin/date --rfc-3339=seconds)"
        wait_utxo_id "''${nodeName}" "''${funds_addr}" "''${tx_in}"
      ...
      # Build transaction.
      local tx_filename=../"''${nodeName}"/tx."''${tx_in}"
      ...
      # Submit transaction.
      # TODO: Command failed: transaction submit  Error: Error while submitting tx:
      #       ShelleyTxValidationError ShelleyBasedEraConway (ApplyTxError
      #       (ConwayUtxowFailure (UtxoFailure (MaxTxSizeUTxO 20699 16384)) :| []))
      ...
      # Store address next UTxO
        calculate_next_utxo                       \
          "''${tx_filename}.signed"               \
          "''${funds_addr}"                       \
      > ../"''${nodeName}"/"''${funds_addr}".utxo
      txOuts_array=()
      batch="$((batch + ${toString outs_per_transaction}))"
    fi
  done

  # All the waiting / batchs do not release the lock.
  release_socket_lock         \
    "''${nodeName}"           \
    "''${tx_filename}.signed" \
    "''${funds_addr}"
...
function create_node_drep_keys {
  ...
  # Function arguments.
  local nodeName=$1 # String for the key file name (not for the socket).
...
################################################################################
# Evenly distribute to all producer nodes the "utxo_*key" genesis funds.

# First send all funds evenly to all producer-assigned addresses and later the
# same to an addresses assigned to each producers' DReps (TODO in as many UTxO
# as the times it intends to vote ???).
################################################################################
function governance_funds_producers {
  ...
  # Send funds to each node (using DRep ID as 0 as logical separation).
  ${coreutils}/bin/echo "governance_funds_producers: Nodes splitting phase!"
...
################################################################################
# Evenly distribute to all all producer-assigned DReps.
# First send all funds evenly to all producer-assigned addresses and later the
# same to an addresses assigned to each producers' DReps (TODO in as many UTxO
# as the times it intends to vote ???).
################################################################################
function governance_funds_producer_dreps {
  ...
  ${coreutils}/bin/echo "governance_funds_producer_dreps: Nodes' assigned DRep(s) splitting phase!"
  ...
  wait_any_utxos "''${nodeName}" "''${producer_addr}" >/dev/null
...
function governance_create_constitution {
  ...
  ${cardano-cli}/bin/cardano-cli conway governance action create-constitution \
    ...
    --constitution-hash        "$(${coreutils}/bin/cat ../"''${nodeName}"/constitution.hash)" \
    --constitution-script-hash "$(${coreutils}/bin/cat ../"''${nodeName}"/guardrails-script.hash)" \
...
function governance_create_withdrawal {
  ...
  ${coreutils}/bin/echo "governance_create_withdrawal: ''${nodeName} - ''${drep_i}"
  ...
  local tx_filename=../"''${nodeName}"/create-withdrawal."''${nodeName}"."''${drep_i}"
  # Create action.
...
function governance_vote_proposal {
  ...
  do
    ...
    local proposal_flag="../''${nodeName}/''${txId}.''${actual_drep}.voted"
    if test -f "''${proposal_flag}"
    then
      break
    fi

    ${coreutils}/bin/echo "governance_vote_proposal: ''${proposal_id} - ''${nodeName} - ''${actual_drep}"
    ...
    local tx_filename=../"''${nodeName}"/vote."''${nodeName}"."''${actual_drep}"
...
function governance_vote_all {
  ...
  ${coreutils}/bin/echo "governance_vote_all: ''${nodeName}"
  ...
  for txId in ''${txIds[*]}
  do
    local proposal_flag="../''${nodeName}/''${txId}.voted"
    if ! test -f "''${proposal_flag}"
    then
