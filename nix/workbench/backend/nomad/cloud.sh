...
fetch-logs-nomadcloud-node() {
  ...
  ############################################################################
  msg "$(blue Fetching) $(yellow "\"healthcheck\"") run files from $(yellow "\"${public_ipv4}\"") ..."
    rsync -auv -e "${ssh_command}"                     \
      -f'- start.sh'                                   \
  ...
  then
    msg "$(blue Fetching) $(yellow "\"generator\"") run files from $(yellow "\"${public_ipv4}\"") ..."
      rsync -auv -e "${ssh_command}"                   \
  ...
  ############################################################################
  msg "$(blue Fetching) $(yellow "\"node\"") run files from $(yellow "\"${public_ipv4}\"") ..."
    rsync -auv -e "${ssh_command}"                         \
  ...
  ##########################################################################
  msg "$(blue Fetching) $(yellow "\"tracer\"") run files from $(yellow "\"${public_ipv4}\"") ..."
    rsync -auv -e "${ssh_command}"                \
  ...
  then
    msg "$(red Failed fetching from) $(yellow "\"${node}\" (\"${public_ipv4}\")")"
    read -p "Hit enter to continue ..."
  fi
}
