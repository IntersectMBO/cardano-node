...
function evict_lmdb() {
  ...
  for node in ''${nodes[*]}
  do
    evict_lmdb_deployed "''${node}" &
    jobs_array+=("$!")
  done
  wait "''${jobs_array[@]}"
}

function evict_lmdb_deployed() {
  local node=$1

  msg "Starting node: ''${node}"

  local node_lmdb_path
  # Can also use the entire directory: "${profile.node.ssd_directory}"
  node_lmdb_path="$(${jq}/bin/jq -r .LedgerDB.LiveTablesPath "../../''${node}/config.json")"
  msg "Using: ''${node_lmdb_path}"

  while true
  do
    # Does NOT exit with non-zero if file does not exist.
    ${vmtouch}/bin/vmtouch -e "''${node_lmdb_path}"/tables/data.mdb
