...
## Profile name list (LOCAL_PROFILES).
## Regenerate with `make profiles-regenerate`.
## -include: the name list arrives with the bundle data; before the first
## export the tree holds only reader code and make must still parse.
-include $(dir $(lastword $(MAKEFILE_LIST)))makefile.profiles.mk
...
list-profiles:
	@for d in nix/workbench/profiles/*/*/*/; do basename $$d; done | sort -u | jq --raw-input . | jq --slurp .
...
show-profile:
	...
	@f=$$(ls nix/workbench/profiles/*/*/${NAME}/profile/profile.json 2>/dev/null | head -n1); \
	 test -n "$$f" || { echo "no such profile: ${NAME}  (make ps lists them)"; exit 1; }; \
	 jq . "$$f"

## List profile names, one per line.
ps:
	@for d in nix/workbench/profiles/*/*/*/; do basename $$d; done | sort -u

.PHONY: list-profiles show-profile ps
...
##   make PROFILE=ci-test ERA=conway shell
...
## Cluster backend: supervisor only in the bundle; nomad runs need cardano-workbench.
BACKEND ?= supervisor
...
##   backend  -> supervisor (the only one in the bundle)
##
## Variant -> flags.
...
##       meant to turn it off for -nix and the old nomad variants, but it only ever
...
.PHONY: $(TARGETS_LOCAL_SUPERVISOR_BASE) $(TARGETS_LOCAL_SUPERVISOR_NIX) $(TARGETS_LOCAL_SUPERVISOR_PROF) $(TARGETS_LOCAL_SUPERVISOR_PROFNIX) $(TARGETS_LOCAL_SUPERVISOR_AUTO) $(TARGETS_LOCAL_SUPERVISOR_AUTONIX) $(TARGETS_LOCAL_SUPERVISOR_AUTOPROF) $(TARGETS_LOCAL_SUPERVISOR_AUTOPROFNIX) $(TARGETS_LOCAL_SUPERVISOR_AUTOSTAY)
