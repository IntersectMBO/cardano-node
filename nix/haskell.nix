...
let
  ...
  projectPackageNames = builtins.attrNames projectComponents;
...
haskell-nix.cabalProject' ({ pkgs
  ...
  '' + lib.optionalString pkgs.stdenv.hostPlatform.isWindows ''
    ...
    package plutus-tx-plugin
      flags: +use-ghc-stub
  '';
  # No materialization for Windows/MacOS yet (https://github.com/input-output-hk/nix-tools/issues/97):
  materialized = lib.mkIf (pkgs.stdenv.hostPlatform.config == "x86_64-unknown-linux-gnu")  ./materialized/x86_64-unknown-linux-gnu/plan-nix;
  shell = {
    ...
    [
      ...
      ({ pkgs, options, ... }: {
        ...
        packages = lib.mapAttrs
          (name: components: {
            # FIXME: this should be set in generated materialization:
            package.isProject = true;

            components.exes = lib.genAttrs components.exes (exe: {
              postInstall = ''
                ${setGitRev}
