...
main = do
  getArgs >>= \case
    ...
    _ -> testPlot >> putStrLn "please specify DB file or log manifest"
...
runDB dbName = do
  ...
  let
    ...
    res1   = reduce TxsInMempoolPerSlot bySlotOjbs
  ...
  let
    ...
    points1 = map (first toDouble) $ foldr (:) [] res1
