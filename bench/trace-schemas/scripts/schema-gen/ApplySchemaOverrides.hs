...
-- | Returns human-readable descriptions of operations that would delete or
-- replace existing fields when applying @patch@ to @target@.  An empty list
-- means the patch is non-destructive (only adds new keys or deep-merges
-- objects).
  ...
  where
    ...
    checkKey path tObj (k, pv) =
      ...
      in case (KM.lookup k tObj, pv) of
           (Nothing, _)          -> []   -- new key: not destructive
           (Just _, A.Null)      -> [keyPath <> ": field deletion"]
           (Just (A.Object tv), A.Object pv') ->
             goObj keyPath tv pv'        -- recurse: only destructive if inner keys are
