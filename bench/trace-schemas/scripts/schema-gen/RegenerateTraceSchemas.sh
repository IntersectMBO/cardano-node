...
nix develop -c cabal run cardano-node -- \
  trace-documentation \
...
nix develop -c bash -lc \
  'GHC_LIBDIR=$(ghc --print-libdir) runghc bench/trace-schemas/scripts/schema-gen/GhciSchemaGen.hs'

echo "[trace-schemas] Applying human overrides..."
nix develop -c bash -lc \
  "runghc -package-env - bench/trace-schemas/scripts/schema-gen/ApplySchemaOverrides.hs --verbose"

echo "[trace-schemas] Validating generated schemas..."
nix develop -c bash -lc \
  "runghc -package-env - bench/trace-schemas/scripts/schema-gen/ValidateTraceSchemas.hs"
