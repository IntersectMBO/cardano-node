...
import           Cardano.Timeseries.Elab.Typing (Binding (..), Context, Def (..), Defs,
                   Ty (Bool, Duration, Fun, Hole, InstantVector, RangeVector, Scalar, Timestamp),
                   TyPrec (Loose), instantiateExpr, prettyTy)
import qualified Cardano.Timeseries.Elab.Typing as Ty
import qualified Cardano.Timeseries.Elab.Typing as Types
import           Cardano.Timeseries.Elab.Unify (UnificationProblem (..), UnifyM)
import qualified Cardano.Timeseries.Elab.Unify as Unify
import           Cardano.Timeseries.Interp.BinaryArithmeticOp
import qualified Cardano.Timeseries.Interp.BinaryArithmeticOp as BinaryArithmeticOp
import           Cardano.Timeseries.Interp.BinaryRelation (BinaryRelation)
import qualified Cardano.Timeseries.Interp.BinaryRelation as BinaryRelation
...
solveCanonicalBinaryRelationElabProblem _ _ lhs Scalar rel rhs Scalar hole Bool = do
  ...
  pure $ Just ([], [])
solveCanonicalBinaryRelationElabProblem _ _ lhs Bool rel rhs Bool hole Bool = do
  modify $ updateDefs $
    instantiateExpr hole
      (BinaryRelation.embedBool rel lhs rhs)
  pure $ Just ([], [])
