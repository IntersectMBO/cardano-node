...
import           Cardano.Timeseries.Interp.BinaryArithmeticOp (BinaryArithmeticOp)
import qualified Cardano.Timeseries.Interp.BinaryArithmeticOp as BinaryArithmeticOp
import           Cardano.Timeseries.Interp.BinaryRelation (BinaryRelation, embedScalar,
...
import           Cardano.Timeseries.Interp.Expr as Expr
...
interp cfg store env (Expr.NotEqBool a b) now = do
  ...
  vb <- interp cfg store env b now >>= expectBoolean
  pure (fromBool (va /= vb))
interp cfg store env (Expr.LtBool a b) now = do
  va <- interp cfg store env a now >>= expectBoolean
  vb <- interp cfg store env b now >>= expectBoolean
  pure (fromBool (va < vb))
interp cfg store env (Expr.LteBool a b) now = do
  va <- interp cfg store env a now >>= expectBoolean
  vb <- interp cfg store env b now >>= expectBoolean
  pure (fromBool (va <= vb))
interp cfg store env (Expr.GtBool a b) now = do
  va <- interp cfg store env a now >>= expectBoolean
  vb <- interp cfg store env b now >>= expectBoolean
  pure (fromBool (va > vb))
interp cfg store env (Expr.GteBool a b) now = do
  va <- interp cfg store env a now >>= expectBoolean
  vb <- interp cfg store env b now >>= expectBoolean
  pure (fromBool (va >= vb))
