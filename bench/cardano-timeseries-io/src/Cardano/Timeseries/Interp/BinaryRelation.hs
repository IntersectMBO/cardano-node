module Cardano.Timeseries.Interp.BinaryRelation(
  BinaryRelation(..),
  embedScalar,
  embedBool,
...
embedBool :: BinaryRelation -> Expr -> Expr -> Expr
embedBool Eq    = EqBool
embedBool Lt    = LtBool
embedBool Lte   = LteBool
embedBool Gt    = GtBool
embedBool Gte   = GteBool
embedBool NotEq = NotEqBool
