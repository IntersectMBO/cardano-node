...
{-- Create a delta timeline with each parameters and cost change that happened
...
main = do
  ...
  -- This are the "epoch_param" and "cost_model" tables exported as JSON.
  ...
  TIO.putStrLn $ TE.decodeUtf8 $ Pretty.encodePretty' defConfig $ snd $ mapAccumL
    (\acc ep ->
      case acc of
