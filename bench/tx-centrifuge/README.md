...
- **Format**: standard cardano-cli `.skey` file. Any key generated by `cardano-cli ... key generate` works — there's no required suffix.
- **Per-workload derivation**: tx-centrifuge interprets the last 3 hex characters of the raw 32-byte secret as a hex offset (`base`, in `0..0xfff`). Workload 0's recycle key is the supplied key exactly; workload N's key uses suffix `(base + N) mod 4096`, sharing the first 61 hex chars. This yields up to 4096 distinct recycle keys/addresses, derived deterministically from one seed key.
...
tx-centrifuge does **not** read initial UTxOs from a config file. At every startup it derives **all 4096 addresses** in the 3-hex-suffix space (`000..fff`) from the seed key's 61-char prefix and issues a single `QueryUTxOByAddress` against the local node to find every spendable UTxO across those addresses. This makes restarts stateless — there is no on-disk state to corrupt, and any UTxOs at any of those addresses are picked up automatically, regardless of which suffix received them (including leftovers from a previous configuration, externally-funded addresses, or addresses derived under an old workload mapping).
...
- **Empty UTxO set behaviour**: if *no* UTxOs are found at any of the 4096 addresses, tx-centrifuge dies with a clear message including workload 0's bech32 address and a sample `cardano-cli` funding command. Partial funding is fine — the round-robin partition spreads the available UTxOs across active workloads.
- **Dust filtering**: at discovery, UTxOs whose value is at or below the maximum configured fee are flagged as dust and skipped (they cannot produce a valid change output). The startup summary reports per-address `(N dust skipped)`.
