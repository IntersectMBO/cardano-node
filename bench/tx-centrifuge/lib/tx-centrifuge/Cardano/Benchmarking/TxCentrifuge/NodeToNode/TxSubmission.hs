...
toEntry tx =
  let !genTx   = Block.toGenTx tx
      ...
      -- Wire size (CBOR-encoded). How the announced size flows:
      -- 1. Network decides whether to request the tx, checking the announced
      --    size against in-flight budgets (txsSizeInflightPerPeer /
      --    maxTxsSizeInflight in TxSubmission.Inbound.V2.Decision).
      -- 2. Transaction is downloaded; the network layer validates that the
      --    actual wire size matches the announced size within 32 bytes
      --    (const_MAX_TX_SIZE_DISCREPANCY in .TxSubmission.Inbound.V2.State).
      -- 3. Network calls addTxs (Consensus.Mempool.Update) to submit the tx to
      --    the mempool. The mempool measures the tx using its own size
      --    (sizeTxF + perTxOverhead in Consensus.Shelley.Ledger.Mempool),
      --    which is smaller (excludes the CBOR wrapper). The announced size is
      --    not used at this stage.
      -- 4. When the mempool is full, addTxs blocks the consumer thread. The tx
      --    stays counted as in-flight in the network layer.
      -- 5. Blocked txs accumulate, saturating maxTxsSizeInflight, which stops
      --    the network layer from requesting more txs from peers.
...
entryToIdSize :: UnAckedEntry
              -> (Mempool.GenTxId Block.CardanoBlock, Net.SizeInBytes)
...
-- mandatory tx, and the non-blocking action drains the rest up to the requested
-- count, capped by @maxBatchSize@.
txSubmissionClient
...
requestTxIds
  ...
  traceWith tracer $
    Tracing.RequestTxIds
      ...
      -- How many the node is ACKing.
      (fromIntegral ackNum)
      -- How many new TxIds it wants.
      (fromIntegral reqNum)
  -- Pull txs from the callbacks, capped by maxBatchSize.
  -------------------------------------------------------
  newTxs <- do
    let !effectiveReq
              | maxBatchSize == 0 = fromIntegral reqNum
              | otherwise         = min
                                      (fromIntegral reqNum)
                                      (fromIntegral maxBatchSize :: Int)
    case blocking of
      TxSub.SingBlocking -> do
        -- Block for exactly one tx (protocol minimum), then remaining up to
        -- effectiveReq-1 without blocking.
        tx1  <- blockingFetch
  ...
  traceWith tracer $
    Tracing.ReplyTxIds
      targetName
      -- How many the node was ACKing.
      (fromIntegral ackNum)
      -- How many new TxIds it wanted.
      (fromIntegral reqNum)
      ...
      (map
        (\entry ->
          ( -- Tx ID.
            Block.fromGenTxId . uaeGenTxId $ entry
            -- Tx size.
          , fromEnum $ uaeSize entry
          )
        )
        newEntries
      )
