...
data BuilderTrace
  = -- | A new transaction was built.
    ...
    -- | Funds were recycled back to the workload's input queue.
    --
    -- * 'String': builder name (the workload name, see 'Runtime.builderName').
    -- * 'Api.TxId': transaction's ID that triggered the recycling (see below).
    -- * 'Bool': @True@ if this is an orphan recycle (original inputs returned
    --   after a rollback), @False@ for a normal confirmation or immediate
...
-- | Build a 'BuilderRecycle' trace.
mkBuilderRecycle :: String      -- ^ Builder name.
                 -> Api.TxId    -- ^ Transaction ID.
                 -> Bool        -- ^ @True@ if orphan recycle.
...
instance Logging.LogFormatting BuilderTrace where
  ...
  forMachine dtal (BuilderRecycle name txId isOrphan funds) = mconcat
    [ "builder" .= name
    , "txId"    .= String (Api.serialiseToRawBytesHexText txId)
    , "orphan"  .= isOrphan
  ...
  forHuman (BuilderRecycle name txId isOrphan funds) =
