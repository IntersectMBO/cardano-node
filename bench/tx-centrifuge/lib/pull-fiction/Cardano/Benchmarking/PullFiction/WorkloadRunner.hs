...
--    same way.
--
-- Both fetch functions write the @[input]@ component of the payload tuple back
-- to the workload's input queue after each fetch. For 'Raw.RecycleOnPull' the
-- builder pairs outputs with the payload so the fetch recycles them; for
-- 'Raw.RecycleOnBuild' and 'Raw.RecycleOnConfirm' the list is empty (recycling
-- happens elsewhere) so the write is a no-op. The callback must not write to
-- the input queue itself (doing so would duplicate inputs). Its only
-- responsibilities are delivering the payload and any application-level
-- bookkeeping.
--
