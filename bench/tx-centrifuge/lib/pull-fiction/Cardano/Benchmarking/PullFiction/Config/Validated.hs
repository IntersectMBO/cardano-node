...
-- config specifies one. 0 means unlimited (use whatever the node requests).
...
data Target = Target
  ...
  , rateLimitSource :: !(Maybe RateLimitSource)
    ...
    -- 0 means unlimited, use whatever the node requests.
    -- target value > workload value > top-level value > default (0, unlimited).
  ...
  resolvedBuilder <- do
    case Raw.maybeBuilder rawWorkload of
      Just parsedBuilder -> do
        -- Workload-level builder is used (top-level was already ruled out above).
        pure parsedBuilder
      Nothing -> do
...
validateRateLimit rl@(Raw.TokenBucket rawTps) = do
  ...
  -- Guard against extremely small TPS values: 1e9 / tps can overflow to
  -- Infinity, and `round Infinity :: Integer` throws at runtime.
  when (isInfinite (1_000_000_000 / rawTps)) $
    Left "RateLimit: tps is too small (1e9 / tps overflows)"
  -- Guard against extremely large TPS values: round(1e9 / tps) can reach 0,
  -- which silently disables the rate limiter.
  when (round (1_000_000_000 / rawTps) == (0 :: Integer)) $
