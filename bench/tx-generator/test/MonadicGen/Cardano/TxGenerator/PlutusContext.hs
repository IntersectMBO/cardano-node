...
{-
Module      : MonadicGen.Cardano.TxGenerator.PlutusContext
...
import           MonadicGen.Cardano.TxGenerator.Setup.Plutus (preExecutePlutusScript)
import           MonadicGen.Cardano.TxGenerator.Types


-- | This collects information describing the budget. It's only
-- directly referenced in "MonadicGen.Cardano.Benchmarking.Script.Env" to supply
-- accessors as the state in an `MonadicGen.Cardano.Benchmarking.Script.ActionM`
-- (RWST) monad and "MonadicGen.Cardano.Benchmarking.Script.Core" where the only
