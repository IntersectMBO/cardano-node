...
{-|
Module      : MonadicGen.Cardano.Benchmarking.Script.Types
...
-- 'MonadicGen.Cardano.Benchmarking.Script.ActionM' using 'Env' as the
...
-- an 'MonadicGen.Cardano.Benchmarking.Env.Error' as the exception.
data Action where
  ...
  -- 'MonadicGen.Cardano.TxGenerator.FundQueue.FundQueue' in it.
  ...
  -- 'MonadicGen.Cardano.Benchmarking.Wallet.walletRefInsertFund' which in turn
  -- is just 'Control.Concurrent.modifyMVar' around
  -- 'MonadicGen.Cardano.TxGenerator.FundQueue.insert'.
  AddFund            :: !AnyCardanoEra -> !String -> !TxIn -> !Lovelace -> !String -> Action
  -- | 'WaitBenchmark' signifies a 'Control.Concurrent.Async.waitCatch'
  -- on the 'MonadicGen.Cardano.Benchmarking.GeneratorTx.AsyncBenchmarkControl'
  -- associated with the ID and also folds tracers into the completion.
  WaitBenchmark      :: !String -> Action
  -- | 'Submit' mostly wraps
  -- 'MonadicGen.Cardano.Benchamrking.Script.Core.benchmarkTxStream'
  -- which in turn wraps
  -- 'MonadicGen.Cardano.Benchmarking.GeneratorTx.walletBenchmark' which
  -- in turn wraps
  -- 'MonadicGen.Cardano.Benchmarking.GeneratorTx.SubmissionClient.txSubmissionClient'
  ...
  -- 'MonadicGen.Cardano.Benchmarking.GeneratorTx.AsyncBenchmarkControl' type,
  ...
  -- the @protoParams@ field of 'MonadicGen.Cardano.Benchmarking.Script.Env.Env'.
  SetProtocolParameters :: ProtocolParametersSource -> Action
  -- | 'LogMsg' logs its message calling
  -- 'MonadicGen.Cardano.Benchmarking.GeneratorTx.traceDebug' i.e. via the tracer.
  LogMsg             :: !Text -> Action
  deriving (Show, Eq)
deriving instance Generic Action

-- | 'Generator' is interpreted by
-- 'MonadicGen.Cardano.Bencmarking.Script.Core.evalGenerator' as a series of
-- transactions, albeit in the form of precursors to UTxO's.
data Generator where
  -- | 'SecureGenesis' gets funds from a genesis via
  -- 'MonadicGen.Cardano.TxGenerator.Genesis.genesisSecureInitialFundForKey'.
  ...
  -- a case in 'MonadicGen.Cardano.Benchmarking.Script.Core.evalGenerator' and
  ...
  -- 'MonadicGen.Cardano.Benchmarking.Script.ActionM', then a difficult-to-understand
  ...
  -- its argument, through 'MonadicGen.Cardano.Benchmarking.Script.Core.evalGenerator'
