...
data FillDefaults = FillDefaults | NoFillDefaults
  deriving (Show, Eq)

data ForceInfra = ForceInfra | NoForceInfra
  deriving (Show, Eq)
...
discoverTestnetConfig TestnetConfig{tcDir, tcMergeFlags} userConfig = do
  ...
  if not dirExists
    then die $ "Testnet directory does not exist: " ++ tcDir
    else pure ()
...
validateFileExists path description = do
  ...
  if not exists
    then die $ "Required " ++ description ++ " file not found: " ++ path
    else pure ()
