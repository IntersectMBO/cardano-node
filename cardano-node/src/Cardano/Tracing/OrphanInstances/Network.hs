...
instance (ToObject peer, Show txid, Show tx)
     => Transformable Text IO (TraceLabelPeer peer (NtN.TraceSendRecv (TxSubmission2 txid tx))) where
  trTransformer = trStructured

instance (ToObject peer, Show txid, Show tx)
     => Transformable Text IO (TraceLabelPeer peer (TraceTxSubmissionOutbound txid tx)) where
  trTransformer = trStructured
