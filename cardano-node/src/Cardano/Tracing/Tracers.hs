...
instance ElidingTracer (WithSeverity (ChainDB.TraceEvent blk)) where
  ...
  doelide (WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.SwitchedToAFork _ _ _ _))) = False
  ...
  conteliding tverb tr ev@(WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.AddedToCurrentChain _ _pt _ _))) (_old, oldt) = do
    ...
    Tracer $ \(WithSeverity _ ev') ->
      case ev' of
          (ChainDB.TraceAddBlockEvent ev) -> case ev of
            ChainDB.SwitchedToAFork  _  newTipInfo _ newChain ->
              traceChainInformation tr (chainInformation newTipInfo newChain)
            ChainDB.AddedToCurrentChain _ newTipInfo _ newChain ->
              traceChainInformation tr (chainInformation newTipInfo newChain)
            _ -> pure ()
          _ -> pure ()
