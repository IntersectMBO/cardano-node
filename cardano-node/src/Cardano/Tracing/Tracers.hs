...
mkTracers
  ...
  -> Maybe EKGDirect
  -> NetworkP2PMode p2p
  -> IO (Tracers (ConnectionId RemoteAddress) (ConnectionId LocalAddress) blk p2p CardanoPeerSelectionState CardanoDebugPeerSelectionState PeerTrustable (CardanoPublicRootPeers RemoteAddress) (CardanoPeerSelectionView RemoteAddress) IO)
...
tracePeerSelectionCountersMetrics
    ...
    -> Maybe EKGDirect
    -> Tracer IO (PeerSelectionCounters (CardanoPeerSelectionView addr))
...
tracePeerSelectionCountersMetrics (OnOff True)  (Just ekgDirect) = pscTracer
  where
    pscTracer :: Tracer IO (PeerSelectionCounters (CardanoPeerSelectionView addr))
    pscTracer = Tracer $ \psc -> do
      ...
      sendEKGDirectInt ekgDirect "cardano.node.metrics.peerSelection.KnownBootstrapPeers" (snd $ viewKnownBootstrapPeers extraCounters)
      sendEKGDirectInt ekgDirect "cardano.node.metrics.peerSelection.ColdBootstrapPeersPromotions" (snd $ viewColdBootstrapPeersPromotions extraCounters)
      sendEKGDirectInt ekgDirect "cardano.node.metrics.peerSelection.EstablishedBootstrapPeers" (snd $ viewEstablishedBootstrapPeers extraCounters)
      sendEKGDirectInt ekgDirect "cardano.node.metrics.peerSelection.WarmBootstrapPeersDemotions" (snd $ viewWarmBootstrapPeersDemotions extraCounters)
      sendEKGDirectInt ekgDirect "cardano.node.metrics.peerSelection.WarmBootstrapPeersPromotions" (snd $ viewWarmBootstrapPeersPromotions extraCounters)
      sendEKGDirectInt ekgDirect "cardano.node.metrics.peerSelection.ActiveBootstrapPeers" (snd $ viewActiveBootstrapPeers extraCounters)
      sendEKGDirectInt ekgDirect "cardano.node.metrics.peerSelection.ActiveBootstrapPeersDemotions" (snd $ viewActiveBootstrapPeersDemotions extraCounters)
