...
import           Cardano.BM.Data.Tracer         ( WithSeverity(..)
                                                , annotateSeverity
                                                )
...
import qualified Ouroboros.Consensus.Node.Run  as Consensus
                                                ( RunNode )
import qualified Ouroboros.Consensus.Node.Tracers
                                               as Consensus
import qualified Ouroboros.Consensus.Network.NodeToClient
                                               as NodeToClient
import qualified Ouroboros.Consensus.Network.NodeToNode
                                               as NodeToNode
...
import           Ouroboros.Consensus.Util.Condense
                                                ( Condense ) -- This should be removed
...
import qualified Ouroboros.Network.NodeToClient
                                               as NtC
...
nullTracers = Tracers { chainDBTracer = nullTracer
...
instance ElidingTracer (WithSeverity (ChainDB.TraceEvent blk)) where
  ...
  isEquivalent (WithSeverity s1 (ChainDB.TraceLedgerReplayEvent ev1)) (WithSeverity s2 (ChainDB.TraceLedgerReplayEvent ev2))
    = s1 == s2 && indexReplType ev1 == indexReplType ev2
  isEquivalent (WithSeverity s1 (ChainDB.TraceGCEvent ev1)) (WithSeverity s2 (ChainDB.TraceGCEvent ev2))
    = s1 == s2 && indexGCType ev1 == indexGCType ev2
  isEquivalent (WithSeverity _s1 (ChainDB.TraceAddBlockEvent _)) (WithSeverity _s2 (ChainDB.TraceAddBlockEvent _))
    = True
  isEquivalent (WithSeverity _s1 (ChainDB.TraceGCEvent _ev1)) (WithSeverity _s2 (ChainDB.TraceAddBlockEvent _))
    = True
  isEquivalent (WithSeverity _s1 (ChainDB.TraceAddBlockEvent _)) (WithSeverity _s2 (ChainDB.TraceGCEvent _ev2))
    = True
  isEquivalent (WithSeverity _s1 (ChainDB.TraceGCEvent _ev1)) (WithSeverity _s2 (ChainDB.TraceCopyToImmDBEvent _))
    = True
  isEquivalent (WithSeverity _s1 (ChainDB.TraceCopyToImmDBEvent _)) (WithSeverity _s2 (ChainDB.TraceGCEvent _ev2))
    = True
  isEquivalent (WithSeverity _s1 (ChainDB.TraceCopyToImmDBEvent _)) (WithSeverity _s2 (ChainDB.TraceAddBlockEvent _))
    = True
  isEquivalent (WithSeverity _s1 (ChainDB.TraceAddBlockEvent _)) (WithSeverity _s2 (ChainDB.TraceCopyToImmDBEvent _))
    = True
  isEquivalent (WithSeverity _s1 (ChainDB.TraceCopyToImmDBEvent _)) (WithSeverity _s2 (ChainDB.TraceCopyToImmDBEvent _))
    = True
  ...
  doelide (WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.IgnoreBlockOlderThanK _)))
    = False
  doelide (WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.IgnoreInvalidBlock _ _)))
    = False
  doelide (WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.BlockInTheFuture _ _)))
    = False
  doelide (WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.StoreButDontChange _)))
    = False
  doelide (WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.TrySwitchToAFork _ _)))
    = False
  doelide (WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.SwitchedToAFork{})))
    = False
  doelide (WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.AddBlockValidation (ChainDB.InvalidBlock _ _))))
    = False
  doelide (WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.AddBlockValidation (ChainDB.InvalidCandidate _))))
    = False
  doelide (WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.AddBlockValidation ChainDB.CandidateContainsFutureBlocksExceedingClockSkew{})))
    = False
  doelide (WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.AddedToCurrentChain events _ _ _)))
    = null events
  ...
  conteliding tverb tr ev@(WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.AddedToCurrentChain{}))) (_old, oldt)
    = do
  ...
  conteliding _tverb _tr ev@(WithSeverity _ (ChainDB.TraceGCEvent _)) (_old, count)
    = return (Just ev, count)
  conteliding _tverb tr ev@(WithSeverity _ (ChainDB.TraceLedgerReplayEvent (LedgerDB.ReplayedBlock pt replayTo))) (_old, count)
    = do
      let slotno = toInteger $ unSlotNo (realPointSlot pt)
          ...
          progress :: Double =
            (fromInteger slotno * 100.0) / fromInteger (max slotno endslot)
      when (count > 0 && (slotno - startslot) `mod` 1000 == 0) $ do  -- report every 1000th slot
        ...
        traceNamedObject
          tr
          ( meta
          , LogValue "block replay progress (%)"
                     (PureD $ (fromInteger $ round (progress * 10.0)) / 10.0)
          )
...
instance (StandardHash header, Eq peer) => ElidingTracer
  ...
  -- equivalent by type and severity
  isEquivalent (WithSeverity s1 _peers1) (WithSeverity s2 _peers2) = s1 == s2
  -- the types to be elided
  doelide (WithSeverity _ peers) =
    let
      checkDecision :: TraceLabelPeer peer (Either FetchDecline result) -> Bool
      checkDecision (TraceLabelPeer _peer (Left FetchDeclineChainNotPlausible))
        = True
      checkDecision (TraceLabelPeer _peer (Left (FetchDeclineConcurrencyLimit _ _)))
        = True
      checkDecision (TraceLabelPeer _peer (Left (FetchDeclinePeerBusy _ _ _)))
        = True
      checkDecision _ = False
    in
      any checkDecision peers
  conteliding _tverb _tr _ (Nothing, _count) = return (Nothing, 0)
  conteliding tverb tr ev (_old, count) = do
    when (count > 0 && count `mod` 1000 == 0)
...
mkTracers tOpts@(TracingOn trSel) tr nodeKern = do
  ...
  pure Tracers
    { chainDBTracer = tracerOnOff' (traceChainDB trSel)
                      $ annotateSeverity
                      . teeTraceChainTip tOpts elidedChainDB
    ...
    , ipSubscriptionTracer = tracerOnOff (traceIpSubscription trSel)
                                         verb
                                         "IpSubscription"
                                         tr
    , dnsSubscriptionTracer = tracerOnOff (traceDnsSubscription trSel)
                                          verb
                                          "DnsSubscription"
                                          tr
    , dnsResolverTracer = tracerOnOff (traceDnsResolver trSel)
                                      verb
                                      "DnsResolver"
                                      tr
    , errorPolicyTracer = tracerOnOff (traceErrorPolicy trSel)
                                      verb
                                      "ErrorPolicy"
                                      tr
    , localErrorPolicyTracer = tracerOnOff (traceLocalErrorPolicy trSel)
                                           verb
                                           "LocalErrorPolicy"
                                           tr
    , acceptPolicyTracer = tracerOnOff (traceAcceptPolicy trSel)
                                       verb
                                       "AcceptPolicy"
                                       tr
    , muxTracer = tracerOnOff (traceMux trSel) verb "Mux" tr
    , handshakeTracer = tracerOnOff (traceHandshake trSel) verb "Handshake" tr
    , localHandshakeTracer = tracerOnOff (traceLocalHandshake trSel)
                                         verb
                                         "LocalHandshake"
                                         tr
    }
...
mkTracers TracingOff _ _ = pure Tracers
...
teeTraceChainTip (TracingOn trSel) elided tr = Tracer $ \ev -> do
...
traceChainInformation tr ChainInformation { slots, blocks, density, epoch, slotInEpoch }
  = do
  -- TODO this is executed each time the chain changes. How cheap is it?
    ...
    let tr' = appendName "metrics" tr
        ...
        traceI msg i =
          traceNamedObject tr' (meta, LogValue msg (PureI (fromIntegral i)))
...
teeTraceChainTip'
  ...
  => Trace IO Text
  -> Tracer IO (WithSeverity (ChainDB.TraceEvent blk))
teeTraceChainTip' tr = Tracer $ \(WithSeverity _ ev') -> case ev' of
...
mkConsensusTracers trSel verb tr nodeKern bcCounters = do
  ...
  pure Consensus.Tracers
    { Consensus.chainSyncClientTracer = tracerOnOff
                                          (traceChainSyncClient trSel)
                                          verb
                                          "ChainSyncClient"
                                          tr
    , Consensus.chainSyncServerHeaderTracer = tracerOnOff
                                                ...
                                                tr
    , Consensus.chainSyncServerBlockTracer = tracerOnOff
                                               (traceChainSyncHeaderServer trSel
                                               )
                                               verb
                                               "ChainSyncBlockServer"
                                               tr
    , Consensus.blockFetchDecisionTracer =
      tracerOnOff' (traceBlockFetchDecisions trSel)
      $ annotateSeverity
      $ teeTraceBlockFetchDecision verb elidedFetchDecision
      $ appendName "BlockFetchDecision" tr
    , Consensus.blockFetchClientTracer = tracerOnOff
                                           (traceBlockFetchClient trSel)
                                           verb
                                           "BlockFetchClient"
                                           tr
    , Consensus.blockFetchServerTracer = tracerOnOff
                                           (traceBlockFetchServer trSel)
                                           verb
                                           "BlockFetchServer"
                                           tr
    , Consensus.forgeStateTracer = tracerOnOff' (traceForgeState trSel)
                                     $ forgeStateTracer trSel tr
    , Consensus.txInboundTracer = tracerOnOff (traceTxInbound trSel)
                                              verb
                                              "TxInbound"
                                              tr
    , Consensus.txOutboundTracer = tracerOnOff (traceTxOutbound trSel)
                                               verb
                                               "TxOutbound"
                                               tr
    , Consensus.localTxSubmissionServerTracer = tracerOnOff
                                                  ...
                                                  tr
    , Consensus.mempoolTracer = tracerOnOff' (traceMempool trSel)
                                  $ mempoolTracer trSel tr bcCounters
    , Consensus.forgeTracer = tracerOnOff' (traceForge trSel) $ Tracer $ \ev ->
      do
        traceWith (forgeTracer verb tr forgeTracers nodeKern bcCounters) ev
        traceWith
          (blockForgeOutcomeExtractor $ toLogObject' verb $ appendName
            "ForgeTime"
            tr
          )
          ev
    , Consensus.blockchainTimeTracer =
      tracerOnOff' (traceBlockchainTime trSel) $ Tracer $ \ev ->
...
teeForge ft nodeKern tverb tr = Tracer $ \ev@(WithSeverity sev event) -> do
  flip traceWith ev $ fanning $ \(WithSeverity _ e) -> case e of
...
teeForge'
  :: Trace IO Text -> Tracer IO (WithSeverity (Consensus.TraceForgeEvent blk))
teeForge' tr = Tracer $ \(WithSeverity _ ev) -> do
  meta <- mkLOMeta Critical Confidential
  traceNamedObject (appendName "metrics" tr) . (meta, ) $ case ev of
...
forgeTracer verb tr forgeTracers nodeKern bcCounters = Tracer $ \ev -> do
  ...
  -- Consensus tracer
  traceWith
    (annotateSeverity $ teeForge forgeTracers nodeKern verb $ appendName
      "Forge"
      tr
    )
    ev
...
notifyBlockForging bcCounters tr = Tracer $ \case
  Consensus.TraceForgedBlock{} -> do
    updatedBlocksForged <- atomicModifyIORef'
      bcCounters
      (\cnts ->
        let nc = bcBlocksForgedNum cnts + 1
...
notifySlotsMissedIfNeeded bcCounters tr = Tracer $ \case
  Consensus.TraceNodeIsLeader{} -> do
    updatedNodeIsLeaderNum <- atomicModifyIORef'
      bcCounters
      (\cnts ->
        let nc = bcNodeIsLeaderNum cnts + 1
  ...
  Consensus.TraceNodeNotLeader{} -> do
    ...
    if numberOfMissedSlots > 0
      then do
        ...
        updatesSlotsMissed <- atomicModifyIORef'
          bcCounters
          (\cnts ->
            let nc = bcSlotsMissedNum cnts + numberOfMissedSlots
...
notifyTxsProcessed bcCounters tr = Tracer $ \case
  ...
  TraceMempoolRemoveTxs txs _ -> do
    ...
    updatedTxProcessed <- atomicModifyIORef'
      bcCounters
      (\cnts ->
        let nc = bcTxsProcessedNum cnts + fromIntegral (length txs)
...
mempoolMetricsTraceTransformer
  :: Trace IO a -> Tracer IO (TraceEventMempool blk)
mempoolMetricsTraceTransformer tr = Tracer $ \mempoolEvent -> do
  let
    tr' = appendName "metrics" tr
...
forgeStateMetricsTraceTransformer tr = Tracer $ \forgeState -> do
  case getKESMetricsData forgeState of
    ...
    TPraosKESMetricsData kesPeriodOfKey (MaxKESEvolutions maxKesEvos) (OperationalCertStartKESPeriod oCertStartKesPeriod)
      -> do
        let
          metricsTr = appendName "metrics" tr
          ...
          logValues =
            ...
            , LogValue "currentKESPeriod" $ PureI $ fromIntegral
              currentKesPeriod
            , LogValue "remainingKESPeriods" $ PureI $ fromIntegral
              kesPeriodsUntilExpiry
            ]
        ...
        when (kesPeriodsUntilExpiry <= 7) $ traceWith
          tr
          ( mempty
          , LogObject
            mempty
...
forgeStateTracer
  :: forall blk
   . (HasKESMetricsData blk, Show (ForgeState blk))
  => TraceSelection
  -> Trace IO Text
  -> Tracer IO (ForgeState blk)
...
nodeToClientTracers'
  ...
  -> NodeToClient.Tracers'
       ...
       DeserialiseFailure
       (Tracer IO)
nodeToClientTracers' trSel verb tr = NodeToClient.Tracers
  { NodeToClient.tChainSyncTracer = tracerOnOff
                                      (traceLocalChainSyncProtocol trSel)
                                      verb
                                      "LocalChainSyncProtocol"
                                      tr
  , NodeToClient.tTxSubmissionTracer = tracerOnOff
                                         (traceLocalTxSubmissionProtocol trSel)
                                         verb
                                         "LocalTxSubmissionProtocol"
                                         tr
  , NodeToClient.tStateQueryTracer = tracerOnOff
                                       (traceLocalStateQueryProtocol trSel)
                                       verb
...
nodeToNodeTracers'
  :: (Consensus.RunNode blk, Condense (TxId (GenTx blk)), HasTxs blk, Show peer)
  => TraceSelection
  -> TracingVerbosity
  -> Trace IO Text
  -> NodeToNode.Tracers' peer blk DeserialiseFailure (Tracer IO)
nodeToNodeTracers' trSel verb tr = NodeToNode.Tracers
  { NodeToNode.tChainSyncTracer = tracerOnOff (traceChainSyncProtocol trSel)
                                              verb
                                              "ChainSyncProtocol"
                                              tr
  , NodeToNode.tChainSyncSerialisedTracer = showOnOff
                                              (traceChainSyncProtocol trSel)
                                              "ChainSyncProtocolSerialised"
                                              tr
  , NodeToNode.tBlockFetchTracer = tracerOnOff (traceBlockFetchProtocol trSel)
                                               verb
                                               "BlockFetchProtocol"
                                               tr
  , NodeToNode.tBlockFetchSerialisedTracer = showOnOff
    (traceBlockFetchProtocolSerialised trSel)
    "BlockFetchProtocolSerialised"
    tr
  , NodeToNode.tTxSubmissionTracer = tracerOnOff
                                       (traceTxSubmissionProtocol trSel)
                                       verb
...
teeTraceBlockFetchDecision verb eliding tr = Tracer $ \ev -> do
...
teeTraceBlockFetchDecision' tr = Tracer $ \(WithSeverity _ peers) -> do
  meta <- mkLOMeta Info Confidential
  let tr' = appendName "peers" tr
  traceNamedObject
    tr'
    (meta, LogValue "connectedPeers" . PureI $ fromIntegral $ length peers)
...
tracerOnOff True verb name trcer =
  annotateSeverity $ toLogObject' verb $ appendName name trcer
...
showOnOff True name trcer =
  annotateSeverity $ showTracing $ withName name trcer
