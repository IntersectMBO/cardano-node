...
instance ToObject (MeasureTxs blk) where
  ...
  toObject _verb (MeasureTxsTimeStop slotNo _blk _txs (Time time)) = mkObject
...
measureTxsEnd tracer = measureTxsEndInter $ toLogObject tracer
 where
  ...
  measureTxsEndInter tracer' = Tracer $ \case
    TraceAdoptedBlock slotNo blk txs -> traceWith tracer' =<< measureTxsEvent
     where
...
instance (Monad m, HasTxId (GenTx blk)) => Outcome m (MeasureTxs blk) where
  ...
  captureObservableValue (MeasureTxsTimeStart tx _ _ time) = pure [(tx, time)]
  ...
  computeOutcomeMetric _ xs ys =
    pure . computeFinalValues $ computeIntermediateValues xsTxId ysTxId
   where
    ...
    computeFinalValues intermediateValues = map
      (\((blk, timeStart), (_, timeEnd)) -> (blk, diffTime timeEnd timeStart))
      intermediateValues
...
instance (Monad m, MonadTime m) => Outcome m (TraceForgeEvent blk) where
  ...
  type OutcomeMetric (TraceForgeEvent blk) = Maybe
  ...
  computeOutcomeMetric _ (startSlot, absTimeStart, _) (stopSlot, absTimeStop, mempoolSize)
    | startSlot == stopSlot
    = pure $ Just (startSlot, (diffTime absTimeStop absTimeStart), mempoolSize)
    | otherwise
    = pure Nothing
