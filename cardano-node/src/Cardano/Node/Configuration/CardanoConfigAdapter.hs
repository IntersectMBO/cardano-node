 ...
 where
  ...
  byronConfig =
    NodeByronProtocolConfiguration
      ...
      , -- cardano-config does not model the Byron software (block) version: its
        -- @LastKnownBlockVersion-*@ keys are among the ones @migrate@ drops, as
        -- they now come from consensus defaults rather than configuration. A
        -- fixed default is used here, which surfaces as a divergence against POM
        -- for any configuration that still sets them (see 'adapterGaps').
