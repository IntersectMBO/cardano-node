...
drawUI p = case lvsScreen p of
  MainView ->
    [withBorder . withHeaderFooter p . withSideBar p $ systemStatsW p]
  Peers ->
    [withBorder . withHeaderFooter p . withSideBar p $ peerListContentW p]
 where
  ...
  withHeaderFooter lvs = vBox . (headerW lvs :) . (: [keysMessageW])

  withSideBar :: LiveViewState blk a -> Widget () -> Widget ()
  withSideBar lvs = hBox . (: [nodeInfoW lvs])

headerW :: LiveViewState blk a -> Widget ()
headerW p =
  C.hCenter . padTop (Pad 1) . padLeft (Pad 2) . padRight (Pad 2) $ hBox
    [ withAttr cardanoAttr . padRight (Pad 10) $ txt "CARDANO"
    , txt "release: "
    , withAttr releaseAttr $ str (lvsRelease p)
    , padLeft Max $ txt "Node: "
    , withAttr nodeIdAttr $ txt (lvsNodeId p)
    ]

keysMessageW :: Widget ()
keysMessageW = padBottom (Pad 1) . padLeft (Pad 1) $ hBox
  [ txt "Press "
...
nodeInfoLabels = padRight (Pad 3) $ vBox
  [ txt "version:"
  , txt "commit:"
  , txt "platform:"
...
nodeInfoW p =
  padTop (Pad 2) . padLeft (Pad 1) . padRight (Pad 2) . padBottom (Pad 2) $ hBox
    [nodeInfoLabels, nodeInfoValues p]


nodeInfoValues :: LiveViewState blk a -> Widget ()
nodeInfoValues lvs = withAttr valueAttr $ vBox
  [ str (lvsVersion lvs)
  , str (take 7 $ lvsCommit lvs)
  , str (lvsPlatform lvs)
  , padTop (Pad 1) $ str
    (formatTime defaultTimeLocale "%X"
    $
...
peerListContentW lvs =
  padLeft (Pad 1)
    ...
    . padTop (Pad 1)
    . vBox
    . ([ txt "Known peers" & padBottom (Pad 1)
...
systemStatsW p = padTop (Pad 1) . padLeft (Pad 1) . padRight (Pad 1) $ vBox
  [ hBox
    [ vBox
      [ hBox
        [ txt "Mempool (Bytes):"
        , withAttr barValueAttr
        . padLeft Max
        . str
        . show
        . (floor :: Float -> Int)
        . fromIntegral
        $ lvsMempoolMaxBytes p
        , txt " max"
        ]
  ...
  , vBox
    [ hBox
      ...
      . padLeft Max
      $ str
      $ withOneDecimal (max (lvsMemoryUsageMax p) 200.0)
      <> " MB"
      ]
    , padBottom (Pad 1) memUsageBar
    ]
  , vBox
    [ hBox [txt "CPU usage:", withAttr barValueAttr . padLeft Max $ str "100%"]
  ...
  , hBox
    [ vBox
      [ hBox
        ...
        . padLeft Max
        $ str
        $ withOneDecimal (max (lvsDiskUsageRMax p) 1.0)
        <> " KB/s"
        ]
    ...
    , padLeft (Pad 3) $ vBox
      [ hBox
        ...
        . padLeft Max
        $ str
        $ withOneDecimal (max (lvsDiskUsageWMax p) 1.0)
        <> " KB/s"
        ]
  ...
  , hBox
    [ vBox
      [ hBox
        ...
        . padLeft Max
        $ str
        $ withOneDecimal (max (lvsNetworkUsageInMax p) 1.0)
        <> " KB/s"
        ]
    ...
    , padLeft (Pad 3) $ vBox
      [ hBox
        ...
        . padLeft Max
        $ str
        $ withOneDecimal (max (lvsNetworkUsageOutMax p) 1.0)
        <> " KB/s"
        ]
 ...
 where
  ...
  memPoolBar =
    updateAttrMap
        (A.mapAttrNames
          [ (mempoolDoneAttr, progressCompleteAttr)
  ...
  memUsageBar =
    updateAttrMap
        (A.mapAttrNames
          [ (memDoneAttr, progressCompleteAttr)
  ...
  memLabel =
    Just
      $ withOneDecimal (lvsMemoryUsageCurr p)
      ++ " MB / max "
      ++ withOneDecimal (lvsMemoryUsageMax p)
      ++ " MB"
  cpuUsageBar :: forall n . Widget n
  cpuUsageBar =
    updateAttrMap
        (A.mapAttrNames
          [ (cpuDoneAttr, progressCompleteAttr)
  ...
  diskUsageRBar =
    updateAttrMap
        (A.mapAttrNames
          [ (diskIODoneAttr, progressCompleteAttr)
  ...
  networkUsageOutLabel =
    Just $ withOneDecimal (lvsNetworkUsageOutCurr p) ++ " KB/s"
...
progressToDoColorLFG, progressToDoColorLBG, progressDoneColorLFG, progressDoneColorLBG, progressToDoColorDFG, progressToDoColorDBG, progressDoneColorDFG, progressDoneColorDBG, darkMainBG
  :: Vty.Color
...
lightTheme :: Theme
lightTheme = newTheme (Vty.black `on` Vty.white) lightThemeAttributes

darkTheme :: Theme
darkTheme = newTheme (Vty.white `on` darkMainBG) darkThemeAttributes
