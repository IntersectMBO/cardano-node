...
liveViewPostSetup
  :: NFData a => LiveViewBackend blk a -> NodeCLI -> NodeConfiguration -> IO ()
liveViewPostSetup lvbe ncli nc = do
  modifyMVar_ (getbe lvbe) $ \lvs -> pure lvs
    { lvsNodeId = nodeId
 ...
 where
  ...
  nodeId :: Text
  nodeId = case ncProtocolConfig nc of
    NodeProtocolConfigurationMock NodeMockProtocolConfiguration { npcMockNodeId }
      -> Text.pack (show npcMockNodeId)

    _ -> Text.pack $ "Port: " <> case naPort <$> nodeAddr ncli of
      Nothing -> "-"
      Just port -> show port


setNodeThread :: NFData a => LiveViewBackend blk a -> Async.Async () -> IO ()
setNodeThread lvbe nodeThr = modifyMVar_ (getbe lvbe)
  $ \lvs -> return $ lvs { lvsNodeThread = LiveViewThread $ Just nodeThr }

storePeersInLiveView :: NFData a => [Peer blk] -> LiveViewBackend blk a -> IO ()
storePeersInLiveView peers lvbe =
  modifyMVar_ (getbe lvbe) $ \lvs -> pure $ lvs { lvsPeers = peers }
...
captureCounters lvbe trace0 = do
  ...
  modifyMVar_ (getbe lvbe)
    $ \lvs -> return $ lvs { lvsMetricsThread = LiveViewThread $ Just thr }
 where
