        ...
        case content of
          LogValue "Meresident" (PureI pages) ->
            modifyMVar_ (getbe lvbe) $ \lvs -> do
              ...
              return $
                lvs
                  { lvsMemoryUsageCurr = mbytes,
                    lvsMemoryUsageMax = maxMemory,
                    lvsUpTime = uptime
                  }
          LogValue "Meresident_size" (Bytes bytes) ->
            -- Darwin
            modifyMVar_ (getbe lvbe) $ \lvs -> do
          ...
          LogValue "IO.rchar" (Bytes bytesWereRead) ->
            modifyMVar_ (getbe lvbe) $ \lvs -> do
              ...
              return $
                lvs
                  { lvsDiskUsageRCurr = currentDiskRate,
          ...
          LogValue "IO.wchar" (Bytes bytesWereWritten) ->
            modifyMVar_ (getbe lvbe) $ \lvs -> do
              ...
              return
                $! lvs
                  { lvsDiskUsageWCurr = currentDiskRate,
          ...
          LogValue "Stat.utime" (PureI ticks) ->
            modifyMVar_ (getbe lvbe) $ \lvs -> do
              ...
              return $
                lvs
                  { lvsCPUUsagePerc = cpuperc,
                    lvsCPUUsageLast = ticks,
                    lvsCPUUsageNs = currentTimeInNs,
                    lvsUpTime = uptime
                  }
          LogValue "Sys.SysUserTime" (Nanoseconds nsecs) ->
            -- Darwin, Windows
            modifyMVar_ (getbe lvbe) $ \lvs -> do
...
updateNetInbound lvs currentTimeInNs inBytes =
  let (_timeDiff, currentNetRate) =
   ...
   in return $
        lvs
          { lvsNetworkUsageInCurr = currentNetRate,
...
updateNetOutbound lvs currentTimeInNs outBytes =
  let (_timeDiff, currentNetRate) =
   ...
   in return
        lvs
          { lvsNetworkUsageOutCurr = currentNetRate,
