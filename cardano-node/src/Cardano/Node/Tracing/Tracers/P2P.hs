...
import           Ouroboros.Network.TxSubmission.Inbound.V2.Types (TraceTxLogic (..), TxSubmissionCounters (..))
...
--------------------------------------------------------------------------------
-- TxSubmissionCounters Tracer
--------------------------------------------------------------------------------

instance LogFormatting TxSubmissionCounters  where
    forMachine = toObject . fromDetailLevel
    forHuman   = showT

instance MetaTrace TxSubmissionCounters where
    namespaceFor TxSubmissionCounters {} =
      Namespace [] ["TxSubmissionCounters"]

    severityFor (Namespace _ ["TxSubmissionCounters"]) _ = Just Debug
    severityFor _ _ = Nothing

    documentFor (Namespace _ ["TxSubmissionCounters"]) = Just $ mconcat
      [ "Counters generated by tx-submission inbound logic"
      ]
    documentFor _ = Nothing
