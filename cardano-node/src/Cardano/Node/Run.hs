      ...
      let diffusionNodeArguments :: Cardano.Diffusion.CardanoNodeArguments IO
          ...
          diffusionConfiguration =
            mkDiffusionConfiguration
              ...
              nc
      in
      let waitForRpcConfigChange oldConfig = atomically $ readTVar rpcConfigVar >>= \new -> check (new /= oldConfig)
          disableRpcServer = atomically . modifyTVar rpcConfigVar $ \config -> config{isEnabled = Identity False}
          rpcServerLoop = do
            config@RpcConfig{isEnabled = Identity enabled} <- readTVarIO rpcConfigVar
            if enabled
              then
                race_
                  -- Run server. If @runRpcServer@ exits, means that we got (and handled already) a fatal error.
                  -- To prevent looping infinitely here, we disable RPC server. This is recoverable if the user
                  -- sends SIGHUP to reload the config and reenable the server.
                  (do 
                    runRpcServer (rpcTracer tracers) (config, networkMagic)
                    traceWith (startupTracer tracers) RpcForceDisabled
                    disableRpcServer)
