...
mkConsensusProtocolByron NodeByronProtocolConfiguration { npcByronGenesisFile, npcByronGenesisFileHash, npcByronReqNetworkMagic, npcByronPbftSignatureThresh, npcByronApplicationName, npcByronApplicationVersion, npcByronSupportedProtocolVersionMajor, npcByronSupportedProtocolVersionMinor, npcByronSupportedProtocolVersionAlt } files
  = do
    ...
    return $ Consensus.ProtocolByron
      genesisConfig
      ...
      (Update.SoftwareVersion npcByronApplicationName npcByronApplicationVersion
      )
...
readGenesis
  ...
  -> ExceptT ByronProtocolInstantiationError IO Genesis.Config
readGenesis (GenesisFile file) mbExpectedGenesisHash ncReqNetworkMagic = do
 ...
 where
  checkExpectedGenesisHash
    :: Genesis.GenesisHash -> ExceptT ByronProtocolInstantiationError IO ()
  checkExpectedGenesisHash actual' = case mbExpectedGenesisHash of
    Just expected | actual /= expected -> throwError
      (GenesisHashMismatch actual expected)
      where actual = fromByronGenesisHash actual'
...
readLeaderCredentials genesisConfig (Just ProtocolFilepaths { byronCertFile, byronKeyFile })
  = case (byronCertFile, byronKeyFile) of
    ...
    (Just delegCertFile, Just signingKeyFile) -> do
      ...
      signingKey <-
        firstExceptT (SigningKeyDeserialiseFailure signingKeyFile)
        . hoistEither
      ...
      bimapExceptT PbftError Just . hoistEither $ mkPBftLeaderCredentials
        genesisConfig
        signingKey
        delegCert
 ...
 where
  deserialiseSigningKey
    :: LB.ByteString -> Either DeserialiseFailure Byron.Crypto.SigningKey
  deserialiseSigningKey = fmap (Byron.Crypto.SigningKey . snd)
...
renderByronProtocolInstantiationError pie = case pie of
  CanonicalDecodeFailure fp failure ->
  ...
  GenesisConfigurationError fp genesisConfigError ->
    "Genesis configuration error in: "
      <> toS fp
      <> " Error: "
      <> (Text.pack $ show genesisConfigError)
  ...
  PbftError pbftLeaderCredentialsError ->
    "PBFT leader credentials error: "
      <> (Text.pack $ show pbftLeaderCredentialsError)
  SigningKeyDeserialiseFailure fp deserialiseFailure ->
    "Signing key deserialisation error in: "
      <> toS fp
      <> " Error: "
      <> (Text.pack $ show deserialiseFailure)
