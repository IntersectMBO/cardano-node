...
-- | No real PlutusV4 cost model exists yet: cardano-ledger's own
-- testingCostModel PlutusV4 (testlib) reuses the PlutusV3 params for it, so
-- this does the same, reusing the identical array cardano-node's own
-- Cardano.Node.Protocol.Conway already hardcodes as plutusV3ExampleValues.
defaultPlutusV4CostModel :: L.CostModel
defaultPlutusV4CostModel =
  case L.mkCostModelsLenient (Map.singleton 2 plutusV3ExampleValues) >>= Map.lookup L.PlutusV4 . L.costModelsValid of
    Nothing -> error "defaultPlutusV4CostModel: missing PlutusV4 cost model in default cost models."
    Just cm -> cm
...
emptyDijkstraGenesis =
  let upgradePParamsDef =  UpgradeDijkstraPParams
                            ...
                            , udppPlutusV4CostModel = defaultPlutusV4CostModel
                            , udppLeiosAnnouncementPeriodLength = Milliseconds32 1000
                            , udppLeiosVotePeriodLength = Milliseconds32 4000
                            , udppLeiosDiffusionPeriodLength = Milliseconds32 7000
                            , udppLeiosCommitteeSize = 900
