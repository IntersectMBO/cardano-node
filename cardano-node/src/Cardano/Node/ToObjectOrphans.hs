...
instance (Condense (HeaderHash blk), ProtocolLedgerView blk)
    ...
    toObject verb p =
        mkObject [ "kind" .= String "Tip"
                , "tip" .= String (showTip' verb p) ]
showTip' :: (Condense (HeaderHash blk))
        => TracingVerbosity -> Point blk -> Text
showTip' verb tip =
    let n :: Int = case verb of
                MinimalVerbosity -> 7
                NormalVerbosity -> 7
                MaximalVerbosity -> 99
    in
    case pointHash tip of
        GenesisHash -> "genesis"
        BlockHash h -> pack $ take n (condense h)

instance (Condense (HeaderHash blk), ProtocolLedgerView blk)
            => ToObject (ChainDB.TraceEvent blk) where

    toObject verb (ChainDB.TraceAddBlockEvent ev) =
        case ev of
    ...
    toObject verb (ChainDB.TraceLedgerReplayEvent ev) =
        case ev of
    ...
    toObject verb (ChainDB.TraceLedgerEvent ev) =
        case ev of
    ...
    toObject verb (ChainDB.TraceCopyToImmDBEvent ev) =
        case ev of
    ...
    toObject verb (ChainDB.TraceGCEvent ev) =
        case ev of
    ...
    toObject verb (ChainDB.TraceOpenEvent ev) =
        case ev of
