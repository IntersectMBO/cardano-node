...
chairmanTest
  ...
  -> NetworkMagic
  -> IO ()
chairmanTest tracer slotLength securityParam runningTime optionalProgressThreshold socketPaths someNodeClientProtocol nw
  = do
    ...
    SomeNodeClientProtocol (p :: ProtocolClient blk (BlockProtocol blk)) <-
 ...
 where
  progressThreshold =
    deriveProgressThreshold slotLength runningTime optionalProgressThreshold
...
--
deriveProgressThreshold :: SlotLength -> DiffTime -> Maybe BlockNo -> BlockNo
...
instance HasHeader blk => Exception (ConsensusFailure blk) where
  displayException (ConsensusFailure (peerid1, tip1) (peerid2, tip2) intersection (SecurityParam securityParam))
    = concat
      [ "consensus failure:\n"
      , "node at "
      , show peerid1
      ...
      , show tip2
      , "\n"
      , "but their chain intersection is at "
      , show intersection
      , "\n"
      , "which is further back than the security param K "
      , show securityParam
      ]
...
consensusCondition (SecurityParam securityParam) chains =
  ...
  in  case find (forkTooLong . snd) forks of
        Just ((peerid1, peerid2), (intersection, tip1, tip2)) ->
          Left $ ConsensusFailure (peerid1, AF.anchorToTip tip1)
                                  (peerid2, AF.anchorToTip tip2)
                                  intersection
                                  (SecurityParam securityParam)
        Nothing -> Right $ ConsensusSuccess
 ...
 where
  ...
  chainForkPoints chain1 chain2 = case AF.intersect chain1 chain2 of
  ...
  forkTooLong (intersection, tip1, tip2) =
    ...
    forkLen tip1 > securityParam && forkLen tip2 > securityParam
   where
...
instance HasHeader blk => Exception (ProgressFailure blk) where
  displayException (ProgressFailure minBlockNo peerid tip) = concat
    [ "progress failure:\n"
    , "the node at "
    , show peerid
    , " has chain tip "
    , show tip
    , "\n"
    , "while the mininum expected block number is "
    , show minBlockNo
    ]
...
runChairman tracer cfg networkMagic securityParam runningTime socketPaths = do
  ...
  let initialChains = Map.fromList
        [ (socketPath, AF.Empty AF.AnchorGenesis) | socketPath <- socketPaths ]
  chainsVar <- newTVarM initialChains

  void $ timeout runningTime $ withIOManager $ \iomgr ->
    forConcurrently_ socketPaths $ \sockPath -> createConnection tracer
                                                                 iomgr
                                                                 cfg
                                                                 networkMagic
                                                                 sockPath
...
-- connection.
handleMuxError
  :: Tracer IO String -> ChainsVar IO blk -> SocketPath -> MuxError -> IO ()
...
createConnection
  ...
  -> SecurityParam
  -> IO ()
createConnection tracer iomgr cfg networkMagic socketPath@(SocketPath path) chainsVar securityParam
  = connectTo
      (localSnocket iomgr path)
      NetworkConnectTracers { nctMuxTracer = nullTracer
                            , nctHandshakeTracer = nullTracer
                            }
      (localInitiatorNetworkApplication (showTracing tracer)
                                        nullTracer
                                        nullTracer
                                        cfg
                                        networkMagic
...
-- corresponding entry.
--
type ChainsVar m blk
  = StrictTVar m (Map SocketPath (AnchoredFragment (Header blk)))
...
chainSyncClient tracer sockPath chainsVar securityParam =
...
checkConsensus
  :: forall blk m
   . (MonadSTM m, MonadThrow (STM m), HasHeader blk, HasHeader (Header blk))
  => ChainsVar m blk
  -> SecurityParam
...
localInitiatorNetworkApplication
  :: forall blk m
   . (RunNode blk, MonadAsync m, MonadST m, MonadTimer m, MonadThrow (STM m))
  => Tracer m (ChairmanTrace blk)
...
localInitiatorNetworkApplication chairmanTracer chainSyncTracer localTxSubmissionTracer cfg networkMagic sockPath chainsVar securityParam
  = foldMapVersions
    (\(version, blockVersion) -> versionedNodeToClientProtocols
      version
      versionData
 ...
 where
  ...
  protocols byronClientVersion = NodeToClientProtocols
    ...
    , localStateQueryProtocol = InitiatorProtocolOnly $ MuxPeer
                                  nullTracer
                                  cStateQueryCodec
