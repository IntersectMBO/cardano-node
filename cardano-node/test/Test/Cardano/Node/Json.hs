...
prop_roundtrip_NodeAddress_JSON = Hedgehog.property $ do
  na <- Hedgehog.forAll genNodeAddress
  Hedgehog.tripping na toJSON fromJSON
  Hedgehog.tripping na encode decode
  Hedgehog.cover 1 "  has address"
    $ isJust (unNodeHostAddress $ naHostAddress na)
  Hedgehog.cover 1 "  no address"
    $ isNothing (unNodeHostAddress $ naHostAddress na)

prop_roundtrip_NodeHostAddress_JSON :: Property
prop_roundtrip_NodeHostAddress_JSON = Hedgehog.property $ do
...
prop_roundtrip_NodeSetup_JSON = Hedgehog.property $ do
...
prop_roundtrip_NetworkTopology_JSON = Hedgehog.property $ do
