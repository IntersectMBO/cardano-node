...
genUseLedgerPeers = do
    ...
    if slot >= 0 then return $ UseLedgerPeers $ UseLedgerAfter $ SlotNo $ fromIntegral slot
                 else return DontUseLedgerPeers

genDomainAccessPoint :: Gen DomainAccessPoint
genDomainAccessPoint =
    DomainAccessPoint <$> domains <*> Gen.integral Range.constantBounded
  where
    domains = Gen.element
                [ "test1"
                , "test2"
...
genUseBootstrapPeers = do
  domains <- Gen.list Range.constantBounded genDomainAccessPoint
