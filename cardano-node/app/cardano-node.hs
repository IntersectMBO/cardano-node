...
main = toplevelExceptionHandler $ do
 ...
 where
  ...
  opts = Opt.info
    ...
    (Opt.fullDesc <> Opt.progDesc "Start node of the Cardano blockchain.")

  helperBrief :: String -> String -> String -> Parser (a -> a)
  helperBrief l d helpText =
    Opt.abortOption (Opt.InfoMsg helpText) $ mconcat [Opt.long l, Opt.help d]
...
parseVersionCmd =
  Opt.subparser
      (mconcat
        ...
        , Opt.metavar "version"
        , Opt.hidden
        , command' "version" "Show the cardano-node version" (pure VersionCmd)
        ]
      )
    <|> Opt.flag'
          VersionCmd
...
runVersionCommand = putTextLn $ mconcat
  [ "cardano-node "
  ...
  where renderVersion = Text.pack . showVersion


runRunCommand :: NodeCLI -> IO ()
runRunCommand npm = do

  eLoggingLayer <- runExceptT
    $ createLoggingLayer (Text.pack (showVersion version)) npm
