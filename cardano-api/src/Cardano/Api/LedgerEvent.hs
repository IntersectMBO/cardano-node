...
import qualified Cardano.Ledger.Shelley.Rules.Rupd as Rupd
...
instance
  ( Crypto ledgerera ~ StandardCrypto,
    Event (Ledger.Core.EraRule "RUPD" ledgerera) ~ Rupd.RupdEvent (Crypto ledgerera),
    Event (Ledger.Core.EraRule "TICK" ledgerera) ~ ShelleyTickEvent ledgerera,
    Event (ShelleyNewEpochEvent ledgerera) ~ ShelleyNewEpochEvent ledgerera,
    Ledger.Core.EraRule "NEWEPOCH" ledgerera ~ ShelleyNewEpochEvent ledgerera,
...
pattern LEDeltaRewardEvent ::
  ( Crypto ledgerera ~ StandardCrypto,
    ...
    Event (Ledger.Core.EraRule "RUPD" ledgerera) ~ Rupd.RupdEvent StandardCrypto
  ) =>
  EpochNo ->
  Map StakeCredential (Set (Reward StandardCrypto)) ->
  AuxLedgerEvent (LedgerState (ShelleyBlock protocol ledgerera))
pattern LEDeltaRewardEvent e m <-
  ShelleyLedgerEventTICK
    (NewEpochEvent (DeltaRewardEvent (Rupd.RupdEvent e (Map.mapKeys fromShelleyStakeCredential -> m))))
