...
nextEpochEligibleLeadershipSlots sbe sGen serCurrEpochState ptclState
  ...
  let -- TPraos.PrtclState _ evolvingNonce candidateNonce = TPraos.csProtocol chainDepState
      -- evolvingNonce = error "" :: Consensus.Nonce
      -- candidateNonce = error "" :: Consensus.Nonce
      Consensus.PraosNonces { Consensus.candidateNonce, Consensus.evolvingNonce } = Consensus.getPraosNonces (Proxy @(ConsensusProtocol era)) chainDepState
