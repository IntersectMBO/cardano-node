...
import           Cardano.Chain.UTxO.Validation  ( TxValidationError(..)
                                                , UTxOValidationError(..)
                                                )
...
renderApplyMempoolPayloadErr err = case err of
...
renderValidationError ve = case ve of
...
renderTxValidationError tve = "Tx Validation: " <> case tve of
  TxValidationLovelaceError txt e ->
    sformat ("Lovelace error " % stext % ": " % build) txt e
  TxValidationFeeTooSmall tx expected actual -> sformat
    ("Tx " % build % " fee " % build % "too low, expected " % build)
    tx
    actual
    expected
  TxValidationWitnessWrongSignature wit pmid sig -> sformat
    ("Bad witness "
    % build
    % " for signature "
    % stext
  ...
  TxValidationNetworkMagicMismatch expected actual -> mconcat
    ["Bad network magic  ", textShow actual, ", expected ", textShow expected]
  TxValidationTxTooLarge expected actual -> mconcat
    [ "Tx is "
    , textShow actual
    , " bytes, but expected < "
    , textShow expected
  ...
  TxValidationUnknownAddressAttributes -> "Unknown address attributes"
  TxValidationUnknownAttributes -> "Unknown attributes"

renderUTxOError :: UTxOError -> Text
renderUTxOError ue = "UTxOError: " <> case ue of
