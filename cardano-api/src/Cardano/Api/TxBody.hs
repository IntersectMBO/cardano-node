...
data TxInsReference build era where
     ...
     TxInsReferenceNone :: TxInsReference buil era

     TxInsReference     :: ReferenceTxInsScriptsInlineDatumsSupportedInEra era
                        -- (potential input to be spent, _ Reference input)
                        -> [(Maybe TxIn, BuildTxWith build (Witness WitCtxTxIn era), TxIn)]
...
-- This is fine we only trying to view
  ...
  case refInsScriptsAndInlineDatsSupportedInEra $ shelleyBasedToCardanoEra era of
    ...
    Just suppInEra ->
      ...
      in TxInsReference suppInEra
           $ zip3 spendingTxIns (repeat ViewTx) (map fromShelleyTxIn . Set.toList $ ledgerRefInputs)
 where
...
makeShelleyTransactionBody era@ShelleyBasedEraBabbage
    ...
    return $
      ShelleyTxBody era
        ...
        txScriptValidity
  where
    getReferenceInputs :: [(Maybe TxIn, BuildTxWith BuildTx (Witness WitCtxTxIn era), TxIn)] -> [TxIn]
    getReferenceInputs = map (\(_,_,i) -> i)
    ...
    getInputsToBeSpentByReferenceInputs
      :: [(Maybe TxIn, BuildTxWith BuildTx (Witness WitCtxTxIn era), TxIn)] -> [(TxIn, BuildTxWith BuildTx (Witness WitCtxTxIn era))]
    getInputsToBeSpentByReferenceInputs ((Just i,sw,_) : x) = (i,sw) : getInputsToBeSpentByReferenceInputs x
    getInputsToBeSpentByReferenceInputs ((Nothing,_,_) : x) = getInputsToBeSpentByReferenceInputs x
    getInputsToBeSpentByReferenceInputs [] = []
   ...
   -- Here we need to be careful and take into account reference sripts
...
collectTxBodyScriptWitnesses TxBodyContent {
  ...
  where
    ...
    -- TODO: This needs to be modified. How? Spending inputs now exist in the RedemeerTxIn type
    -- so we must extract those, add them to the "normal" txins, order them and then
    -- we can get the correct scriptwitness index!
    scriptWitnessesTxIns
      :: [(TxIn, BuildTxWith BuildTx (Witness WitCtxTxIn era))]
      -> [(ScriptWitnessIndex, AnyScriptWitness era)]
    scriptWitnessesTxIns txins =
        [ (ScriptWitnessIndexTxIn ix, AnyScriptWitness witness)
        -- !!!!So your reference input should not be the txin in the tuple. it should exist in the witness!!!
          -- The tx ins are indexed in the map order by txid
