...
import           Control.Concurrent.STM.TVar (modifyTVar', writeTVar, readTVar, readTVarIO, newTVarIO)
  ...
  case BM.lookup nodeId nodesNames of
    ...
    Nothing -> do
      ...
      maybePair <- atomically do
        oldBimap <- readTVar connectedNodesNames
        ...
        for_ @Maybe maybePair \pair ->
          writeTVar connectedNodesNames $ 
            uncurry BM.insert pair oldBimap
