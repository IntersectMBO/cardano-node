...
runAcceptorsClient tracerEnv tracerEnvRTView howToConnect (ekgConfig, tfConfig, dpfConfig) = withIOManager \iocp -> do
  traceWith (teTracer tracerEnv) $ TracerSockConnecting
    (case howToConnect of
       EKGF.LocalPipe p -> p
       EKGF.RemoteSocket host port -> Text.unpack host ++ ":" ++ show port)
  case howToConnect of
    EKGF.LocalPipe p -> do
    ...
    EKGF.RemoteSocket host port -> do
      listenAddress:|_ <- Socket.getAddrInfo Nothing (Just (undefined host)) (Just (undefined port))
