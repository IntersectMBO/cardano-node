...
propDataPoint rootDir localSock = do
  ...
  withAsync (doRunCardanoTracer config stopProtocols dpAskers) . const $
    withAsync (launchForwardersSimple Responder localSock 1000 10000) . const $ do
      sleep 1.0
      -- We know that there is one single "node" only (and one single asker too).
      ((_, dpAsker):_) <- M.toList <$> readTVarIO dpAskers
      dpValues <- askForDataPoints dpAsker ["test.data.point", "some.wrong.Name"]
