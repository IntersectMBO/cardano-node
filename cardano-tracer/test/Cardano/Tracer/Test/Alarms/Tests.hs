...
-- | Generate a filter that narrows the given ceiling: fixed single-value
--   constraints are kept verbatim, open ones may become fixed, the minimum
--   severity may only rise, and the scope/label maps may only gain entries.
genNarrowing :: AlarmFilter -> Gen AlarmFilter
genNarrowing ceiling_ = do
  src   <- keepOrFix (afSource ceiling_) genSource
  rid   <- keepOrFix (afRuleId ceiling_) genRule
  sev   <- case afMinSeverity ceiling_ of
             Nothing -> oneof [pure Nothing, Just <$> genSeverity]
             Just s  -> Just <$> genSeverity `suchThat` (>= s)
  -- left-biased union: every ceiling entry survives with its value
  scope <- Map.union (afScope ceiling_) <$> genLabels
  lbls  <- Map.union (afLabels ceiling_) <$> genLabels
  pure (AlarmFilter src rid sev scope lbls)
 where
  keepOrFix :: Maybe a -> Gen a -> Gen (Maybe a)
  keepOrFix (Just c) _   = pure (Just c)
  keepOrFix Nothing  gen = oneof [pure Nothing, Just <$> gen]

-- | If 'b' narrows 'a' and 'c' narrows 'b', then 'c' narrows 'a'.
--   Guarantees the ceiling relation composes across chained checks. The
--   chain is generated by construction ('genNarrowing') — random
--   independent filters almost never narrow each other, so an '==>'
--   precondition would discard virtually every case.
propFilterNarrowsTransitive :: Property
propFilterNarrowsTransitive =
  forAll genFilter \a ->
  forAll (genNarrowing a) \b ->
  forAll (genNarrowing b) \c ->
    counterexample ("a=" <> show a <> " b=" <> show b <> " c=" <> show c) $
      conjoin
        [ counterexample "generator must narrow: b vs a" (filterNarrows a b)
        , counterexample "generator must narrow: c vs b" (filterNarrows b c)
        , counterexample "transitivity violated: c vs a" (filterNarrows a c)
        ]
