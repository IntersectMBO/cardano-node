...
import           Cardano.Tracer.Run (runCardanoTracerWithConfigBrake)
...
propLogs format rootDir localSock = do
  ...
  withAsync (runCardanoTracerWithConfigBrake (config rootDir localSock) stopProtocols) $ \_ ->
    withAsync (launchForwardersSimple Responder localSock 1000 10000) $ \_ -> do
      sleep 15.0 -- Wait till some rotation is done.
      atomically $ modifyTVar' stopProtocols . const $ True
      sleep 1.0
