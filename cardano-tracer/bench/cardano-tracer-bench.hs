...
main = do
  ...
  let myBench :: TracerConfig -> [TraceObject] -> Benchmarkable
      myBench config traceObjects = let
        ...
        cleanup TracerEnv{teRegistry} = elemsRegistry teRegistry >>= traverse_ (hClose . fst)

        benchmark :: TracerEnv -> IO ()
        benchmark traceEnv = beforeProgramStops do traceObjectsHandler traceEnv nId traceObjects
