...
You can think of `cardano-node` as a **producer** of logging/monitoring information, and `cardano-tracer` as a **consumer** of this information. After the network connection between them is established, `cardano-tracer` periodically asks for such an information, and `cardano-node` replies with it.
...
3. Data points, which contains arbitrary information about the node. Please note that `cardano-tracer` asks for new data points only by _explicit_ request, there is no constant asking.
...
Distributed scenario is for real-world case: for example, you have `N` nodes working on `N` different AWS-instances, and you want to collect all the logging/monitoring information from these nodes using one `cardano-tracer` process working on your machine. It is **highly recommended** to use distributed scenario.
...
This is an example with 3 nodes and one tracer:
...
Since you already have your SSH credentials, the connection between your machines will be secure.
...
After you open `http://127.0.0.1:3000` in your browser, you will see the list of identifiers of connected nodes (or the warning message, if there are no connected nodes yet).

Each identifier is a hyperlink to the page where you will see the list of metrics received from the corresponding node.
...
Here the page with the list of identifiers of connected nodes will be available at `http://127.0.0.1:3100`. After clicking to some hyperlink, you will be redirected to `http://127.0.0.1:3101` where you will see EKG monitoring page for corresponding node.
