...
| `m{environment="mainnet"}` | `m{environment = "mainnet"}` (`=` and `!=` only) |
| `> bool`, `== bool` | plain `>`/`==` — comparisons are typed booleans already |
| `and` / `or` (scalar logic) | `&&` / `||` |
...
alarms:
  timeseriesRules:
    ...
    - ruleId: mempool-high                     # cardano_node_mempool_high
      ...
      severity: warning
      query: "cardano_node_metrics_txsInMempool_int > 200"
      evaluateEvery: 30s
      for: 10m
    ...
    - ruleId: slot-leadership-checks-missed    # too_many_slot_leadership_checks_missed
      ...
      severity: critical
      query: "rate (cardano_node_metrics_slotsMissed_int[now - 5m; now]) > 0.5"
      evaluateEvery: 30s
      for: 2m
    ...
    - ruleId: kes-expiry-10-periods            # cardano_node_KES_expiration_metric_10period_notice
      ...
      severity: warning
      query: "cardano_node_metrics_remainingKESPeriods_int <= 10"
      evaluateEvery: 5m
      for: 5m
    - ruleId: kes-expiry-5-periods
      summary: "Less than 5 KES periods remaining"
      severity: error
      query: "cardano_node_metrics_remainingKESPeriods_int <= 5"
      evaluateEvery: 5m
      for: 5m
    - ruleId: kes-expiry-1-period
      summary: "KES expires within 1 period"
      severity: critical
      query: "cardano_node_metrics_remainingKESPeriods_int <= 1"
      evaluateEvery: 5m
      for: 5m
    ...
    - ruleId: blockfetch-delay-high            # cardano_blockfetchclient_blockdelay_high
      ...
      severity: warning
      query: "cardano_node_metrics_blockfetchclient_blockdelay_cdfFive_real < 0.90"
      evaluateEvery: 1m
      for: 10m
    ...
    - ruleId: blockfetch-delay-critical        # cardano_blockfetchclient_blockdelay_critical
      ...
      severity: critical
      query: "cardano_node_metrics_blockfetchclient_blockdelay_cdfFive_real < 0.50"
      evaluateEvery: 1m
      for: 10m
    ...
    - ruleId: connection-count-high            # cardano_connection_count_high
      ...
      severity: warning
      query: "cardano_node_metrics_connectionManager_inboundConns_int > 450"
      evaluateEvery: 1m
      for: 10m
    ...
    - ruleId: chain-density-degraded           # chain_quality_degraded
      ...
      severity: warning
      query: '100 * quantile_by ("environment") 0.2 (cardano_node_metrics_density_real{environment != "preview"} * 20) < 70'
      evaluateEvery: 1m
      for: 5m
```
...
All of this targets the Phase-2 timeseries rule evaluator from the concept
doc, which is not yet implemented. The value of this exercise now: the
catalogue above is a realistic acceptance suite for that evaluator — if these
rules can be configured and fire correctly, Phase 2 is functionally complete
for SRE use.
